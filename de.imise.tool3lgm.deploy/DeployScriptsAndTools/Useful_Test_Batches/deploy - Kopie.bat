...
::Pfad zu 7zip
SET "SEVENZIPZIP_PROGRAM_DIR=7zip"
...
::Pfad zum lib-Verzeichnis im Deploy-Projekt des Tools
::SET "DEPLOY_PROJECT_TOOL3LGM_LIB_DIR=%DEPLOY_PROJECT_TOOL3LGM_DIR%\lib\"
::Pfad zum Plugins-Verzeichnis im Deploy-Projekt des Tools
::SET "DEPLOY_PROJECT_TOOL3LGM_PLUGIN_DIR=%DEPLOY_PROJECT_TOOL3LGM_DIR%\Plugins\"
...
SET "DEPLOY_TOOL3LGM_ICON=..\Icons\toolIcon_gross.ico"
...
SET "CLH=/wait java -classpath ^"%DEPLOY_TOOLS_JAR%^" de.axs.deploytools.ChangeLineHandler"
...
CALL mvn -B clean install
::suche die jar-Datei im target-Ordner
FOR %%F IN (target\*.jar) DO (
 set AXSUTILS_JAR=%AXSUTILS_PROJECT_DIR%\%%F
 goto axsutils_jar_found
)
:axsutils_jar_found
CD /D %SCRIPT_LOCATION%
ECHO %AXSUTILS_JAR%
...
::suche die jar-Datei im target-Ordner
FOR %%F IN (target\*.jar) DO (
 set TOOL3LGM_JAR=%TOOL3LGM_PROJECT_DIR%\%%F
 goto tool3lgm_jar_found
)
:tool3lgm_jar_found
CD /D %SCRIPT_LOCATION%
ECHO %TOOL3LGM_JAR%
...
::suche die jar-Datei im target-Ordner
FOR %%F IN (target\*.jar) DO (
 set METAMODEL_ORIGINAL_JAR=%METAMODEL_ORIGINAL_PROJECT_DIR%\%%F
 goto metamodel_original_jar_found
)
:metamodel_original_jar_found
CD /D %SCRIPT_LOCATION%
ECHO %METAMODEL_ORIGINAL_JAR%
...
::suche die jar-Datei im target-Ordner
FOR %%F IN (target\*.jar) DO (
 set METAMODEL_SERVICE_JAR=%METAMODEL_SERVICE_PROJECT_DIR%\%%F
 goto metamodel_service_jar_found
)
:metamodel_service_jar_found
CD /D %SCRIPT_LOCATION%
ECHO %METAMODEL_SERVICE_JAR%
...
::suche die jar-Datei im target-Ordner
FOR %%F IN (target\*.jar) DO (
 set IHE_TEMPLATE_PLUGIN_JAR=%IHE_TEMPLATE_PROJECT_DIR%\%%F
 goto ihe_template_plugin_jar_found
)
:ihe_template_plugin_jar_found
CD /D %SCRIPT_LOCATION%
ECHO %IHE_TEMPLATE_PLUGIN_JAR%

CD /D %SCRIPT_LOCATION%

::Das Kopieren sollten bereits die pom.xml per Maven erledigen
::copy createt jars to the Deploy Directory
::COPY /Y "%TOOL3LGM_PROJECT_DIR%\target\tool3lgm-1.0.jar" "%DEPLOY_PROJECT_TOOL3LGM_LIB_DIR%\tool3lgm.jar"
::COPY /Y "%AXSUTILS_PROJECT_DIR%target\de-axs-utils-1.0.0.jar" "%DEPLOY_PROJECT_TOOL3LGM_LIB_DIR%\de-axs-utils-1.0.0.jar"
...
DEL %DEPLOY_PROJECT_TOOL3LGM_START_BAT% /s /q
::Siehe https://documentation.help/bat-to-exe-converter/de.html#cmd
CALL Bat_To_Exe_Converter.exe -bat %DEPLOY_PROJECT_TOOL3LGM_START_BAT% -save %DEPLOY_PROJECT_START_TOOL3LGM_EXE% -icon %DEPLOY_TOOL3LGM_ICON% -invisible
...
CD /D %DEPLOY_DESTINATION_DIR%

:: ZIP the deploy Files with 7zip
FOR /d %%X IN ("%DEPLOY_PROJECT_TOOL3LGM_DIR%") DO "%SEVENZIP_PROGRAM_DIR%\7z.exe" a "%%X_V%lgmVersion%.zip" "%%X\"
