...
::SET "CYGWIN_PROGRAM_DIR=cygwin64\bin"
SET "CYGWIN_PROGRAM_DIR=C:\Program Files\cygwin64"
::das Shell-Script, welches im Cygwin das Tool-Start-Script ausführbar macht und dann
::das Tool als tar.gz-Datei packt
SET "CYGWIN_DEPLOY_TARGZ_SCRIPT_FILE=%CYGWIN_PROGRAM_DIR%\DeployTool3LGMTarZip.sh"
...
::Name des Deploy-Ordner des Tools. Das ist auch der Name des Ordners, in dem das Tool in die zip und
::tar.gz-Dateien zu packende Tool liegt. ACHTUNG: das Cygwin-ShellScript DeployTool3lgmTarZip.sh geht
::von genau diesem Namen aus.
...
::"CYGWIN_PROGRAM_DIR=C:\Program Files\cygwin64"
::"CYGWIN_DEPLOY_TARGZ_SCRIPT_FILE=%CYGWIN_PROGRAM_DIR%\DeployTool3LGMTarZip.sh"
::ShellScript zum Erstellen der tar.gz-Datei ins home/ des aktuellen Benutzers kopieren
SET "CYGWIN_USER_HOME=%CYGWIN_PROGRAM_DIR%\home\%USERNAME%"
XCOPY "%CYGWIN_DEPLOY_TARGZ_SCRIPT_FILE%" "%CYGWIN_USER_HOME%\make_targz.sh*" /r /y

SET "CYGWIN_DEPLOY_TOOL3LGM_DIR=%CYGWIN_USER_HOME%\%DEPLOY_PROJECT_TOOL3LGM_DIR_NAME%"
RD "C:\Program Files\cygwin64\home\Ich\3LGM2Tool" /s /q
XCOPY "%DEPLOY_PROJECT_TOOL3LGM_DIR%" "%CYGWIN_DEPLOY_TOOL3LGM_DIR%\" /S /E
CALL "%CYGWIN_PROGRAM_DIR%\bin\bash.exe" -l -c "$HOME/make_targz.sh"
::RD "C:\Program Files\cygwin64\home\Ich\3LGM2Tool" /s /q
