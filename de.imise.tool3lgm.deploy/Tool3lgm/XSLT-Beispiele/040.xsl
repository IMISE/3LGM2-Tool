...
<!--name: Anwendungsbaustein auf Physischen Datenverarbeitungsbaustein -->
<!--type: html -->
<!--description: Welche Anwendungsbausteine sind auf welchen Physischen Datenverarbeitungsbausteinen installiert? -->
...
<xsl:variable name="header_text" select="'Welche Anwendungsbausteine sind auf welchen Physischen Datenverarbeitungsbausteinen installiert?'" />
<xsl:variable name="cont_text" select="'Inhalt'" />
<xsl:variable name="comp_text" select="'rechnerunterstützte Anwendungsbausteine'" />
...
<xsl:variable name="conf_text" select="'Datenverarbeitungsbaustein-Konfiguration'" />
...
<html>
	...
	<body>
		...
		<ul>
			...
			<li><a href="#gem"><xsl:value-of select="$hybr_text" /></a></li>
		...
		<a name="gem"><h3><xsl:value-of select="$hybr_text" /></h3></a>
		<table border="1" cellpadding="3" cellspacing="0" width="100%">
			<thead><tr><th width="40%"><xsl:value-of select="$appl_text" /></th><th width="20%"><xsl:value-of select="$conf_text" /></th><th width="40%"><xsl:value-of select="$phys_text" /></th></tr></thead>
			<tbody align="left" valign="top">
				<xsl:apply-templates select="modell_3lgm_2/objects/element[@class='Anwendungsbaustein']">
					<xsl:sort select="str:removeLineBreak(child::field[@name='name'])" order="ascending" data-type="text" />
				</xsl:apply-templates>
			</tbody>
		</table>
...
<xsl:template match="modell_3lgm_2/objects/element[@class='Anwendungsbaustein']">
	<tr>
		<td>
			<xsl:call-template name="getRowspanRekursiv">
				<xsl:with-param name="kanten" select="key('kanteDbkAnb', @hash)" />
...
<xsl:template name="physischer_DVBaustein">
	...
	<xsl:for-each select="key('kantePhyDbk', $konfiguration)">
		<td><xsl:value-of select="str:removeLineBreak(key('hash', child::field[@name='end'])/field[@name='name'])" /></td>
		<xsl:if test="position() != last()" >
			<xsl:text disable-output-escaping="yes">&lt;/tr&gt;&lt;tr&gt;</xsl:text>
		</xsl:if>
	</xsl:for-each>
