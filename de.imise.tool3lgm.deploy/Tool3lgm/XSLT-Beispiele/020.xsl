...
<!--name: Art der Erledigung von Aufgaben -->
<!--type: html -->
<!--description: Welche Aufgaben werden noch konventionell erledigt, bzw. wo gibt es verschiedene Möglichkeiten? -->
...
<xsl:variable name="header_text" select="'Welche Aufgaben werden noch papierbasiert erledigt, bzw. wo gibt es verschiedene M&#246;glichkeiten?'" />
<xsl:variable name="task_text" select="'Aufgabe'" />
<xsl:variable name="comp_text" select="'rechnerunterbasierte Anwendungsbausteine'" />
<xsl:variable name="paper_text" select="'papierbasierte Anwendungsbausteine'" />
<xsl:variable name="hybr_text" select="'gemischte Anwendungsbausteine'" />
...
<xsl:key name="ANB" match="/modell_3lgm_2/objects/element[@class='AwbAwbkVerbindung']" use="child::field[@name='start']" />
...
<html>
	...
	<body>
		...
		<table border="1" cellpadding="3" cellspacing="0" width="100%">
			<tr><th width="16%"><xsl:value-of select="$task_text" /></th><th width="28%"><xsl:value-of select="$comp_text" /></th><th width="28%"><xsl:value-of select="$paper_text" /></th><th width="28%"><xsl:value-of select="$hybr_text" /></th></tr>
...
<xsl:template match="element[@class='Aufgabe']">
		<tr>
			<td width="16%"><xsl:value-of select="str:removeLineBreak(field[@name='name'])" /></td>
...
<xsl:template name="anwendungsbaustein">
	...
	<td width="28%">
		<xsl:for-each select="key('AufOrgKomb', $aufgabe)" >
			<xsl:for-each select="key('ABK', field[@name='end'])">
				<xsl:for-each select="key('ANB', child::field[@name='end'])">
					<xsl:variable name="awb" select="key('hash', child::field[@name='end'])" />
					<xsl:if test="$awb/@class = 'RechAnwendungsbaustein'">
	...
	<td width="28%">
		<xsl:for-each select="key('AufOrgKomb', $aufgabe)" >
			<xsl:for-each select="key('ABK', field[@name='end'])">
				<xsl:for-each select="key('ANB', child::field[@name='end'])">
					<xsl:variable name="awb" select="key('hash', child::field[@name='end'])" />
					<xsl:if test="$awb/@class = 'Anwendungsbaustein'">
						<xsl:value-of select="str:removeLineBreak($awb/child::field[@name='name'])" />
							<br/>
					</xsl:if>
				</xsl:for-each>
