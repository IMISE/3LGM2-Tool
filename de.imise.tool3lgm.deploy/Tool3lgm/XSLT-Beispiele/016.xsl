...
<!--type: html -->
<!--description: Welche Aufgabe interpretiert, bearbeitet, oder bearbeitet und interpertiert welchen Objekttypen? -->
...
<xsl:variable name="both_text" select="'interpertiert und bearbeitet Objekttyp'" />
...
<!-- Inhalt der Seit erzeugen -->
	<body>
		...
		<table border="1" cellpadding="3" cellspacing="0">
			<thead><tr><th><xsl:value-of select="$task_text" /></th><th><xsl:value-of select="$read_text" /></th><th><xsl:value-of select="$write_text" /></th><th><xsl:value-of select="$both_text" /></th></tr></thead>
...
<xsl:template match="modell_3lgm_2/objects/element[@class='Aufgabe']">
	...
	<xsl:variable name="anzahlInterBearb" select="count(key('AufgabeInterBearb', @hash))" />

	<xsl:choose>
		<xsl:when test="($anzahlInterpretiert &gt; $anzahlBearbeitet)">
			<xsl:choose>
				<xsl:when test="($anzahlInterpretiert &gt; $anzahlInterBearb)">
					<xsl:call-template name="rekursivAnfang">
						...
						<xsl:with-param name="anzahlInterBearb" select="$anzahlInterBearb" />
				...
				<xsl:otherwise>
					<xsl:call-template name="rekursivAnfang">
						...
						<xsl:with-param name="anzahlBearbeitet" select="$anzahlBearbeitet" />
						<xsl:with-param name="anzahlInterBearb" select="$anzahlInterBearb" />
						<xsl:with-param name="anzahl" select="$anzahlInterBearb" />
					</xsl:call-template>				
				</xsl:otherwise>
			</xsl:choose>
		</xsl:when>
		<xsl:otherwise>
			<xsl:choose>
				<xsl:when test="($anzahlBearbeitet &gt; $anzahlInterBearb)">
					<xsl:call-template name="rekursivAnfang">
						<xsl:with-param name="hash" select="@hash" />
						<xsl:with-param name="anzahlInterpretiert" select="$anzahlInterpretiert" />
						<xsl:with-param name="anzahlBearbeitet" select="$anzahlBearbeitet" />
...
<xsl:template name="rekursivAnfang">
	...
	<xsl:param name="anzahlInterBearb" />
	...
	<tr>
		...
		<xsl:call-template name="rekursiv">
			...
			<xsl:with-param name="interBearb" select="key('AufgabeInterBearb', $hash)" />
			<xsl:with-param name="anzahlInterpretiert" select="$anzahlInterpretiert" />
			<xsl:with-param name="anzahlBearbeitet" select="$anzahlBearbeitet" />
			<xsl:with-param name="anzahlInterBearb" select="$anzahlInterBearb" />
...
<xsl:template name="rekursiv">
	...
	<xsl:choose>
		...
		<xsl:otherwise>
			...
			<xsl:choose>
				<xsl:when test="$index &gt; $anzahlInterBearb"> <!-- interpertiert+ bearbeitet ist fertig -->
					<td>&#160;</td>
				</xsl:when>
				<xsl:otherwise>
					<td><xsl:value-of select="str:removeLineBreak(key('hash', $interBearb[$index]/child::field[@name='end'])/child::field[@name='name'])" /></td>
				</xsl:otherwise>
			</xsl:choose>
			...
			<xsl:call-template name="rekursiv">
				...
				<xsl:with-param name="interBearb" select="$interBearb" />
				<xsl:with-param name="anzahlInterpretiert" select="$anzahlInterpretiert" />
				<xsl:with-param name="anzahlBearbeitet" select="$anzahlBearbeitet" />
				<xsl:with-param name="anzahlInterBearb" select="$anzahlInterBearb" />
