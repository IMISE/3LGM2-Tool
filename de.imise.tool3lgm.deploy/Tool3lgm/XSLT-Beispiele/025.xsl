...
<!--name: Aufgabe erledigt durch Anwendungsbaustein -->
<!--type: html -->
<!--description: Welche Aufgaben werden in welchen Organisationseinheiten durch welche Anwendungsbausteine erledigt (Variente 2)? (Teilaufgaben mit in uebergeordnete Aufgaben einbeziehen; Zellen nicht verbinden) -->
<!--author: Thomas Rudert 28.06.2004 -->

<xsl:stylesheet
	xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	xmlns:xs="http://www.w3.org/2001/XMLSchema"
	xmlns:str="http://whatever"
	xmlns:lgm="http://whatever"
	xmlns:int="http://whatever"
	version="2.0"
	exclude-result-prefixes="str lgm int">

<xsl:variable name="header_text" select="'Welche Aufgaben werden in welchen Organisationseinheiten durch welche Anwendungsbausteine erledigt?'" />
<xsl:variable name="note_text" select="'Bemerkung:'" />
<xsl:variable name="note1_text" select="'das Dokument ist nach der hierarchischen Struktur der Aufgaben aufgebaut, Teilaufgaben k&#246;nnen daher mehrfach auftreten'" />
<xsl:variable name="note2_text" select="'die Organisationseinheiten und Anwendungsbausteine der Teilaufgaben werden mit in die &#252;bergeordnete Aufgaben einbezogen'" />
<xsl:variable name="note3_text" select="'die Organisationseinheiten zu einer Aufgabe sind nach ihrer Aufgaben-Organisationseinheiten-Konfiguration gruppiert'" />
<xsl:variable name="note4_text" select="'die Anwendungsbausteine zu einer Organisationseinheit sind nach ihrer Anwendungsbaustein-Konfiguration gruppiert'" />
<xsl:variable name="note5_text" select="'wenn ein Anwendungsbaustein rechnerbasiert oder gemischt ist, dann steht hinter diesem Anwendungsbaustein in Klammern das dazugeh&#246;rige Softwareprodukt'" />
...
<xsl:function name="str:countOccurrences" as="xs:integer">
	...
	<xsl:choose>
		<xsl:when test="contains($inputString, $subString)" >
			<xsl:sequence select="1 + str:countOccurrences(substring-after($inputString, $subString), $subString)" />
		</xsl:when>
		<xsl:otherwise>
...
<!-- jeder AufgabeHash kommt maximal einmal vor und jeder AufOrgKombHash kommt bei jeder AufgabeHash maximal einmal vor -->
<xsl:function name="lgm:updateResult" as="xs:string">
	...
	<xsl:choose>
		...
		<xsl:otherwise>
			<xsl:variable name="aufgabe" select="substring-before(substring-after($position, '$]$'), '$)$')" />
			<xsl:sequence select="lgm:updateResult(lgm:addAufOrgKomb($result, concat('$($', $aufgabe), concat('$+$', $aufOrgKomb)), substring-after($position, '$)$'), $aufOrgKomb)" />
		</xsl:otherwise>
	</xsl:choose>
</xsl:function>
...
<xsl:function name="lgm:hierarchyNext" as="xs:string">
	...
	<xsl:choose>
		...
		<xsl:otherwise>
			<xsl:sequence select="concat(concat(string(number($temp)+1), '.'), substring-after($current, '.'))" />
		</xsl:otherwise>
	</xsl:choose>
</xsl:function>
...
<xsl:function name="lgm:hierarchySet" as="xs:string">
	...
	<xsl:choose>
		...
		<xsl:otherwise>
			<xsl:sequence select="concat(concat(string($new), '.'), substring-after($current, '.'))" />
		</xsl:otherwise>
	</xsl:choose>
</xsl:function>
...
<xsl:function name="lgm:hierarchyOutput" as="xs:string">
	...
	<xsl:choose>
		...
		<xsl:otherwise>
			<xsl:sequence select="concat(concat(lgm:hierarchyOutput(substring-after($current, '.')), '.'), $temp)" />
		</xsl:otherwise>
	</xsl:choose>
</xsl:function>

<!-- Funktion fuegt AufOrgKomb zur Aufgabe hinzu, falls sie da noch nicht aufgefuehrt wird -->
...
<!-- AufOrgHash mit vorangestellten Trennzeichen $+$ -->
	...
	<xsl:choose>
		<xsl:when test="contains($result, $aufgabe)">
			<xsl:variable name="subString" select="concat($aufgabe, substring-before(substring-after($result, $aufgabe), '$)$'))" />
			<xsl:choose>
				<xsl:when test="contains($subString, $aufOrgKomb)">
					<xsl:sequence select="$result" />
				</xsl:when>
				<xsl:otherwise>
					<xsl:sequence select="str:replaceSubstring($result, $subString, concat($subString, $aufOrgKomb))" />
				</xsl:otherwise>
			</xsl:choose>
		</xsl:when>
		<xsl:otherwise>
			<xsl:sequence select="concat(concat(concat($aufgabe, $aufOrgKomb), '$)$'), $result)" />
		</xsl:otherwise>
	</xsl:choose>
</xsl:function>
...
<xsl:key name="OberaufgabenVon" match="/modell_3lgm_2/objects/element[@class='AufAufVerbindung']" use="child::field[@name='start']" />
<xsl:key name="TeilaufgabenVon" match="/modell_3lgm_2/objects/element[@class='AufAufVerbindung']" use="child::field[@name='end']" />

<!-- alle Kanten von Aufgabe zu einer AufgabeOrgansisationseinheitKombination (AufOrgKomb - KOH) -->
<xsl:key name="AufOrgKomb" match="/modell_3lgm_2/objects/element[@class='AufAufOrgVerbindung']" use="child::field[@name='start']" />

<!-- alle Kanten von AufOrgKomb zu einer Anwendungsbausteinkonfiguration (AufOrgKomb - ABK) -->
<xsl:key name="ABK" match="/modell_3lgm_2/objects/element[@class='AwbkAufOrgVerbindung']" use="child::field[@name='start']" />

<!-- alle Kanten von AufOrgKomb zu einer Organisationseinheit (AufOrgKomb - OEH) -->
<xsl:key name="OEH" match="/modell_3lgm_2/objects/element[@class='OrgAufOrgVerbindung']" use="child::field[@name='start']" />

<!-- alle Kanten von AnwBauKonf zu Anwendungsbaustein(rechnerunt., konvent., gemischt) (ABK - AWB)  finden -->
<xsl:key name="AWB" match="/modell_3lgm_2/objects/element[@class='AwbAwbkVerbindung']" use="child::field[@name='start']" />

<!-- alle Kanten von RechAwb bzw. Awb zu einem Anwendungsprogramm (AWB - AWP) -->
<xsl:key name="AWP" match="/modell_3lgm_2/objects/element[@class='AwbAwpVerbindung' or @class='RawbAwpVerbindung']" use="child::field[@name='start']" />

<!-- alle Kanten von Anwendungsprogramm zu einem Softwareprodukt (AWP - SWP) -->
<xsl:key name="SWP" match="/modell_3lgm_2/objects/element[@class='AwpSwpVerbindung']" use="child::field[@name='start']" />
...
<html>
	...
	<body>
		...
		<h4><xsl:value-of select="$note_text" /></h4>
		<ul>
			<li><xsl:value-of select="$note1_text" /></li>
			<li><xsl:value-of select="$note2_text" /></li>
			<li><xsl:value-of select="$note3_text" /></li>
			<li><xsl:value-of select="$note4_text" /></li>
			<li><xsl:value-of select="$note5_text" /></li>
		</ul>
		<xsl:choose>
			...
			<xsl:otherwise>
				<table border="1">
					<tr><th></th><th><xsl:value-of select="$task_text" /></th><th><xsl:value-of select="$org_text" /></th><th><xsl:value-of select="$appl_text" /></th></tr>
...
<xsl:template name="aufgaben">
	...
	<xsl:param name="maxIndex" select="count(/modell_3lgm_2/objects/element[@class='Aufgabe'])" />
	<xsl:variable name="aufgabe" select="/modell_3lgm_2/objects/element[@class='Aufgabe'][$index]/@hash" />
	<xsl:choose>
		<xsl:when test="count(key('OberaufgabenVon', $aufgabe)) = 0">
			...
			<xsl:call-template name="resolveHierarchy">
				...
				<xsl:with-param name="result" select="$result" />
				<xsl:with-param name="hierarchy" select="lgm:hierarchyDown(string($hierarchy))" />				
			</xsl:call-template>
			<xsl:if test="$index &lt; $maxIndex">
				<xsl:call-template name="aufgaben">
					<xsl:with-param name="result" select="$result" />
					<xsl:with-param name="hierarchy" select="lgm:hierarchyNext(string($hierarchy))" />
...
<xsl:template name="ausgabe">
	...
	<xsl:param name="result" />
	<xsl:param name="hierarchy" select="1" />
	<xsl:variable name="aufOrgKomb" select="concat(substring-before(substring-after($result, concat($aufgabe, '$+$')), '$)$'), '$+$')" />
	...
	<xsl:choose>
		<xsl:when test="string-length(substring-before($aufOrgKomb, '$+$')) = 0">
			<tr>
				<td>
					...
					<xsl:value-of select="str:removeLineBreak(key('hash', $aufgabe)/child::field[@name='name'])" />
				</td>			
				<td>&#160;</td><td>&#160;</td>
			</tr>
		</xsl:when>
		<xsl:otherwise>
			<xsl:call-template name="ausgabeOrg">
				...
				<xsl:with-param name="col1" select="lgm:hierarchyOutput(string($hierarchy))" />
				<xsl:with-param name="col2" select="str:removeLineBreak(key('hash', $aufgabe)/child::field[@name='name'])" />
			</xsl:call-template>
		</xsl:otherwise>
	</xsl:choose>
</xsl:template>
...
<xsl:template name="ausgabeOrg">
	...
	<xsl:if test="string-length($hash) &gt; 0">
		<xsl:choose>
			<xsl:when test="count(key('ABK', $hash)) = 0">
				<tr>
					<td>
					...
					<td>
						<xsl:for-each select="key('OEH', $hash)">
							...
							<xsl:value-of select="str:removeLineBreak(key('hash', child::field[@name='end'])/child::field[@name='name'])" />
			...
			<xsl:otherwise>
				<xsl:call-template name="ausgabeABK">
					...
					<xsl:with-param name="col3" select="$hash" />
				</xsl:call-template>
			</xsl:otherwise>
		</xsl:choose>
...
<xsl:template name="ausgabeABK">
	...
	<xsl:param name="col2" />
	<xsl:param name="col3" />
	<xsl:for-each select="key('ABK', $col3)">
		<tr>
			<td>
				<xsl:value-of select="$col1" />
			</td>
			...
			<td>
				<xsl:for-each select="key('OEH', $col3)">
					<xsl:sort select="str:removeLineBreak(key('hash', child::field[@name='end'])/child::field[@name='name'])" order="ascending" data-type="text" />
					<xsl:value-of select="str:removeLineBreak(key('hash', child::field[@name='end'])/child::field[@name='name'])" />
					<xsl:if test="position() != last()">
						<xsl:text>, </xsl:text>
...
<xsl:template name="ausgabeAWB">
	...
	<xsl:for-each select="key('AWB', $ABK)">
		<xsl:sort select="str:removeLineBreak(key('hash', child::field[@name='end'])/child::field[@name='name'])" order="ascending" data-type="text" />
		<xsl:variable name="awb" select="key('hash', child::field[@name='end'])" />
...
<xsl:template name="resolveHierarchy">
	...
	<xsl:for-each select="key('TeilaufgabenVon', $masterTask)">
		...
		<xsl:variable name="current" select="lgm:hierarchySet($hierarchy, position())" />
		<xsl:call-template name="ausgabe">
		...
		<xsl:if test="not(contains($usedTasks, $hash))">
			<xsl:call-template name="resolveHierarchy">
				...
				<xsl:with-param name="usedTasks" select="concat(concat($hash, '$,$'), $usedTasks)" />
...
<xsl:template name="rekursiveStart">
	...
	<xsl:param name="index" select="1" />
	<xsl:variable name="aufgaben" select="/modell_3lgm_2/objects/element[@class='Aufgabe']" />
	<xsl:choose>
			...
			<xsl:choose>
				<xsl:when test="count(key('OberaufgabenVon', $hash)) = 0">
					<xsl:call-template name="rekursive">
						...
						<xsl:with-param name="position" select="concat(concat('$[$1$,$1$]$', $hash), '$)$')" />
					</xsl:call-template>
				</xsl:when>
				<xsl:otherwise>
					<xsl:call-template name="rekursiveStart">
...
<!-- ...$)$$[$indexAufOrgKomb$,$indexTeilAufgabe$]$AufgabeHash$)$ -->
	<xsl:variable name="hash" select="substring-before(substring-after($position, '$]$'), '$)$')" />
	<xsl:variable name="indexTeilaufgabe" select="number(substring-before(substring-after($position, '$,$'), '$]$'))" />
	<xsl:variable name="indexAufOrgKomb" select="number(substring-before(substring-after($position, '$[$'), '$,$'))" />
	<xsl:variable name="aufOrgKomb" select="key('AufOrgKomb', $hash)/child::field[@name='end']" />
	<xsl:choose>
		<xsl:when test="$indexAufOrgKomb &gt; count($aufOrgKomb)">
			...
			<xsl:choose>
				<xsl:when test="$indexTeilaufgabe &gt; count($partOf)">
					<xsl:variable name="temp" select="substring-after($position, '$)$')" />
					<xsl:choose>
						<xsl:when test="string-length($temp) = 0">
							<xsl:call-template name="rekursiveStart">
								<xsl:with-param name="result" select="$result" />
				...
				<xsl:otherwise>					
					<xsl:variable name="subTask" select="key('TeilaufgabenVon' ,$hash)[$indexTeilaufgabe]/child::field[@name='start']" />
					<xsl:choose>
						<xsl:when test="contains($position, $subTask)">
							...
							<xsl:variable name="temp" select="concat(concat(concat(substring-before($position, '$,$'), '$,$'), string($indexTeilaufgabe+1)), concat('$]$', substring-after($position, '$]$')))" />
							<xsl:call-template name="rekursive">
								<xsl:with-param name="result" select="$result" />
								<xsl:with-param name="position" select="$temp" />
								<xsl:with-param name="topIndex" select="$topIndex" />
						...
						<xsl:otherwise>
							...
							<xsl:call-template name="rekursive">
								<xsl:with-param name="result" select="$result" />
								<xsl:with-param name="position" select="concat(concat(concat('$[$1$,$1$]$', $subTask), '$)$'), $temp)" />
								<xsl:with-param name="topIndex" select="$topIndex" />
							</xsl:call-template>
						</xsl:otherwise>
					</xsl:choose>
		...
		<xsl:otherwise>
			<xsl:variable name="positionNew" select="concat(concat(concat(concat(substring-before($position, '$[$'), '$[$'), string($indexAufOrgKomb+1)), '$,$'), substring-after($position, '$,$'))" />
			<xsl:call-template name="rekursive">				
				<xsl:with-param name="position" select="$positionNew" />
				<xsl:with-param name="result" select="lgm:updateResult($result, $position, $aufOrgKomb[$indexAufOrgKomb])" />
				<xsl:with-param name="topIndex" select="$topIndex" />
			</xsl:call-template>
		</xsl:otherwise>
	</xsl:choose>
