...
<!--name: alle Aufgaben -->
<!--type: html -->
<!--description: Aufgaben mit Namen, Beschreibung, Anforderungen und Bemerkungen -->
<!--author: Thomas Rudert 04.06.2003 -->
...
<xsl:variable name="zielobjekt" select="'Aufgabe'" />
<xsl:variable name="header_text" select="'Aufgaben'" />
<xsl:variable name="name_text" select="'Bezeichnung'" />
<xsl:variable name="desc_text" select="'Beschreibung'" />
<xsl:variable name="requ_text" select="'Anforderung'" />
<xsl:variable name="note_text" select="'zu beachten'" />
...
<html>
	...
	<body>
		...
		<table cellspacing="0" cellpadding="3" border="1">
			<tr><th><xsl:value-of select="$name_text" /></th><th><xsl:value-of select="$desc_text" /></th><th><xsl:value-of select="$requ_text" /></th><th><xsl:value-of select="$note_text" /></th></tr>
			<tbody align="left" valign="top">
				<xsl:for-each select="key('element', $zielobjekt)">
					<xsl:sort data-type="text" select="child::field[@name='name']" />
					<tr><td><xsl:value-of select="str:replaceSubstring(child::field[@name='name'], '\-', '')" />&#160;</td>
					<td><xsl:value-of select="str:replaceSubstring(child::field[@name='description'], '&#x000a;', $zeilenumbruch)" disable-output-escaping="yes" />&#160;</td>
					<td><xsl:value-of select="str:replaceSubstring(child::field[@name='requirement'], '&#x000a;', $zeilenumbruch)" disable-output-escaping="yes" />&#160;</td>
					<td><xsl:value-of select="str:replaceSubstring(child::field[@name='note'], '&#x000a;', $zeilenumbruch)" disable-output-escaping="yes" />&#160;</td></tr>
				</xsl:for-each>
			</tbody>
		</table>
	</body>
