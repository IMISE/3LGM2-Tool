...
<xsl:template name="stringTrennen">
	...
	<xsl:choose>
		<xsl:when test="$eintraege &gt; 1">
			<td><xsl:value-of select="str:removeLineBreak(key('hash', substring-before($zeichenkette, '§$§'))/child::field[@name='name'])" /></td>
		...
		<xsl:otherwise>
			<td><xsl:value-of select="str:removeLineBreak(key('hash', $zeichenkette)/child::field[@name='name'])" /></td>
		</xsl:otherwise>
	</xsl:choose>			
</xsl:template>
