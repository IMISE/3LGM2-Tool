...
<!--name: Communication Standards: Overview -->
<!--type: html -->
<!--description: Overview about the usage of communication standards -->
...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:key name="BSSKOM"
		match="/modell_3lgm_2/objects/element[@class='BssKommstVerbindung']"
	...
	<xsl:key name="KOMBEZETNT"
		match="/modell_3lgm_2/objects/element[@class='KommbezEtntVerbindung']"
		use="child::field[@name='start']" />

	<xsl:key name="ETNTKOM"
		match="/modell_3lgm_2/objects/element[@class='EtntKommstVerbindung']"
		use="child::field[@name='start']" />

	<xsl:key name="BSSETNT"
		match="/modell_3lgm2_2/objects/element[@class='BssEtntVerbindung']"
		use="child::field[@name='start']" />
	...
	<xsl:template match="/">
		<html>
			<head>
				<title>Overview about the usage of communication standards</title>
			...
			<body>
				...
				<h1>Overview about the usage of communication standards</h1>
				<h2>
					<xsl:value-of select="modell_3lgm_2/header/title" />
				</h2>
				<table border="1" cellpadding="4" cellspacing="0">
					<tr>
						<th>Communication Standard</th><!--<th>Component Interfaces</th> -->
						<th>Existing connections</th>
						<th>Possible connections</th>
					</tr>
					<tbody valign="top" align="left">
						<tr>
							<xsl:apply-templates
								select="modell_3lgm_2/objects/element[@class='Kommunikationsstandard']">
	...
	<xsl:template match="element[@class='Kommunikationsstandard']">
		<!-- Anzahl der Kanten von der Aufgabe zu einer AufgabeOrgansisationseinheitKombination 
			(AufOrgKomb - KOH) -->
		...
		<xsl:choose>
			<xsl:when test="count($BSS) &gt; 0">
				<td>
					<xsl:value-of
						select="str:removeLineBreak(key('hash',$BSS/child::field[@name='end'])/child::field[@name='name'])" />
				</td>
				<!--<td><xsl:for-each select="key('hash',$BSS/child::field[@name='start'])"> 
					<xsl:value-of select="str:removeLineBreak(child::field[@name='name'])" /> 
					<xsl:if test="position() != last()"> <br/> </xsl:if> </xsl:for-each> </td> -->
				<td>
			...
			<xsl:otherwise>
				<td>
					<xsl:value-of select="str:removeLineBreak(child::field[@name='name'])" /><!--</td><td> -->
					<br />
				</td>
				<td>
					<br />
	...
	<xsl:template name="KommBeziehung">
		<xsl:param name="KOM" />
		<xsl:variable name="KOMBEZ"
			select="/modell_3lgm_2/objects/element[@class='KommBeziehung']" />
		<xsl:for-each select="$KOMBEZ">
			<!-- Test ob Kommunikationsbeziehung eine ETNT-beziehung mit dem Kommunikationsstandard 
				verwendet beide Schnittstellen den Kommunikationsstandard verwenden -->
			<xsl:if
				test="key('ETNTKOM',key('KOMBEZETNT',@hash)/child::field[@name='end'])/child::field[@name='end'] = $KOM
		 and key('KOMBSS',child::field[@name='start'])/child::field[@name='end'] = $KOM and key('KOMBSS',child::field[@name='end'])/child::field[@name='end'] = $KOM">
				<!--and key('KOMBEZETNT',@hash)/child::field[@name='end'] = key('BSSETNT',child::field[@name='start'])/child::field[@name='end'] 
					and key('KOMBEZETNT',@hash)/child::field[@name='start'] = key('BSSETNT',child::field[@name='start'])/child::field[@name='end'] 
					"> -->
				<!--<xsl:value-of select="key('hash',child::field[@name='start'])/child::field[@name='name']"/> -->
				<xsl:value-of
					select="key('hash',key('AWBKOM',child::field[@name='start'])/child::field[@name='start'])/child::field[@name='name']" />
				<xsl:text> (</xsl:text>
				<xsl:value-of
					select="key('hash',key('SW',key('AWP',key('AWBKOM',child::field[@name='start'])/child::field[@name='start'])/child::field[@name='end'])/child::field[@name='end'])/child::field[@name='name']" />
				<xsl:text>) </xsl:text>
				...
				<xsl:value-of
					select="key('hash',key('AWBKOM',child::field[@name='end'])/child::field[@name='start'])/child::field[@name='name']" />
				<xsl:text> (</xsl:text>
				<xsl:value-of
					select="key('hash',key('SW',key('AWP',key('AWBKOM',child::field[@name='end'])/child::field[@name='start'])/child::field[@name='end'])/child::field[@name='end'])/child::field[@name='name']" />
				<xsl:text>) </xsl:text>
				<br />
				<br />
			</xsl:if>
	...
	<xsl:template name="KommBeziehung2">
		...
		<xsl:for-each select="$KOMBEZ">
			<xsl:if
				test="key('KOMBSS',child::field[@name='start'])/child::field[@name='end'] = $KOM and key('KOMBSS',child::field[@name='end'])/child::field[@name='end'] = $KOM">
				<!--<xsl:value-of select="key('hash',child::field[@name='start'])/child::field[@name='name']"/> -->
				<xsl:value-of
					select="key('hash',key('AWBKOM',child::field[@name='start'])/child::field[@name='start'])/child::field[@name='name']" />
				<xsl:text> (</xsl:text>
