...
<!--type: html -->
<!--description: Functions with names ans descriptions -->
...
<xsl:variable name="header_text" select="'Functions'" />
...
<html>
	...
	<body>
		...
		<table cellspacing="0" cellpadding="3" border="1">
			<tr>
				<th><xsl:value-of select="$name_text" /></th>
				<th><xsl:value-of select="$desc_text" /></th>
			</tr>
			<tbody align="left" valign="top">
			<xsl:for-each select="key('element', $zielobjekt)">
				<xsl:sort data-type="text" select="child::field[@name='name']" />
				<tr>
					<td><xsl:value-of select="str:replaceSubstring(child::field[@name='name'], '\-', '')" />&#160;</td>
