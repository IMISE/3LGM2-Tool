...
<!--type: html -->
<!--description: What entity types can be communicated between which application components? -->
...
<xsl:variable name="header_text" select="'What entity types can be communicated between which application components?'" />
<xsl:variable name="note_text" select="'Note:'" />
<xsl:variable name="notes1_text" select="'With the help of this document you can analyze which application components communicate which entity types between each other. An entity type can be represented by document types and message types. A message type can be combined with an event type in an ETMT-combination. Application components can have one or more component interfaces, which can be connected with another interfaces. In a model you can define for every component interface which ETMT-combinations it is able to receive and send.'" />
<xsl:variable name="notes2_text" select="'The generated document has the following structure:'" />
...
<xsl:variable name="notes4_text" select="'As the communication link between two component interfaces is connected to an ETMT combination within the meta model, but not in the 3LGM&#178;-Tool, it can happen that pairs of communicating application components are listed between which communication does not take place or is not reasonable.'" />
...
<html>
	...
	<body>
		...
		<p><xsl:value-of select="$notes1_text" /><br/><xsl:value-of select="$notes2_text" /><br/><xsl:value-of select="$notes3_text" /><br/><xsl:value-of select="$notes4_text" /></p>
		<table with="100%" cellspacing="0" cellpadding="3" border="1">
			<thead><tr><td width="50"></td><th><xsl:value-of select="$send_text" /></th><th><xsl:value-of select="$rece_text" /></th><th><xsl:value-of select="$event_text" /></th></tr>
			</thead>
...
<xsl:template match="modell_3lgm_2/objects/element[@class='Objekttyp']">
	<tr><td><xsl:attribute name="colspan">4</xsl:attribute><h3><xsl:attribute name="style">margin-top:12pt; margin-bottom:0pt;</xsl:attribute><xsl:value-of select="str:removeLineBreak(child::field[@name='name'])" /></h3></td></tr>
...
<xsl:template name="ausgabe">
	...
	<tr><td></td>
		<td>
			...
			<xsl:text> (</xsl:text>
				<xsl:value-of select="key('hash',key('SW',key('AWP',key('kanteAnwBau' ,$sender)/child::field[@name='start'])/child::field[@name='end'])/child::field[@name='end'])/child::field[@name='name']" />
			<xsl:text>) </xsl:text><br/>
		
		</td>
		<td>
		<xsl:variable name="empfaengerStart" select="key('hash', key('kanteAnwBau' ,$empfaenger)/child::field[@name='start'])/child::field[@name='name']" />
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				...
				<xsl:text> (</xsl:text>
					<xsl:value-of select="key('hash',key('SW',key('AWP',key('kanteAnwBau' ,$empfaenger)/child::field[@name='start'])/child::field[@name='end'])/child::field[@name='end'])/child::field[@name='name']" />
				<xsl:text>) </xsl:text><br/>
