...
<!--name: Software supporting functions -->
<!--type: html -->
<!--description: Which software supports which functions? -->
...
<!-- alle Kanten von Software zu Anwendungsprogramm finden -->
<xsl:key name="AWP" match="/modell_3lgm_2/objects/element[@class='AwpSwpVerbindung']" use="child::field[@name='end']" /> 

<!-- alle Kanten von Anwendungsprogramm zu Anwendungsbaustein finden -->
<xsl:key name="AB" match="/modell_3lgm_2/objects/element[@class='RawbAwpVerbindung']" use="child::field[@name='end']" /> 
...
<html>
	<head>
		<title>Which software supports which functions?</title>
		<meta name="author" content="Tool3lgm" />
	</head>
	<body>
		<xsl:comment>This HTML file was generated automatically from a 3LGM&#178; model.</xsl:comment>
		<h1>Which software is used for the support of which functions?</h1>
		<h2><xsl:value-of select="modell_3lgm_2/header/title" /></h2>
		<table border="1" cellpadding="2" cellspacing="0">
			<tr><th>Software</th><th>Function</th></tr>
			<tbody valign="top" align="left"><tr>
				<xsl:apply-templates select="modell_3lgm_2/objects/element[@class='Softwareprodukt']">
					
					<!-- nach den Namen der Software sortieren -->
...
<xsl:template match="element[@class='Softwareprodukt']">
	<xsl:variable name="AWBKs" select="key('AWBK', key('AB',key('AWP',@hash)/child::field[@name='start'])/child::field[@name='start'])" />
	<xsl:choose>
		<xsl:when test="count(key('hash',key('AUF',key('KOH',$AWBKs/child::field[@name='start'])/child::field[@name='start'])/child::field[@name='start'])) &gt; 0">
			...
			<td><xsl:for-each select="key('hash',key('AUF',key('KOH',$AWBKs/child::field[@name='start'])/child::field[@name='start'])/child::field[@name='start'])">
				...
				<!--<xsl:for-each select="key('hash',key('AUF',key('KOH',child::field[@name='start'])/child::field[@name='start'])/child::field[@name='start'])">-->
					<xsl:value-of select="child::field[@name='name']"/><br/>
				<!--</xsl:for-each>-->
			</xsl:for-each><!--<br/>--></td>
