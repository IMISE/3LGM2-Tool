...
<!--name: Entity Types: Storage -->
<!--type: html -->
<!--description: In what application components are the entity types stored? -->
...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:variable name="header_text"
		select="'In what application components are the entity types stored?'" />
	<xsl:variable name="obje_text" select="'Entity Type'" />
	<xsl:variable name="mast_text" select="'Master application component'" />
	<xsl:variable name="appl_text" select="'Application components'" />
	...
	<xsl:key name="ObjLogspVerbindung"
		match="/modell_3lgm_2/objects/element[@class='ObjLogspVerbindung']"
		use="child::field[@name='start']" />
	<xsl:key name="DbsBauVerbindung"
		match="/modell_3lgm_2/objects/element[@class='AwbDoksVerbindung' or @class='AwbDbsVerbindung' or @class='KawbDoksVerbindung' or @class='RawbDbsVerbindung']"
		use="child::field[@name='end']" />

	<xsl:key name="ObjReprVerbindung"
		match="/modell_3lgm_2/objects/element[@class='ObjReprVerbindung']"
		use="child::field[@name='start']" />
	<xsl:key name="ReprDbsVerbindung"
		match="/modell_3lgm_2/objects/element[@class='DoksDokVerbindung' or @class='DbsDatVerbindung']"
		use="child::field[@name='end']" />

	<xsl:variable name="baustein"
		select="/modell_3lgm_2/objects/element[@class='Anwendungsbaustein' or @class='RechAnwendungsbaustein' or @class='KonAnwendungsbaustein']" />
		...
		<html>
			...
			<body>
				...
				<table cellspacing="0" cellpadding="3" border="1">
					<tr>
						<th>
							<xsl:value-of select="$obje_text" />
						</th>
						<th>
							<xsl:value-of select="$mast_text" />
						</th>
						<th>
	...
	<xsl:template name="rekursiv1">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:variable name="repForm"
					select="$kanteObjRep[$indexKanteObjRep]/child::field[@name='end']" />
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:call-template name="rekursiv3">
					<xsl:with-param name="objekttyp" select="$objekttyp" />
					<xsl:with-param name="repForm" select="$repForm" />
					<xsl:with-param name="dbs"
						select="$kanteRepDbs[$indexKanteRepDbs]/child::field[@name='start']" />
					<xsl:with-param name="kanteObjRep" select="$kanteObjRep" />
					<xsl:with-param name="maxKanteObjRep" select="$maxKanteObjRep" />
					<xsl:with-param name="indexKanteObjRep" select="$indexKanteObjRep" />
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:variable name="hash"
					select="$kanteDbsAwb[$indexKanteDbsAwb]/child::field[@name='start']" />
				<xsl:choose>
				...
				<xsl:when test="$anzahl &gt; 0">
					<td>
						<xsl:attribute name="rowspan"><xsl:value-of
							select="$anzahl" /></xsl:attribute>
						<xsl:value-of
							select="str:removeLineBreak(key('hash', $objekttyp)/child::field[@name='name'])" />
					</td>
					<td>
						<xsl:attribute name="rowspan"><xsl:value-of
							select="$anzahl" /></xsl:attribute>
						...
						<xsl:if
							test="count(key('hash', key('DbsBauVerbindung', key('ObjLogspVerbindung', $objekttyp)/child::field[@name='end'])/child::field[@name='start'])/child::field[@name='name']) &gt; 0">
							<xsl:text> (</xsl:text>
							<xsl:value-of
								select="key('hash',key('SW',key('AWP',key('DbsBauVerbindung', key('ObjLogspVerbindung', $objekttyp)/child::field[@name='end'])/child::field[@name='start'])/child::field[@name='end'])/child::field[@name='end'])/child::field[@name='name']" />
							<xsl:text>) </xsl:text>
						</xsl:if>
						<br />
					</td>
	...
	<xsl:template name="stringTrennen">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<td>
					<xsl:value-of
						select="str:removeLineBreak(key('hash', $zeichenkette)/child::field[@name='name'])" />
					<xsl:text> (</xsl:text>
					<xsl:value-of
						select="key('hash',key('SW',key('AWP',$zeichenkette)/child::field[@name='end'])/child::field[@name='end'])/child::field[@name='name']" />
					<xsl:text>) </xsl:text>
				</td>
			</xsl:otherwise>
		</xsl:choose>
