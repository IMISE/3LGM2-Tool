...
<!--name: Organizational Units using application components -->
<!--type: html -->
<!--description: Which organizational units use which application components? -->
...
<xsl:key name="OEH" match="/modell_3lgm_2/objects/element[@class='OrgAufOrgVerbindung']" use="child::field[@name='end']" />
...
<html>
	<head>
		<title>Which organizational units use which application components?</title>
		<meta name="author" content="Tool3lgm" />
	</head>
	<body>
		<xsl:comment>This HTML file was generated automatically from a 3LGM&#178; model.</xsl:comment>
		<h1>Which organizational units use which application components?</h1>
		<h2><xsl:value-of select="modell_3lgm_2/header/title" /></h2>
		<table border="1" cellpadding="2" cellspacing="0">
			<tr><th>Organizational Unit</th><th>Application Component</th></tr>
...
<xsl:template match="element[@class='Organisationseinheit']">
	<xsl:variable name="AWBs" select="key('hash',key('AWB', key('ABK',key('OEH',@hash)/child::field[@name='start'])/child::field[@name='end'])/child::field[@name='end'])" />
	<tr><td><xsl:value-of select="str:removeLineBreak(child::field[@name='name'])" /></td>
	<td>
