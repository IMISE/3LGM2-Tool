...
<!--type: html -->
<!--description: Which functions are supported in which organizational units 
	by which application components (Variant 1)? (sub-ordinated functions are 
	comprised under the super-ordinated functions) -->
...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:variable name="header_text"
		select="'Which functions are supported in which organizational units by which application components?'" />
	<xsl:variable name="note_text" select="'Note:'" />
	<xsl:variable name="note1_text"
		select="'The documet is arranged according to the hierarchical structure of the functions, hence sub-ordinated functions can occur repeatedly.'" />
	<xsl:variable name="note2_text"
		select="'the organizational units and application components of sub-ordinated functions are integrated into the super-ordinated functions.'" />
	<xsl:variable name="note3_text"
		select="'The organizational units assigned to a function are arranged according to their function-organizational unit combination.'" />
	<xsl:variable name="note4_text"
		select="'The application components assigned to a function are arranged according to their application component configuration'" />
	<xsl:variable name="note5_text"
		select="'If an application component is computer-based or mixed, the corresponding software product is given in brackets behind the application component'" />
	...
	<xsl:variable name="error_text"
		select="'There are no functions in this model!'" />
	...
	<xsl:template match="/">
		<html>
			...
			<body>
				<xsl:comment>
					This HTML file was generated wurde maschinell aus einem
					3LGM&#178;-Modell erzeugt.
				</xsl:comment>
				<h1>
					<xsl:value-of select="$header_text" />
