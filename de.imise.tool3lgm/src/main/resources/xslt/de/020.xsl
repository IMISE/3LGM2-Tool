...
<xsl:variable name="comp_text" select="'rechnerbasierte Anwendungsbausteine'" />
...
<html>
	...
	<body>
		...
		<table border="1" cellpadding="3" cellspacing="0" width="100%">
			<tr>
				<th width="20%"><xsl:value-of select="$task_text" /></th>
				<th width="40%"><xsl:value-of select="$comp_text" /></th>
				<th width="40%"><xsl:value-of select="$paper_text" /></th>
			</tr>
...
<xsl:template name="anwendungsbaustein">
	...
	<td width="40%">
		...
		<xsl:for-each select="key('hash',key('ANB',key('ABK', key('AufOrgKomb', $aufgabe)/child::field[@name='end'])/child::field[@name='end'])/ child::field[@name='end'])" >
			<xsl:sort select="str:removeLineBreak(child::field[@name='name'])" order="ascending" data-type="text" />			
