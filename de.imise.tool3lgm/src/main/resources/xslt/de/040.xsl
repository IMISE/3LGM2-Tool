...
<!--type: html -->
<!--description: Welche Anwendungsbausteine sind auf welchen Physischen Datenverarbeitungsbausteinen 
	installiert? -->
...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:variable name="conf_text"
		select="'Datenverarbeitungsbaustein-Konfiguration'" />
	<xsl:variable name="phys_text"
		select="'Physischer Datenverarbeitungsbaustein'" />
	...
	<xsl:key name="kanteDbkAnb"
		match="/modell_3lgm_2/objects/element[@class='PdvbkAwbVerbindung']"
		use="child::field[@name='start']" />

	<!-- Kante von physischen Datenverarbeitungsbaustein zu Datenverarbeitungsbaustein-Konfiguration -->
	<xsl:key name="kantePhyDbk"
		match="/modell_3lgm_2/objects/element[@class='PdvbPdvbkVerbindung']"
			...
			<body>
				...
				<table border="1" cellpadding="3" cellspacing="0" width="100%">
					...
					<tbody align="left" valign="top">
						<xsl:apply-templates
							select="modell_3lgm_2/objects/element[@class='RechAnwendungsbaustein']">
				...
				<table border="1" cellpadding="3" cellspacing="0" width="100%">
					...
					<tbody align="left" valign="top">
						<xsl:apply-templates
							select="modell_3lgm_2/objects/element[@class='KonAnwendungsbaustein']">
							<xsl:sort select="str:removeLineBreak(child::field[@name='name'])"
								order="ascending" data-type="text" />
						</xsl:apply-templates>
					</tbody>
	...
	<xsl:template
		match="modell_3lgm_2/objects/element[@class='RechAnwendungsbaustein']">
		<tr>
			<td>
				...
				<xsl:choose>
					<xsl:when
						test="string-length(normalize-space(child::field[@name='name'])) &gt; 0">
	...
	<xsl:template
		match="modell_3lgm_2/objects/element[@class='KonAnwendungsbaustein']">
		<tr>
			<td>
				<xsl:call-template name="getRowspanRekursiv">
					<xsl:with-param name="kanten" select="key('kanteDbkAnb', @hash)" />
	...
	<xsl:template name="datenverarbeitungsbaustein_konfiguration">
		...
		<xsl:for-each select="key('kanteDbkAnb', $anwendungsbaustein)">
			<xsl:variable name="anzahlPhy"
				select="count(key('kantePhyDbk', child::field[@name='end']))" />
			<xsl:choose>
				<xsl:when test="$anzahlPhy &lt; 1">
					<td>
						<xsl:choose>
							<xsl:when
								test="key('hash' ,child::field[@name='end'])/field[@name='name'] = '- &lt;Konfiguration&gt; -'">
								Konfiguration
								<xsl:value-of select="position()" />
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of
									select="str:removeLineBreak(key('hash' ,child::field[@name='end'])/field[@name='name'])" />
							</xsl:otherwise>
						</xsl:choose>
					</td>
					<td>&#160;</td>
				...
				<xsl:when test="$anzahlPhy &gt; 1">
					<td>
						<xsl:attribute name="rowspan"><xsl:value-of
							select="$anzahlPhy" /></xsl:attribute>
						<xsl:choose>
							<xsl:when
								test="key('hash' ,child::field[@name='end'])/field[@name='name'] = '- &lt;Konfiguration&gt; -'">
								Konfiguration
	...
	<xsl:template name="physischer_DVBaustein">
		...
		<xsl:for-each select="key('kantePhyDbk', $konfiguration)">
			<td>
				<xsl:value-of
					select="str:removeLineBreak(key('hash', child::field[@name='start'])/field[@name='name'])" />
			</td>
			<xsl:if test="position() != last()">
				<xsl:text disable-output-escaping="yes">&lt;/tr&gt;&lt;tr&gt;</xsl:text>
			</xsl:if>
		...
		<xsl:choose>
			<xsl:when test="$index &gt; $maxIndex">
				<xsl:if test="$counter &gt; 1">
					<xsl:attribute name="rowspan"><xsl:value-of
						select="$counter" /></xsl:attribute>
				</xsl:if>
			</xsl:when>
			<xsl:otherwise>
				<xsl:variable name="number"
					select="count(key('kantePhyDbk', $kanten[$index]/field[@name='end']))" />
				<xsl:choose>
