 ...
 *
 * Klasse, die alle global benötigten Parameter, und deren Zugriffsmethoden, des
 * Programms enthält
 */
public abstract class Tool3lgmConstants {
	...
	/** 
	 * Wenn <code>true</code>, liefern die {@link GDCommands} in ihrer toString()-Methode
	 * den Namen des Kommandos zurück.
	 * Wenn <code>false</code>, dann liefert die toString()-Methode die Nummer des Kommandos
	 * in der values()-Liste der {@link GDCommands} als String.
	 * Man braucht die lesbaren Namen im Grunde nur zum Debuggen. Im Regelbetrieb sollte der 
	 * Parameter auf <code>false</code> stehen, damit die Kommandos, von denen potenziell sehr
	 * viele im Undo-Redo-Stack geloggt werden, nicht so riesig werden.
	 */
...
//	public static final String RESOURCE_DIR_NAME = "tool3lgm/resources/";
	...
	/** 
	 * Datei-Endung für große Icons.<p>
	 * Verwendungsweise: <br>
	 * <code>Icon largeIcon = getIcon(</code><em>iconNamePrefix</em><code> + LARGE_ICON_SUFFIX)</code> <br>
	 * Bsp.: <br>
	 * <code>Icon largeIcon = getIcon("UNDO") + LARGE_ICON_SUFFIX)</code> liefert das {@link Icon}
	 * mit dem Namen "UNDO_LARGE.gif" aus dem Icon-Ordner in den Resourcen.
	 */
	public static final String LARGE_ICON_SUFFIX = "_LARGE.gif";

	/** 
	 * Datei-Endung für kleine Icons.<p>
	 * Verwendungsweise: <br>
	 * <code>Icon smallIcon = getIcon(</code><em>iconNamePrefix</em><code> + SMALL_ICON_SUFFIX)</code>
	 * Bsp.: <br>
	 * <code>Icon smallIcon = getIcon("UNDO") + SMALL_ICON_SUFFIX)</code> liefert das {@link Icon}
	 * mit dem Namen "UNDO_SMALL.gif" aus dem Icon-Ordner in den Resourcen.
	 */
	...
	/**
	 * Name des Verzeichnisses, in dem die lokalisierten XSLT-Scripte in den Ordnern mit dem
	 * Sprachkürzel der akuellen <code>Locale</code> zu finden sind.
	 */
	...
	/**
	 * Mögliche Typen der FileFilter. Die Bezeichnungen der Bildtypen entsprechen exakt denen, die 
	 * ImageCodec.createImageEncoder() als Typ-String versteht. Für alle diese Typen gibt es 
	 ...
	 */
	public static enum FileFilterType {LGM3, LGM3_ZIP, LGM3_UNZIPPED, USERFIELD, LIC, XSL, CSV}
	 ...
	 * @return
	 */
	public static final FileNameExtensionFilter getFileNameExtensionFilter(FileFilterType fileFilterType) {
	 ...
	 * 		Array von Objecten, deren jeweilige toString()-Methode den Filternamen angibt, der
	 * 		an dieser Stelle im Rückgabearray stehen soll.
	 * @return
	 * @see #getFileNameExtensionFilter(FileFilterType)
	 */
	public static final FileNameExtensionFilter[] getFileNameExtensionFilters(FileFilterType... fileFilterType) {
	...
	public static final Map<ActionIdentifier, KeyStroke> KEYSTROKES = new KeyStrokeMap 
	(
			ActionIdentifier.ACTION_NEW_MODEL,	KeyEvent.VK_N,			KeyEvent.CTRL_DOWN_MASK,
			ActionIdentifier.ACTION_OPEN_MODEL,	KeyEvent.VK_O,			KeyEvent.CTRL_DOWN_MASK,
			ActionIdentifier.ACTION_SAVE_MODEL,	KeyEvent.VK_S,			KeyEvent.CTRL_DOWN_MASK,
			ActionIdentifier.remove,			KeyEvent.VK_DELETE,		0,
			ActionIdentifier.redo,				KeyEvent.VK_Y,			KeyEvent.CTRL_DOWN_MASK,
			ActionIdentifier.undo,				KeyEvent.VK_Z,			KeyEvent.CTRL_DOWN_MASK,
			ActionIdentifier.select_all,		KeyEvent.VK_A,			KeyEvent.CTRL_DOWN_MASK,
			ActionIdentifier.copy,				KeyEvent.VK_C,			KeyEvent.CTRL_DOWN_MASK,
			ActionIdentifier.cut,				KeyEvent.VK_X,			KeyEvent.CTRL_DOWN_MASK,
			ActionIdentifier.paste,				KeyEvent.VK_V,			KeyEvent.CTRL_DOWN_MASK,
			ActionIdentifier.search,			KeyEvent.VK_F,			KeyEvent.CTRL_DOWN_MASK,
			ActionIdentifier.repository,		KeyEvent.VK_F7,			0,
			ActionIdentifier.analysis_editor,	KeyEvent.VK_F9,			0,
			ActionIdentifier.reset_result,		KeyEvent.VK_R,			KeyEvent.CTRL_DOWN_MASK
	);

	/** Array aller {@link Action}s, die im gesamten Baukasten durch
	 	ihre dazugehörigen {@link KeyStroke}s ausgelöst werden können */
	...
	/** 
	 * Name der Datei mit Analysen. Unter diesem Namen ex. die Standarddatei in den localisierten Resourcen.
	 * Wenn der Benutzer irgendeine XMLAnalyse mal aufgerufen hat, dann gibt es mit diesem Namen im
	 * APPLICATION_PATH eine Datei (wenn der Benutzer dort Schreibrecht hat) oder in seinem user.home-Pfad
	 * (wenn er im APPLICATION_PATH kein Schreibrecht hat)
	 */
	 ...
	 * @return
	 */
	public static final boolean isPopupTrigger(MouseEvent e) {
		return ((e.getModifiers() & InputEvent.BUTTON1_MASK) != InputEvent.BUTTON1_MASK);
	}

	/**
	 * Gibt das Oberste Verzeichnis zurück, in dem sich Anwendungsdaten befinden, also das
	...
	/**
	 * gibt String für die Dateiangabe des programminternen Zwischenspeichers
	 * zurück
	 * @return String mit Verzeichnis- und Dateiangabe des Zwischenspeichers
	 */
	public static String getClipboardPath() { return clipboardPath; }

	/**
	 * setzt die Dateiangabe des programminternen Zwischenspeichers
	 * @param _path String mit Verzeichnis- und Dateiangabe des Zwischenspeichers
	 */
	public static void setClipboardPath(String _path) { clipboardPath = _path; }

	/**
	 * Gibt ein Array aller {@link Action}s wieder, die im gesamten Baukasten durch
	 * ihre dazugehörigen {@link KeyStroke}s ausgelöst werden können.
	 * 
	 * @see #KEYSTROKES
	 * @return
	 ...
	 * parents.
	 * @param key String with key for resource
	...
	/**
	 * Liefert den auslösenden {@link KeyStroke} für die durch <code>key</code> identifizierte
	 * {@link StaticAction}.
	 * @param identifier
	 * @return
	 */
	public static KeyStroke getKeyStroke(ActionIdentifier key) {
		return KEYSTROKES.get(key);
	}

	/**
	 * Liefert eine kurze Beschreibung für die durch <code>key</code> identifizierte
	 * {@link StaticAction}. (Zur Verwendung als Tooltip)
	 * @param identifier
	 * @return
	 */
	public static String getDescription(String key) throws MissingResourceException{
		return getResString(key);
	}


	/**
	 * Liefert den command-<code>String</code> für das {@link KeyEvent}, das durch die durch <code>key</code> 
	 * identifizierte {@link StaticAction} ausgelöst wird.<br>
	...
	/**
	 * Gets a string for the given key from the error resource bundleor one of its
	 * parents.
	 * 
	 * @param key
	 * @return String with value of resource
	 */
	public static String getErrString(String key) { return errorBundle.getString(key); }
	 ...
	 * @return ImageIcon
	 */
	public static ImageIcon getLocalizedIcon(String name) {
		ImageIcon retVal;
		try {
	 ...
	 * @return
	 */
	private static ImageIcon getImageIcon(String dir) {
	 ...
	 * @return int
	 */
	public static int getDebugGraphicsOption() { System.out.println(debugGraphicsOption);return debugGraphicsOption; } 
	 ...
	 */
	public static void setDebugGraphicsOption(boolean LOG, boolean FLASH, boolean BUFFERED, int flashTime) {
		debugGraphicsOption = 0;
		if (LOG) debugGraphicsOption |= DebugGraphics.LOG_OPTION;
		if (FLASH) {
			debugGraphicsOption |= DebugGraphics.FLASH_OPTION;
			DebugGraphics.setFlashTime(flashTime);
		} 
		if (BUFFERED) debugGraphicsOption |= DebugGraphics.BUFFERED_OPTION;
	}

	/**
	 * Gibt fuer eine ArrayList von {@link NodeContainer}n oder {@link ModelElement}s
	 * einen String des Inhalts zurück. Ist insertNewLines==false, wird eine kommaseparierte Liste
	 * zurückgegeben.
	 * 
	 * @param ArrayList
	 * @param boolean
	 * @param boolean
	 * 
	 * TODO:AXS:in eine eigene Klasse verlegen
	 */
	public static String getElementListString(List<?> list, boolean showSzenarios, boolean insertNewLines){
	 ...
	 */
	public static boolean lockSupportedByFileSystem(File f) {
		return false;
		/*		try {
			File testFile = new File(f.getCanonicalPath() + "__3lgm_lock_test");
	 ...
	 * Diese werden durch Auslesen der Dateien "Tool3lgmResources_LANGUAGECODE.properties" aus
	 * dem resource-Package ermittelt. Es wird davon ausgegangen, dass auf jeden Fall englische
	 * Ressourcen existieren, die in der Datei "Tool3lgmResources.properties" hinterlegt sind.<br>
	 * 
	...
	/**
	 * Liefert eine Liste der relativen Pfade aller XSLT-Scripte im Resourcenverzeichnis der
	 * aktuellen Locale. Werden für die aktuelle Locale keine Scripte gefunden, werden die
	 * englischen Scripte geladen.
	 * 
	 * @return
	 * 		Liste aller Standard-XSLT-Scripte
	 * 
	 */
	...
	private static final class KeyStrokeMap extends HashMap<ActionIdentifier, KeyStroke> {	
		...
		public KeyStrokeMap(Object... id_keyCode_modifiers) {
			for (int i=0; i<id_keyCode_modifiers.length; i+=3) {
				put((ActionIdentifier)id_keyCode_modifiers[i], 
						createKeyStroke((Integer)id_keyCode_modifiers[i+1],(Integer)id_keyCode_modifiers[i+2]));
			}
		}

		private static KeyStroke createKeyStroke(int keyCode, int modifiers) {
