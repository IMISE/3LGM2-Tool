...
public class ToolSplashScreen extends BasicSplashScreen {
    ...
    /**
     * Prefix of the value of the line with the tag and commit in the
     * version.info file
     */
    private static final String GIT_VERSION_INFO_TAG_PREFIX = "Tool3LGM_Version_";

    /** gets the version.info tags generated by a maven plugin */
    private static final GitVersionInfo gitVersionInfo = GitVersionInfoHandler.getGitVersionInfo(Tool3lgmConstants.GIT_VERSION_FILE, GIT_VERSION_INFO_TAG_PREFIX);
    ...
    protected final void update(final Graphics g) {
        ...
        String versionString = getResString("version") + ": " + gitVersionInfo.tag;
        drawString(g, versionString, 100);
        if (versionString.contains("dev")) {
            String commitString = getResString("commit") + ": " + gitVersionInfo.commit;
