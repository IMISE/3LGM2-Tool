 ...
 * <p>
 * Beinhaltet alle Einträge bzw. Untermenüs, die das Kontextmenu überhaupt
 * anzeigen kann. Der Kontext wird aus {@link ElementSelectionContext} ermittelt
 * und das Menu entsprechend gebaut.
 * 
 * @author fstephan
 * 
 */
...
*/
	...
	// ActionLibrary.ContextActions.CREATE_TEXTFIELD,
	        ...
	        new JSeparator(), MenuCollection.LayoutSubMenus.LAYER_LAYOUT_MENU, new JMenuItem(ActionLibrary.LayoutActions.GLOBAL_LAYOUT), new JSeparator()/*, INTERNAL_MENU*/);
	...
	protected FSTContextMenu(String s) {
		super(s);
	}

	public static void showMenu(Component invoker, MouseEvent cause) {
		showMenu(invoker, cause.getX(), cause.getY());
	}

	public static void showMenu(Component invoker, int x, int y) {
	...
	private static Component create(Object entry) {
	...
	private static class ElementMenu extends DynamicPopupMenu {
		...
		private Component subordinatedElementsMenu = new SubordinatedElementsMenu();
		private Component takeOverInSubmodelMenu = new TakeOverInSubmodelMenu();
		private Component connectToSubmodelMenu = new ConnectToSubmodelMenu();
		private Component analyseMenu = new AnalyseMenu();
		private Component configsVisibility = new JCheckBoxMenuItem(ActionLibrary.ContextActions.CONFIGURATIONS_VISIBILITY);
