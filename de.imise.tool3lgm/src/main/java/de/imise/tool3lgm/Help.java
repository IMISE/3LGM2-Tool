...
public class Help {
	 ...
	 */
	public String getHelpID(Object o) {
		...
		if (o instanceof ElementPropertyDialog) {
			...
			if (me instanceof Anwendungsbaustein) return "dialoge_gemischt_ab";
			if (me instanceof Anwendungsprogramm) return "dialoge_anwendungsprogramm";
			if (me instanceof Aufgabe) return "dialoge_aufgabe";
			// SEBP auforgkombination
			if (me instanceof Bausteinschnittstelle) return "dialoge_baustein_s";
			if (me instanceof Bausteintyp) return "dialoge_bausteintyp";
			if (me instanceof Benutzungsschnittstelle) return "dialoge_benutzer_s";
			if (me instanceof Datenbanksystem) return "dialoge_dbs";
			if (me instanceof Datensatztyp) return "dialoge_datensatztyp";
			// SEBP dbkonfiguration
			if (me instanceof DBVerwaltungssystem) return "dialoge_db_system";
			if (me instanceof Dokumentensammlung) return "dialoge_dokumentensammlung";
			if (me instanceof Dokumententyp) return "dialoge_dokumententyp";
			if (me instanceof Ereignistyp) return "dialoge_ereignistyp";
			if (me instanceof EtntEtdtKombination) return "dialoge_etnt_kombi";
			if (me instanceof Kommunikationsprozess) return "dialoge_kom_prozess";
			if (me instanceof Kommunikationsstandard) return "dialoge_komm_standard";
			if (me instanceof KonAnwendungsbaustein) return "dialoge_papier_ab";
			if (me instanceof Nachrichtentyp) return "dialoge_nachrichtentyp";
			if (me instanceof Netzprotokoll) return "dialoge_netzprotokoll";
			if (me instanceof Netztyp) return "dialoge_netztyp";
			if (me instanceof Objekttyp) return "dialoge_objekttyp";
			if (me instanceof Organisationseinheit) return "dialoge_organisationseinheit";
			if (me instanceof Organisationsplan) return "dialoge_organisationsplan";
			if (me instanceof PhysischerDVBaustein) return "dialoge_dv_baustein";
			// SEBP Prozess
			if (me instanceof RechAnwendungsbaustein) return "dialoge_rechner_ab";
			if (me instanceof Softwareprodukt) return "dialoge_softwareprodukt";
			if (me instanceof Standort) return "dialoge_standort";
			if (me instanceof Subnetz) return "dialoge_subnetz";
		}
