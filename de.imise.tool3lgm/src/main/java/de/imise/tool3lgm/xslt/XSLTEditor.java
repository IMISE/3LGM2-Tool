 ...
 * Verfuegung
 */
public class XSLTEditor extends JDialog
	...
	public XSLTEditor(JDialog owner) {
		super(owner);
		setComponents(new ExtendedTextArea());
	}

	public XSLTEditor(JDialog owner, File _file) {
		super(owner);
		file = _file;
		setComponents(new ExtendedTextArea(loadFromFile(file)));
	}

	public XSLTEditor(JDialog owner, InputStream stream) {
		super(owner);
		readOnly = true;
	...
	public XSLTEditor(JDialog owner, String text) {
		super(owner);
	 ...
	 */
	private void setComponents(ExtendedTextArea _textArea) {
		textArea = _textArea;
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getActionCommand().equals("action_save")) {
			...
			if ((result = JOptionPane.showConfirmDialog(this,
				Tool3lgmConstants.getResString("quest_saveBeforeExit"), "",
	 ...
	 */
	private String loadFromFile(File f) {
		StringBuilder text = new StringBuilder();
		String line;
		try {
	...
	private boolean saveToFile() {
		...
		try {
			...
			JOptionPane.showMessageDialog(this,
				e.getMessage(),
				Tool3lgmConstants.getResString("err_file"),
