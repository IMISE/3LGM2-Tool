...
import de.imise.tool3lgm.graphtools.userfield.dialog.UserFieldDeclarationDialog;
		...
		public static class ExportActions {
			...
			public static final Action EXPORT_DATA = new StaticAction(ActionIdentifier.export_data, PPP, true) {
				...
				public void actionPerformed(ActionEvent e) {
					if (!isEnabled())
						return;
					ExtendedFileChooser saveDialog = new ExtendedFileChooser(FileFilterType.CSV);
					...
					if (saveDialog.showSaveDialog(getTool()) == ExtendedFileChooser.APPROVE_OPTION)
						GDCollection.exportData(getSelectedDoc(), saveDialog.getSelectedFile());
				}
			};
		}
		
		/*
		 * Die Actions zum Öffnen der zuletzt verwendeten Dateien befinden sich in der
		 * Klasse {@link DynamicActions}
		 */
...
//				}
		...
		/** Löscht die Kindelemente der aktuell selektierten Elemente und hängt alle Eigenschaften der Kinder an das Oberelement */
...
/*
			private void removeChilds(ModelElement me, GraphDocument doc) {
				GDCollection gdcoll = doc.getCollection();
				ArrayList<ModelElement> parts = me.getPartElements(false);
				while (parts.size() > 0) {
					ModelElement part = parts.get(0);
					ArrayList<ModelElement> partsParents = part.getDirectParentElements();
					for (int i = 0; i < partsParents.size(); i++) {
						//in partsParents nur die Parents lassen, an die alle Verbindungen
						//des Parts umgehängt oder dupliziert werden muss (allen die auch
						//gleichzeitig Part von me sind braucht man die Informationen
						//ihrer Kinder nicht unterzuhängen, da sie ja auch weggelassen werden) 
						if (parts.contains(partsParents.get(i)))
							partsParents.remove(i--);
					}
					//sicher ist sicher -> Kopie anlegen, falls durch irgendwelche Seiteneffekte sich die Kantenliste nochmal ändert
					...
					for (int i = 0; i < partEdges.size(); i++) {
						...
						//Alle Kanten zwischen dem zu löschenden Teil und dem Oberelement, das die Eigenschaften des Teilelementes
						//Bekommen soll, werden nicht umgehängt (eigentlich kann das nur die Teil-Von-Kante selbst sein, aber in
						//neuen Metamodellen wäre auch etwas anderes denkbar)
						if (edge instanceof PartOfBeziehung)
							continue;
						//Mit wievielen Elementen von der Art des parts darf das umzuhängende Element maximal verbunden sein?
						int maxConnectedToPartCardinality = edge.isStartClass(part.getClass()) ? edge.getMaxEndToStartCardinality() : edge.getMaxStartToEndCardinality();
						for (int j = partsParents.size() - 1; j > 0; j--) {
							ModelElement parent = partsParents.get(j);
							//falls mehr Parents vorhanden sind, als mit dem umzuhängenden Element selbst verbunden 
							//sein dürfen, muss das umzuhängende Element dupliziert werden
							if (maxConnectedToPartCardinality >= j) {
								ArrayList<ModelElement> elements2Duplicate = new ArrayList<ModelElement>();
							//einfach umhängen
							} else {
								
							}
						}
						for (ModelElement parent : partsParents) {
...
*/
			...
			/**
			 * Gibt eine Liste aller Elemente zurück, die dupliziert werden müssten, um die Konsistenz zu 
			 * erhalten, wenn ein anderes Element die gleichen Verbindungen bekommen sollte wie das übergebene.
			 * 
			 * @param elementsList
			 * 		Liste aller Elemente, die dupliziert werden sollen
			 * @param sourceIndex
			 * 		Index des Elementes, dessen Verbindungen darauf geprüft werden sollen, ob die verbundenen Elemente
			 * 		dupliziert werden müssten, wenn es selbst dupliziert werden würde
			 * /
			private void fillElements2Duplicate(ArrayList<ModelElement> elementsList, int sourceIndex) {
				ModelElement me = elementsList.get(sourceIndex);
				for (Kante edge : me.getEdges()) {
					boolean meIsEdgeStart = edge.isStart(me);
					ModelElement connected = meIsEdgeStart ? edge.getEnd() : edge.getStart();
					//Mit wievielen Elementen von der Art des parts darf das umzuhängende Element maximal verbunden sein?
					int maxConnectedToOtherCardinality = meIsEdgeStart ? edge.getMaxEndToStartCardinality() : edge.getMaxStartToEndCardinality();
					int actualConnectedToOtherCardinality = meIsEdgeStart ? connected.countConnectionsToThis(edge.getClass()) : connected.countConnectionsFromThis(edge.getClass());
					//das verbundene Element darf nicht mit einem weiteren Element verbunden 
					if (actualConnectedToOtherCardinality >= actualConnectedToOtherCardinality) {
			...
			/**
			 * Dupliziert das übergebene Element und alle seine Verbindungen außer die
			 * übergebene Kante. Wenn verbundene Elemente bereits mit der maximalen
			 * Anzahl der 
			 * 
			 * @param me
			 * @param exceptionalEdge
			 * @param alreadyDuplicated
			 * 		Menge aller Elemente, die nicht dupliziert werden sollen, da sie bereits dupliziert wurde. Damit kann man verhindern,
			 * 		dass Elemente im Kreis dupliziert werden
			 * /
			public void duplicate(ModelElement me, Kante exceptionalEdge, HashSet<ModelElement> alreadyDuplicated) {
			 ...
			 * @return
			 */
			private Kante link(GDCollection gdcoll, Kante oldEdge, ModelElement start, ModelElement end) {
	...
	public static class ViewActions {
		...
		/** Wechselt das Layout des ModelBrowsers * /
		public static final Action SHOW_SUBMODELS_IN_BROWSER_SIDE_BY_SIDE = new StaticAction(ActionIdentifier.showSubModelsInBrowserSideBySide, false, UserProperties.isShowSubModelsInBrowserSideBySide()) {

			@Override
            public void actionPerformed(ActionEvent e) {
				UserProperties.setShowSubModelsInBrowserSideBySide(isSelected());
				ModelBrowser br = getTool().getModelBrowserPanel().getActiveBrowser();
				if (br != null) {
					LGMTabbedPane p = (LGMTabbedPane) br.getSelectedComponent();
					if (p != null)
		...
		// ONE_LAYER_PERSPECTIVE oder          // 
		// THREE_LAYER_PERSPECTIVE aktiviert.  //
		// Dies wird durch                     //
		// InputGraphArea.isMultiViewEnabled() //
		// sichergestellt.                     //
		/////////////////////////////////////////
	...
	public static class LayoutActions {
		 ...
		 * 
		 * Im Folgenden werden unter "Elemente" nur {@link NodeContainer} verstanden, die keine {@link BendpointContainer} sind.
		 * Das schließt demnach auch {@link EdgeContainer} aus. <br>
		 * Im Falle des Einführens eines Kanten-Layouts sollte hier paralle zu dieser Klasse eine separate Klasse mit den jeweiligen Actions erstellt werden
		 * 
		 * @author fstephan
		 */
		public static class ElementLayout {
			 ...
			 * Im Folgenden werden unter "Elemente" nur {@link NodeContainer} verstanden, die keine {@link BendpointContainer} sind.
			 * Das schließt demnach auch {@link EdgeContainer} aus. <br>
			 * Im Falle des Einführens eines Kanten-Layouts sollte hier parallel zu dieser Klasse eine separate Klasse mit den Actions für
			 * die Kanten erstellt werden.
			 */
	...
	public static class OptionsActions {
		...
		public static class Analysis {
			...
			/** 
			 * (De-)Aktiviert das Anzeigen von Konfigurations-Redundanz für {@link Aufgabe}n.
			 * TODO: AXS,FTS: Wirft Exception seit ToolMenu 
			 */
		...
		public static class Graphics {
			...
			/** Zeigt einen ColorChooser zum auswählen der Farbe, mit der Analyseergnisse in der Grafik hervorgehovben werden */
			...
			/** (Deaktiviert das Anzeigen von Bausteinschnittstellen ohne Kommunikationspartner im Teilmodell */
	...
	public static class ExtrasActions {
		...
		/** (De-)Aktiviert das automatische Zuweisen von ETNT-Verbindungen zu Kommunikationsbeziehnungen */
...
//TODO:AXS:################ Das hier muss wieder raus, wenn Frank den DescriptionFrame wieder eingebaut hat				
...
*/
	 ...
	 * Das Generieren ist notwendig, weil sich diese Actions wärend der Programmausführung 
	 * ändern können und damit nicht statisch definierbar sind.
	 * @author fstephan
	 */
	public static class DynamicActions {
		
		/** Gibt ein Array zurück, dessen Elemente Actions zum Öffnen der zuletzt verwendeten Dateien sind */
		...
		public static final Action[] getSelectInternalFrameActions() {
			...
			for(final AbstractInternalFrame internalFrame : internalFrames) {
				...
				if(internalFrame != selectedFrame) // frame ist nicht aktiviert
					next = ++index;
				else // frame ist aktiviert
					next = 0;
		...
		/**
		 * Erzeugt ein Array von Actions zum Vervinden bzw. Trennen der momentan selektierten Elemente
		 * @param command
		 * 			trennen/verbinden ({@link GDCommands#LINK}/{@link GDCommands#UNLINK})
		 * @param icon
		 * 			Trennen- bzw. Verbinden-Icon
		 * @return
		 */
		private static Action[] getConnectionActions(GDCommands command, ImageIcon icon) {
				...
				for (Class<? extends ModelElement> me2Class : doc.getSelectedRealElementClasses()) {
					for (Class<? extends Kante> edgeClass : ModelConstants.getEdgeTypes(me1Class, me2Class)) {
						if (PartOfBeziehung.class.isAssignableFrom(edgeClass)) {
							...
							// Doppeldeutige Kanten mit identischer Start- und Endklasse brauchen nur 1x angeboten werden
	...
	public static class ContextActions {
		...
		/*
		public static final Action TAKE_OVER_IN_SUBMODEL = new CommandAction(GDCommands.SELECT_LINKED_SZENARIO);
		public static final Action LINK_WITH_SUBMODEL = new CommandAction(ActionIdentifier.link_with_submodel,GDCommands.LINK_SELECTED_TO_SZENARIO, "'null'");

		public static final Action SET_ELEMENT_VISIBLE = new CommandAction(ActionIdentifier.set_element_visible,GDCommands.UNLINK);
		public static final Action SET_ELEMENT_INVISIBLE = new CommandAction(ActionIdentifier.set_element_invisible,GDCommands.UNLINK);
		public static final Action EXPAND_ELEMENT = new CommandAction(ActionIdentifier.expand_element,GDCommands.UNLINK);
		public static final Action COLLAPSE_ELEMENT = new CommandAction(ActionIdentifier.collapse_element,GDCommands.UNLINK);
		public static final Action ELEMENT_ANALYSIS = new CommandAction(ActionIdentifier.element_analysis,GDCommands.UNLINK);
		public static final Action JOIN_ELEMENTS = new CommandAction(ActionIdentifier.join,GDCommands.UNLINK);
		
		
		public static final Action CREATE_TEXTFIELD = new CommandAction(ActionIdentifier.create_textfield,GDCommands.LINK);
		
		*/
		/** 
		 * Action, die das Anzeigen von Interebenenbeziehungen für die ausgewählten Elemente/
		 * die ausgewählte Ebene (de-)aktiviert.
		 */
		...
		/*
		public static final Action VERIFICATION = new CommandAction(ActionIdentifier.verification,GDCommands.LINK);
		public static final Action INTERACTIVE = new CommandAction(ActionIdentifier.interactive,GDCommands.LINK);
		public static final Action COMMANDLINE = new CommandAction(ActionIdentifier.commandline,GDCommands.LINK);
		public static final Action SHOW_QUEUE = new CommandAction(ActionIdentifier.show_queue,GDCommands.LINK);
		public static final Action TEST = new CommandAction(ActionIdentifier.test,GDCommands.LINK);
		*/
	 ...
	 */
	@Deprecated
	public static Action[] toActionArray(Class<?> clazz) throws IllegalArgumentException, IllegalAccessException {
