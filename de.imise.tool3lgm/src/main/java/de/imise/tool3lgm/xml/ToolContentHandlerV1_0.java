...
public class ToolContentHandlerV1_0 implements ContentHandler {
	...
	/** ArrayList mit allen Containern die ein Icon besitzen; da die Icons erst zu letzt eingelesen werden, wird den Containern zuerst nur der HashString des Icons mitgeteilt. Nach dem einlesen der Icons müssen diese Container noch das eigentliche Icon aus der Hashmap der Collection laden. Das passiert in der Methode setIcon(); */
	protected ArrayList<NodeContainer> containerWithIcon = new ArrayList<NodeContainer>();

	/**
	 * 
	 */
	public ToolContentHandlerV1_0(GDCollection coll) {
		super();
		collection = coll;
	}
	...
	/**
	 * ruft Methoden doc._relinkTraces(), doc._duplicateAufOrgs(), doc._duplicateABKonfs() auf und setzt doc, container, collection, elementValue auf null;
	 * in der naechsten Version (V1_2) sollte diese Funktion ueberschrieben werden, damit die 3 Funktionen nicht mehr aufgerufen werden, da diese nur Fehler korrigert haben, die dann nicht mehr auftreten duerften 
...
//				container.refreshText();
	 ...
	 * @return
	 */
	protected static final String attsToString(Attributes atts) {
		String retVal = "";
