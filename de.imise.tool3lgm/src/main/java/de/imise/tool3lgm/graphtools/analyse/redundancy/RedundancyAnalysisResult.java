 ...
 * </ol>
 * 
 * Ein solches 'Ergebnis' wird mit den zu analysierenden Klassen und dem MetaPfad
 * initialisiert und dann in den Redundanzalgorithmus geschickt, der das Ergebnis
 * füllt.
 * 
 * 
 * @author AXS
...
public class RedundancyAnalysisResult {
	...
	/**
	 * Liste aller AWB, auf die man auch braucht, da sie in jedem Minimalset vorkommen, die aber nur eine
	 * einelementige Äquivalenzklasse bilden.
	 */
	AlphabeticalSet<ModelElement> moreNeededAWB = new AlphabeticalSet<ModelElement>();

	/** 
	 * Liste von <code>AlphabeticalSet</code>s mit AWB, die jeweils alle dieselben Aufgaben unterstützen.<br>
	 * Die betrachteten Aufgaben sind bereits eingeschränkt um alle Aufgaben, die bereits von den exklusiven
	 * AWB unterstützt werden.
	 */
	ArrayList<AlphabeticalSet<ModelElement>> equalsSets;

	/**
	 * Mappt von einem überflüssigen AWB auf alle AWB, die nicht verzichtbar sind und die diesen AWB überflüssig
	 * machen. Das heißt in der Liste sind alle AWB, die in <code>exclusiveAWB</code> oder <code>moreNeededAWB</code>
	 * vorkommen und mind. eine Aufgabe des überflüssigen AWB unterstützen.
	 */
	 ...
	 */
	private String analyseOptionString;
     ...
     * 			Klasse für deren Elemente Redundanz aufgedeckt werden soll
     * @param endClass 
     * 			Klasse bezüglich der Elemente der <code>endClass</code> redundant sein können
     * @param metaPath
     * 			MetaPfad über den Elemente der Art <code>startClass</code> und <code>endClass</code> analysiert werden sollen.
     * @param analyseOptionString
     * 			String der bei der Ausgabe des Ergebnisses als Analyseoption angezeigt werden soll
	 */
	public RedundancyAnalysisResult(GDCollection gdcoll, Class<? extends ModelElement> startClass, Class<? extends ModelElement> endClass, MetaPath metaPath, String analyseOptionString) {
		super();
		this.gdcoll = gdcoll;
