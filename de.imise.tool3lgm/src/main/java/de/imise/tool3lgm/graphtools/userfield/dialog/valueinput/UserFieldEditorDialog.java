...
public class UserFieldEditorDialog extends PropertyDialog {
	 ...
	 * Instanz kann über {@link #getDialog(Frame, GraphDocument)} erhalten werden.
	 */
	 ...
	 */
	private ClassificationNumberEditorPanel panelCN;
	
	/**
	 * Das Panel für die Verteilungsgewichte
	 */
	private DistributionWeightEditorPanel panelDW;
	
	/**
	 * Das Panel für die ModelVariablen
	 */
	private ModelVariableEditorPanel panelMV;
	
	/**
	 * Das Panel für die berechneten Kennzahlen
	 */
	private ClassificationNumberFormulaPanel panelCNF;
	 ...
	 * "Nein" gewählt hat.
	 * Wenn ja, schließt sich der Dialog und alle Änderungen gehen verloren, 
	 * wenn nein, bleibt der Dialog geöffnet.
	 */
	...
	/**
	 * Konstruktor
	 * Zugriff von außen nicht über diesen Konstruktor möglich.
	 * Verwende stattdessen {@link #getDialog(Frame, GraphDocument)}
	 */
	protected UserFieldEditorDialog(Frame owner, GDCollection gdcoll) {
		super(owner, gdcoll);
	...
	private void init() {
		...
		this.cancelAction = new AbstractAction(Tool3lgmConstants.getResString("cancel")) {
			@Override
			public void actionPerformed(ActionEvent e) {
				...
				for (int i = 0; i < panels.length; i++) {
					 ...
					 * Damit können Werte auch ohne Bestätigung mit "Enter" übernommen werden.
					 */
					panel.stopEditing();
				...
				/*
				 * Falls in einem Table Änderungen aufgetreten sind, 
				 * wird eine Datenverlust-Verwarnung angezeigt
				 */ 
				if (dataChanged == true) {
	...
	/**
	 * Setzen der Actions für <code>applyButton</code>,<code>okButton</code>
	 * und <code>cancelButton</code>.
	 */
	private void setDefaultActionsOfButtons() {
		final UserFieldEditorDialog finalDialog = this;
		/*
		 * Übernimmt die Werte aus den Tabellen der Panels ins Model, 
		 * falls sich die Daten in einer der Tabellen geändert haben.
		 */
		...
		/*
		 * Übernimmt die Werte in den Tabellen der Panels ins Model
		 * und schließt diesen Dialog anschließend.
		 */
		this.okButton.setAction(new AbstractAction(Tool3lgmConstants.getResString("ok")) {
			@Override
			public void actionPerformed(ActionEvent e) {
		 ...
		 */
		this.cancelButton.setAction(new AbstractAction(Tool3lgmConstants.getResString("cancel")) {
			@Override
			public void actionPerformed(ActionEvent e) {
				finalDialog.dispose();
			}
	...
	/**
	 * Erstellt und initialisiert ein Panel, dass das Row- und ColumnElement im Table
	 * an der aktuellen Mausposition anzeigt und fügt dieses Panel dem Dialog hinzu.
	 */
	private JPanel createElementsAtPointPane() {
		...
		UserFieldEditorPanel[] panels = this.getEditablePanels();
		for (int i=0; i<panels.length; i++) {
	 ...
	 * @param owner
	 * 			Frame, das diesen Dialog enthält
	 * @param gdcoll
	 * 			Modell, das die Daten für die Kennzahlen und Verteilungsgewichte
	 * 			der ModelElemente enthält
	 * @return JDialog 
	 */
	public static JDialog getDialog(Frame owner, GDCollection gdcoll) {
	...
	public UserFieldEditorPanel[] getEditablePanels() {
		return new UserFieldEditorPanel[] {panelCN, panelDW, panelMV, panelCNF};
	}
	...
	/**
	 * Wenn das Drücken des {@link #okButton}s Grund für das Schließen des Dialogs ist,
	 * wurden bereits alle Änderungen übernommen. <br>
	 * Falls nicht, wird {@link #cancelAction} ausgeführt, das heißt, 
	 * dass eine Datenverlust-Warnung angezeigt wird und nach Wunsch des Users
	 * alle Änderungen rückgängig gemacht werden, oder das Schließen des Dialogs 
	 * abgebrochen wird. 
	 * 
	 * Außerdem wird beim Schließen des Dialoges seine Größe und Position gespeichert
	 * und beim erneuten Öffnen wieder auf diese Werte gesetzt. Die Speicherung dieser 
	 * Werte erfolgt allerdings nur, wenn der Dialog durch "Ok" beendet wird.
	 * 
