...
/**
 * Regeln, die statt der im Metamodell vorgegebenen Regeln angegeben werden können, was bei der Konsistenzprüfung
 * als Inkonsistenz angesehen werden soll.
 * 
 * Hierrüber kann man die im Metamodell vorgegebenen Kardinalitäten für den Konsistenzcheck ändern bzw. filtern.
 *
 * @author AXS
 * @create 17.02.2011
 */
public class ConsistencyDefinition {
	 ...
	 */
	private HashMap<Class<? extends Kante>, Pair<Pair<Integer, Integer>, Pair<Integer, Integer>>> edgeClassToCardinalities = new HashMap<Class<? extends Kante>, Pair<Pair<Integer,Integer>,Pair<Integer,Integer>>>();
	
	/** Explizite Instanz des Keysets von {@link #edgeClassToCardinalities}, damit es nicht bei jedem contains()-Aufruf neu gebildet werden muss. */
	private Set<Class<? extends Kante>> edgeClassCardinalitiesKeySet = null;
	
	/** 
	 * Mappt von einer Elemnentklasse auf alle Kanten, die bei Elementen dieser Klasse nur an solchen Elementen ohne Kindelemente vorkommen dürfen.
	 * Also alle Elemente der Keyklasse dürfen keine Kante der in der Liste befindlichen Art haben, wenn ihnen über eine Teil-Von-Beziehung  ein
	 * anderes Element untergeordnet ist.
	 */
	private HashMap<Class<? extends ModelElement>, Collection<Class<? extends Kante>>> elementToOnlyLeafAllowedEdgeClasses = new HashMap<Class<? extends ModelElement>, Collection<Class<? extends Kante>>>();
	 ...
	 * @param name
	 */
	public ConsistencyDefinition(String name) {
		super();
	}
	 ...
	 * @param description
	 */
	public ConsistencyDefinition(String name, String description) {
		super();
	}
	 ...
	 */
	public void add(Class<? extends Kante> edgeClass, Integer minStartToEndCardinality, Integer maxStartToEndCardinality, Integer minEndToStartCardinality, Integer maxEndToStartCardinality) {
		if (edgeClass == null)
			return;
		...
		edgeClassToCardinalities.put(edgeClass, new Pair<Pair<Integer,Integer>, Pair<Integer,Integer>>(new Pair<Integer, Integer>(minStartToEndCardinality, maxStartToEndCardinality), new Pair<Integer, Integer>(minEndToStartCardinality, maxEndToStartCardinality)));
		//die Variable keySet wieder null setzen, um sie als geändert zu markieren
		edgeClassCardinalitiesKeySet = null;
	}
	
	/**
	 * Fügt die übergebene Kantenklase mit den ursprünglichen Kardinalitäten der Kante in die Liste ein
	 * 
	 * @param edgeClass
	 */
	public void add(Class<? extends Kante> edgeClass) {
	 ...
	 * @param edgeClass
	 */
	public void addOnlyLeafAllowedEdgeClass(Class<? extends ModelElement> elementClass, Class<? extends Kante> edgeClass) {
	...
	/**
	 * Setzt für diese Elementart, dass alle Kanten nur an Blättern im Teil-Von-Baum dieser Elemente hängen dürfen.
	 * 
	 * @param elementClass
	 */
	public void addOnlyLeafAllowedEdges(Class<? extends ModelElement> elementClass) {
	 ...
	 * @return
	 */
	public final int getMinStartToEndCardinality(Class<? extends Kante> edgeClass) {
	 ...
	 * @return
	 */
	public final int getMaxStartToEndCardinality(Class<? extends Kante> edgeClass) {
		if (edgeClassToCardinalities == null || edgeClassToCardinalities.size() == 0)
	 ...
	 * @return
	 */
	public final int getMinEndToStartCardinality(Class<? extends Kante> edgeClass) {
		if (edgeClassToCardinalities == null || edgeClassToCardinalities.size() == 0)
	 ...
	 * @return
	 */
	public final int getMaxEndToStartCardinality(Class<? extends Kante> edgeClass) {
		if (edgeClassToCardinalities == null || edgeClassToCardinalities.size() == 0)
	...
	/**
	 * Liefert <code>true</code>, wenn die Kanteklasse im Key-Set der Map auf die Kardinalitäten enthalten ist
	 * oder wenn diese Map leer ist (uns somit alle Kanten als enthalten gelten).
	 * 
	 * @param egdeClass
	 * @return
	 */
	public final boolean contains(Class<? extends Kante> egdeClass) {
