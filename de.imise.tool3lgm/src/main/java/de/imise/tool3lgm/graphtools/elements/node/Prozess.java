 ...
 * @author AXS
 *
 * Ein Prozess ist ein Knoten der Kanten zu Aufgaben haelt. 
 * Die Reihenfolge der Kanten zu den Aufgaben in der ArrayList connections legt den Prozessablauf fest.  
 */
public final class Prozess extends Knoten {

	public static Color[] farben = { Color.black, Color.blue, Color.cyan, Color.darkGray, Color.gray, Color.green,
...
//----------------------------------------------------------------------------------------------------------------------------------
	/**
	 * Gibt eine Liste mit Prozessschritten  für eine einzelne Aufgabe zurück.
	 * In den einzelnen LGMProzessStep dieser Liste sind nur die startAufgabe, endAufgabe und der Objekttyp gesetzt.
	 * endAufgabe ist immer identisch mit der Aufgabe an Position pos aus der Liste der Prozessaufgaben.
	 * 
	 * Es ist also eine Liste aller Aufgaben die im Prozess vor aufgabenPos stehen inkl. der von diesen 
	 * Aufgaben im gegebenen GraphDocument bearbeiteten Objekttypen, die von der Aufgabe an pos  gleichzeitig
	 * interpretiert werden.
	 * 
	 * Ex. kein Objekttyp den die Aufgabe an pos interpretiert und eine Vorgaengeraufgabe bearbeitet, wird ein 
	 * LGMProzessStep zurueckgegeben, in dem nur die endAufgabe der Aufgabe an pos entspricht und der Rest null ist. 
	 * 
	 * Wird testOnly=true uebergeben, dann werden nicht alle Schritte gesucht sondern nur der erste vollstaendige zurückgegeben
	 * oder eine leere Liste, wenn keiner existiert.
	 ...
	 * @param boolean
	 */
	public List<LGMProzessStep> getProcessStepsForAufgabe(List<ModelElement> aufgaben, int pos, boolean testOnly){
...
//----------------------------------------------------------------------------------------------------------------------------------
	 ...
	 * 
	 * In den einzelnen LGMProzessStep dieser Liste sind nur die startAufgabe, endAufgabe und der Objekttyp gesetzt.
	 * endAufgabe ist immer identisch mit der uebergebenen Aufgabe.
	 * 
	 * Es ist also eine Liste aller Aufgaben die im Prozess vor aufgabenPos stehen inkl. der von diesen 
	 * Aufgaben im gegebenen GraphDocument bearbeiteten Objekttypen, die von der übergebenen Aufgabe gleichzeitig
	 * interpretiert werden.
	 * 
	 * Ex. kein Objekttyp den die uebergebene Aufgabe interpretiert und eine Vorgaengeraufgabe bearbeitet, wird ein 
	 * LGMProzessStep zurueckgegeben, in dem nur die endAufgabe der uebergebenen Aufgabe entspricht und der Rest null ist. 
	 * 
	 * Wird testOnly=true uebergeben, dann werden nicht alle Schritte gesucht sondern nur der erste vollstaendige zurückgegeben
	 * oder eine leere Liste, wenn keiner existiert.
	 * Pos wird wie immer ab 0 gezaehlt.
	 * 
	 * @param ArrayList
	 * 			Liste mit Elementen der Klasse <code>Aufgabe</code>.
	 * @param ModelElement
	 * @param int
	 * @param GraphDocument
	 * @param boolean
	 */
	public List<LGMProzessStep> getProcessStepsForAufgabe(List<ModelElement> aufgaben, ModelElement aufgabe, int pos, boolean testOnly) throws IndexOutOfBoundsException, IllegalArgumentException{
...
//----------------------------------------------------------------------------------------------------------------------------------
	...
	/**
	 * Liefert eine Liste aller Prozesschritte und all ihrer Kombinationen, die sich aus unterschiedlichen
	 * Konfigurationen der Aufgaben ergeben. Die aufgaben muss eine Liste der ElementContainer von Aufgaben
	 * aus dem selben GraphDocument sein.
	 * Es wird null zurückgegeben, wenn die ArrayList aufgaben weniger als 2 Elemente hat.
	 * 
	 * Folgende Arten von Schritten koennen in der Rueckgabeliste stehen:
	 * 	1.) vollstaendige Schritte, also Schritte fuer die nach einem Kommunikationsprozess gesucht werden kann (alle ElementContainer des LGMProzessStep (Startaufgabe, Endaufgabe, Objekttyp, Startkonfiguration, Endkonfiguration) sind nicht null)
	 * 	2.)	LGMProzessStep, in dem nur die endAufgabe gesetzt ist ( = endAufgabe hat keinen Objekttyp interpretiert, den eine der Aufgaben davor im Prozess bearbeitet hat (siehe getBuisnessProcessSteps))
	 * 	3.) LGMProzessStep, in dem eine oder beide Konfigurationen null sind ( = mind. eine Aufgabe hat keine Konfiguration)
	 * 
	 * @param ArrayList
	 */
    public List<LGMProzessStep> getKommProzessStepCombinations(List<ModelElement> aufgaben){
