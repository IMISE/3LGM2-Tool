...
//TODO: Konzept der Klasse GraphElementLayout überarbeiten und toXMLString-Methode verändern und nur GraphElementLayout-Informationen speichern die nicht Standard aus Mapping entsprechen 
...
//		STANDARD_ELEMENT_LAYOUT.line_style = 
	 ...
	 * @return
	 */
	private static String getColorXMLString(Color color, String col_name) {
		if (color == null)
			return "";
	 ...
	 * @return
	 */
	public final String getXMLString(boolean forCopy, boolean expanded) {
		return getXMLString(null, forCopy, expanded);
	}

	private final String getXMLString(Class<?> elementClass, boolean forCopy, boolean expanded) {
		return "<" + (expanded ? "" : "ne") + "layout" + (elementClass!=null ? " class=\""+elementClass.getSimpleName()+"\"" : "") + ">"
			+ getColorXMLString(bg_color,"bg_color")
			+ getColorXMLString(fg_color,"fg_color")
			+ getColorXMLString(border_color,"border_color")
			+ (line_thickness != 1 ? "<line_thickness>"+line_thickness+"</line_thickness>" : "")
			+ (line_style != 0 ? "<line_style>"+line_style+"</line_style>" : "")
			+ (form != null ? "<form>"+form.ordinal()+"</form>" : "")
			+ (font != null ? "<font_family>"+font.getName()+"</font_family>" : "")
			+ (font != null ? "<font_style>"+font.getStyle()+"</font_style>" : "")
			+ (font != null ? "<font_size>"+font.getSize()+"</font_size>" : "")
			+ "<x>"+(forCopy ? x+10 : x)+"</x>"
			+ "<y>"+(forCopy ? y+10 : y)+"</y>"
			+ (width!=-1 ? "<width>"+width+"</width>" : "")
	 ...
	 */
	public String toXMLString(boolean expanded) {
	 ...
	 * @return String der vollstaendige XML-Tag zu diesem Objekt
	 */
	public String toXMLString(Class<?> elementClass, boolean expanded) {
		...
		xmlString.append(">" + getColorXMLString(bg_color, "bg_color")
			+ getColorXMLString(fg_color, "fg_color")
			+ getColorXMLString(border_color, "border_color"));
