...
/**
 * Class that manages to collect and serve all informations about elements which
 * should be highlighted in a special way for a selected element. At the moment
 * the highlight reacts only on single selections.
 *
...
public class SelectionHighlighter implements LGMChangeListenerSimple {
     ...
     * highlighted for a selection (but in another way than the selection
     * itself). This can be used to mark/highlight elements which are (better)
     * connectable a selected element.
     */
    private static final HashSet<ElementContainer> highlightForSelection = new HashSet<>();

    /**
     * @param doc the {@link GraphDocument} this highlighter reacts on selection
     *            changed events
     */
    ...
    public void selectionChanged(final GraphDocument doc) {
        ...
        Collection<IMetaPath> bestConnectableMetPath = metaModel.getBestConnectableMetPath(meClass);
        for (IMetaPath metaPath : bestConnectableMetPath) {
    ...
    /**
     * @param ec {@link ElementContainer} that should be checked if it is
     *            highlighted
     * @return <code>true</code> if the given {@link ElementContainer} is/shloud
     *         be highlighted
     */
