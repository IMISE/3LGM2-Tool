...
public class ShortestCommunicationPathFinder {
	 ...
	 * Distanzmatrix = Kostenmatrix nach FloydWarshall) als "echter Wert" auftauchen! (es ist 
	 * extrem unwahrscheinlich, dass das mal passiert, da die Einzelkosten immer 1 sind)wenn 
	 * man einen negativen Wert nehmen wollte, müsste man im Algorithmus mehr Vergleiche 
	 * anstellen -> wär langsamer 
	 */
	 ...
	 */
	private HashMap<ModelElement, ArrayList<ModelElement>> objectTypeToInterfaceList = new HashMap<ModelElement, ArrayList<ModelElement>>();
	
	/**
	 * Mappt von einem Objekttyp auf die dazugehörige Kostenmatrix der Kommunikationswege.
	 * Eine Kostenmatrix ist ein zweidimensionales Array, welches für alle Schnittstellen, 
	 * über die der Objekttyp kommuniziert werden kann, den <code>int</code>-Wert der Kosten
	 * speichert. Die Indizes der Schnittstellen in den Matrix-Feldern entsprechen den Indizes
	 * der Schnittstellen in <code>objectTypeToInterfaceList</code>.
	 * Die Kostenmatrizen haben die Form <code>int[][]</code>:<br>
	 ...
	 */
	private HashMap<ModelElement, int[][]> objectTypeToCostMatrix = new HashMap<ModelElement, int[][]>();

	/**
	 * Mappt von einem Objekttyp auf die dazugehörige Pfadmatrix der Kommunikationswege.
	 * Eine Pfadmatrix ist ein zweidimensionales Array, welches für jede Kombination aus
	 * Zeilen- und Spaltenschnittstelle die Liste der Schnittstellen speichert, über die
	 * man einen Objekttyp von der Zeilen- zur Spaltenschnittstelle versenden kann. Ist keine
	 * Kommunikation möglich, bleibt die Liste <code>null</code>. Die Indizes der Schnittstellen
	 * in den Matrix-Feldern entsprechen den Indizes der Schnittstellen in <code>objectTypeToInterfaceList</code>.
	 * Die Pfadmatrizen haben die Form <code>ArrayList[][]</code>:<br>
	 * Indizes: 1. Zeilenschnittstelle, 2. Spaltenschnittstelle<br>
	 * Wert: Liste der Schittstellen, die einen Pfad von der Zeilen- zur SPlatenschnittstelle beschreiben. Wenn
	 * es einen Pfad gibt, sind die Zeilen- und Spaltenschnittstelle immer auch enthalten.
	 */
	private HashMap<ModelElement, ArrayList<ModelElement>[][]> objectTypeToPathMatrix = new HashMap<ModelElement, ArrayList<ModelElement>[][]>();

	/**
	 * Cache zur effizienten Untersuchung von Beziehungen zwischen Elementen
	 */
	private ModelAnalyzerCache analyzerCache;
	
	/**
	 * @param analyzerCache
	 * 		ein bereits initialisierter <code>ModelAnalyzerCache</code> dessen <code>GDCollection</code>
	 * 		den Kontext vorgibt 
	 */
	public ShortestCommunicationPathFinder(ModelAnalyzerCache analyzerCache) {
		super();
	...
	/**
	 * @param gdcoll
	 * 		Modell, in dem nach kürzesten Pfaden gesucht werden soll
	 */
	public ShortestCommunicationPathFinder(GDCollection gdcoll) {
		super();
	...
	/**
	 * @param otIndex
	 * 		Index des Objekttypen, für den die Matrix ausgegeben werden soll. Wenn alle ausgegeben werden sollen, muss Index < 0 sein.
	 * /
	...
	public void printCostMatrix(int otIndex){
		//Ausgabe der Kostenmatrizen, die nach Ausführen von findShortestPath(o) die Distanzmatrizen sein sollten
		System.out.println();
	 ...
	 * 
	 * @param o
	 * 		Index des Objekttyps, für den die Pfadmatrix mit den kürzesten Pfaden aufgebaut werden soll.
	 */
    private void initShortestPath(Objekttyp ot) {
		Object interfacesObject = objectTypeToInterfaceList.get(ot);
		//wenn diese init-Methode schon einmal für den Objekttyp aufgerufen wurde -> nicht noch einmal initialisieren
		if (interfacesObject!=null)
			return;
		//alle Schnittstellen einsammeln, die den Objekttyp überhaupt übertragen können
		//über den Pfad: Objekttyp - Repräsentationsform - ETNTKombination  <-> KommBeziehung - Start und Ende der Kante
		//kann man alle Schnittstellen einsammeln, die den Objekttyp überhaupt übertragen können
		ArrayList<ModelElement> interfaces = new ArrayList<ModelElement>();
		//Liste aller Schnittstellenpaare, von denen der Objekttyp von der ersten zur zweiten Schnittstelle geschickt 
		//werden kann. Wird ein Objekttyp mit Teil-Objekttypen über eine Kommunikationsbeziehung geschickt, so gilt 
		//das auch für alle seine Teile
		HashSet<SameTypePair<ModelElement>> sendToReceiveInterfacePairs = new HashSet<SameTypePair<ModelElement>>();
		...
		HashSet<ModelElement> reprSet = new HashSet<ModelElement>();
		...
		for (ModelElement ntdt : reprSet) {
			...
			//für jede ETNTKombination und ETDTKombination alle Kommunikationsbeziehungen einsammeln
			for (ModelElement etntdtKombi : ntdt.getConnectedElements(EtntEtdtKombination.class)) {
				int[] directions = {Doppelkante.FORWARD, Doppelkante.BACKWARD};
				for (int d=0; d<directions.length; d++){
					//für alle Kommunikationsverbindungen, die den Objekttyp in der jewieligen Richtung verschicken können
					for (ModelElement commBezMe : etntdtKombi.getConnectedElements(KommBeziehung.class, KommbezEtntVerbindung.class, directions[d])) {
						...
						if (directions[d]==Doppelkante.FORWARD)
							sendToReceiveInterfacePairs.add(new SameTypePair<ModelElement>(start, end));
						else
							sendToReceiveInterfacePairs.add(new SameTypePair<ModelElement>(end, start));
					}
				}
			}
		}
		...
		ArrayList<ModelElement>[][] pathMatrix = new ArrayList[N][N];
...
//		int f0=0, f1=0, f2=0, f3=0;
		...
		for (int z = 0; z < N; z++) {
			//wird die Liste aller Schnittstellen, die die aktuelle Schnittstelle mit OT erreichen kann
			...
			//Eigentlich sollte eine Schnittstelle immer genau mit 1 Anwendungssystem verbunden sein. Damit im Fehlerfall (keins oder mehr als 1)
			//keine üble Exception fliegt, gehen wir hier mal nicht von dem "eigentlich"-Fall aus
			Collection<ModelElement> sameAWBsOfInterfaceZ = new HashSet<ModelElement>();
...
//			System.out.println(ss+ " kommt an " + al.size() + " Schnittstellen");
			...
			for (int s = 0; s < N; s++) {
				//die Schnittstelle hat zu sich selbst die Kommunikationskosten 0. Die Pfadmatrix bleibt hier leer
				if (s == z) {
					costMatrix[z][s] = 0;
//					f0++;
					continue;
				...
				ArrayList<ModelElement> awbOfInterfaceS = sS.getConnectedElements(Anwendungsbaustein.class, AwbKommssVerbindung.class);
				//wenn die beiden Schniistellen auf dem selben Baustein liegen, haben sie die Kosten 0 und man muss sich den Pfad merken
				if (sameAWBsOfInterfaceZ.containsAll(awbOfInterfaceS)){
					costMatrix[z][s] = 0;
					pathMatrix[z][s] = new ArrayList<ModelElement>();
					pathMatrix[z][s].add(sZ); 
					pathMatrix[z][s].add(sS);
//					f1++;
				//wenn sie nicht auf demselben Baustein liegen und eine Kommunikationsbeziehung zwischen ihnen besteht,
...
// 					f3++;
			...
			for (int z = 0; z < N; z++){
				for (int s = 0; s < N; s++) {
					...
					if (tempDistance < costMatrix[z][s]) {
						...
						pathMatrix[z][s] = new ArrayList<ModelElement>(pathMatrix[z][pivot]);
	 ...
	 * @return
	 */
	public MinimalCommunicationPath getShortestPath(Anwendungsbaustein startAWB, Anwendungsbaustein endAWB, Objekttyp objekttyp){
		ArrayList<ModelElement> start = new ArrayList<ModelElement>(1);
		start.add(startAWB);
		ArrayList<ModelElement> end = new ArrayList<ModelElement>(1);
		end.add(endAWB);
	 ...
	 * @return
	 */
	public MinimalCommunicationPath getShortestPath(Collection<ModelElement> startAWBs, Collection<ModelElement> endAWBs, Objekttyp objekttyp){
	...
	public class MinimalCommunicationPath {
		...
		/**
		 * Länge des Pfades über die Schnittstellen. Wenn die erste und 2. Schnittstelle eines Pfades auf demselben
		 * Anwendungssystem liegen, so kann dieser Pfad von den Pfadkosten her trotzdem minimal sein, da die Kosten
		 * für Schnittstellen auf dem selebn Anwendungssystem immer 0 betragen. Ein wikrlich minimaler Pfad hat die
		 * geringsten Kosten bei geringster Länge. 
		 */
		 ...
		 * @param objekttyp
		 */
		private MinimalCommunicationPath(Collection<ModelElement> startAWBs, Collection<ModelElement> endAWBs, Objekttyp objekttyp, ShortestCommunicationPathFinder sPathFinder){
			...
			//wenn keine Kommunikation nötig ist, weil Start- und Endbaustein auf demselben Gesamtbaustein liegen
			...
			//jetzt alle Teile und Oberbausteine der Start- und Endbausteine in die Prüfung einschließen
			...
			//wenn noch Teile oder Oberbausteine hinzugeommen sind -> die Pürfung nochmal machen (dieses
			//doppelte Vorgehen hat den Vorteil, dass zuerst ein Element gefunden werden, dass direkt in den
			//startAWBs und endAWBs enthalten war und erst dann ein Element aus den Ereiterung fullStartAWBs und fullEndAWBs   
			...
			ArrayList<ModelElement>[][] pathMatrix = sPathFinder.objectTypeToPathMatrix.get(objekttyp);
			...
			loop: for (ModelElement startSS : startSchnittStellen) {
				...
				for (ModelElement endSS : endSchnittStellen) {
					...
					ArrayList<ModelElement> path = pathMatrix[posOfStartInSchnittStellen][posOfEndInSchnittStellen];
