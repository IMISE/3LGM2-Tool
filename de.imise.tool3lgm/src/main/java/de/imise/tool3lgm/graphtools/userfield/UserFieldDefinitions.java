...
/**
 * Beinhaltet alle <code>UserField</code>s in einer <code>HashMap</code>, 
 * die für Knoten, Kanten und das Modell deklariert und definiert wurden. 
...
public class UserFieldDefinitions extends UserFieldDefinitionChangeHandler implements Cloneable {
	...
	/**
	 * Klasse, über die die sogenannten Modellvariablen identifiziert werden, also Variablen,
	 * die nicht für ein spezielles Element sondern für das Gesamtmodell gelten und zur 
	 * Verfügung stehen.
	 */
	...
	/**
	 * Liefert einen anzeigbaren String für den globalen Identifier (da es Modellvariablen
	 * sind wird hier der Res-String für Model zurück gegeben)
	 * @return
	 */
	...
	/**
	 * Konstante um für <code>firstInconsistentUserFieldFormulaIndex</code> anzugeben, dass
	 * alle darin befindlichen Formeln berechnet werden können.
	 */
	private static final int NO_INCONSISTENCE_INDEX_FOUND = -1;
	
	/**
	 * Maximale Anzahl der abhängigen <code>UserField</code>s, die im Warnungsdialog vor dem Löschen
	 * eines <code>UserField</code>s angezeigt werden.
	 */
	private static final int MAX_USED_USERFIELD_DELETE_NUMBER = 10;
	
	/**
	 * Konstante um für <code>firstInconsistentUserFieldFormulaIndex</code> anzugeben, dass
	 * die Liste <code>formulaUserFieldList</code> neu sortiert werden müsste, um festzustellen,
	 * ob sich alle Formeln berechnen lassen bzw. welche inkonsitent sind.
	 */
	private static final int FORMULA_INCONSITENCE_INDEX_UNKNOWN = -2;
	
	/**
	 * Die Liste <code>formulaUserFieldList</code> wird (wenn keine Kreisreferenzen in den
	 * Formeln vorkommen) so sortiert, dass sich jede Formel in der Liste berechnen lässt,
	 * wenn alle Formeln berechnet wurden, die sich in der Liste davor befinden. Sollte doch
	 * mind. eine Kreisreferenz vorliegen, wird in dieser Variable hier der Index des ersten
	 * <code>UserField</code>s in <code>formulaUserFieldList</code> gespeichert, der sich nicht
	 * mehr berechnen lässt. Lassen sich alle Formeln berechnen, dann ist der Index -1.
	 */
	 ...
	 * @param gdcoll
	 */
	public UserFieldDefinitions(GDCollection gdcoll) {
		super(gdcoll);
		calculator =  new Calculator(this);
	}
	
	/**
	 * Hängt der zuletzt benutzen Liste ein neues Element an.
	 * Die Methode erwartet beim Aufruf ein <code>UserField</code>. Das <code>UserField</code> wird an eine Liste, 
	 * die sich in der <code>classToUserFieldListMap</code>- HashMap befindet, angehangen.  
	 * 
	 * 
	 ...
	 * Das trifft bei Kennzahlformeln, Knenzahlen und Verteilungsgewichten zu, die in
	 * anderen Kennzahlformeln verwendet werden.
	 * Außerdem wird für zu löschende Format-UserFields geprüft, ob sie mind. einem anderen
	 * UserField als Format zugewiesen sind. 
	 * 
	 * @param userField
	 * @return
	 */
	public final boolean isInUse(UserField userField){
	 ...
	 */
	public ArrayList<UserField> remove(UserField userField) {
	 ...
	 * @return Anzahl der userFields
	 * /
	public int getUserFieldCount(Class<? extends ModelElement> userFieldTargetClass) {
		if (!classToUserFieldListMap.containsKey(userFieldTargetClass))
			return 0;
		UserFieldList ufl = (UserFieldList) classToUserFieldListMap.get(userFieldTargetClass);
		return ufl.getUserFieldsCount();
	}

	/**
	 * Gibt die Anzahl der globalen UserField zurück.
	 * @return Anzahl der globalen userFields
	 * /
	public int getGlobalUserFieldCount() {
		return getUserFieldCount(GLOBAL_USERFIELD_IDENTIFIER_CLASS);
	}
*/
	 ...
	 * @param otherDef
	 */
	public void addAll(UserFieldDefinitions otherDef) {
		for (Class<? extends UserFieldTarget> clazz : classToUserFieldListMap.keySet())
	 ...
	 * @return
	 */
	public String getCopyString(Set<UserField> userFields) {
	 ...
	 * @param name
	 * @return
	 * 		das erstebeste UserField mit dem übergebenen Namen oder <code>null</code>, wenn keins gefunden wurde
	 */
	public UserField getUserField(Class<? extends UserFieldTarget> userFieldTargetClass, String name) {
...
*/
	 ...
	 */
	public boolean hasUserFields(Class<? extends UserFieldTarget> userFieldTargetClass) {
	 ...
	 * @return
	 */
	public Iterable<UserField> getUserFields(Class<? extends UserFieldTarget> userFieldTargetClass) {
	...
	/** 
	 * Die Methode <code>get</code> gibt unter Angabe der zugehörigen Klasse 
	 * und des entsprechenden Indices ein <code>UserField</code> zurück. 
	 * Es wird aus der HashMap die zur übergebenen Klasse gehörende ArrayList geladen, falls sie nicht schon geladen ist, 
	 * und das Element an der Stelle<code>index</code> zurückgegeben.
	 * 
	 * @param userFieldTargetClass
	 * @param index
	 * @return UserField 
	 * /
	public UserField get(Class<?> userFieldTargetClass, int index) {
		UserFieldList ufl = classToUserFieldListMap.get(userFieldTargetClass);
		if (ufl != null)
			return ufl.get(index);
		return null;
	}

	/**
	 * Liefert das globale {@link UserField} mit dem entsprechenden Index (oder <code>null</code> wenn
	 ...
	 * @see #get(Class, int) 
	 * /
	public UserField getGlobal(int index) {
		return get(GLOBAL_USERFIELD_IDENTIFIER_CLASS, index);
	}

	
	
	/**
	 * Gibt <code>UserField</code> zurück, für das der <code>hashString</code> angegeben wurde.
	 * @param hashString
	 * @return <code>UserField</code>
	 */
	public UserField getUserField(String hashString) {
		return hashStringToUserFieldMap.get(hashString);
	}
	

	/**
	 * Prüft für ein übergebenes <code>UserField</code>, ob es zu den berechenbaren
	 * Formel-UserFields gehört.
	 * 
	 * @param formulaUserField
	 * @return
	 * 		<code>true</code>, wenn es ein Formel-UserField ist, das berechnet werden kann, sonst <code>false</code>
	 */
	public boolean isCalculatable(UserField formulaUserField){
	...
	/**
	 * Prüft, ob die Formeldefinitionen konsistent sind - also ob sich die Formeln
	 * nicht im Kreis referenzieren.
	 * 
	 * @return
	 * 		<code>false</code>, wenn alle Formeln berechnet werden können, sonst <code>true</code>
	 */
...
//			UserProperties.setEnableClassificationNumberCalculation(false);
	 ...
	 * @return
	 */
	public static boolean hasSimpleCrossReferences(UserField userField) {
			...
			if (firstString.equals(userField.getHashCode())) {
				...
				if (secondString.equals(Calculator.OPERAND_DELIMITER)) {
					if (st.hasMoreTokens())
						...
						// Jetzt prüfen, ob schon die Ricthung kommt oder erst
						// ein VG
	...
	/**
	 * @return
	 * 		Iterator aller Schlüsselwerte der Map, die von den Klassen auf die für sie
	 * 		definierten UserFields mappt
	 */
	...
	/**
	 * Sortiert die Liste <code>formulaUserFieldList</code> so, dass sie konsistent ist.
	 * Siehe Kommentar zur Variable <code>formulaUserFieldList</code>.
	 * 
	 * @return
	 * 		Liste von <code>UserField</code>s, die sich im Kreus referenzieren oder von solchen Elementen
	 * 		abhängig sind bzw. <code>null</code>, wenn es keine Kreisreferenzen gibt
	 */
	...
	/* (non-Javadoc)
	 * @see tool3lgm.graphtools.userfield.event.UserFieldEventHandler#clearCalculatedUserFieldValues()
	 */
	@Override
	...
	/**
	 * Berechnet den Wert des übergebenen <code>UserField</code>s für das übergebene
	 * <code>UserFieldTarget</code>.
	 *  
	 * @param userField
	 * @param target
	 * @return
	 * 		<code>UserField.ERROR_OBJECT_VALUE</code>, wenn die Berechnung nicht 
	 * 		durchgeführt werden konnte oder den <code>Double</code>-Wert als <code>String</code> 
	 */
	protected final String calculate(UserField userField, UserFieldTarget target){
...
*/
	...
	/**
	 * Immer wenn sich was an den Definitionen ändert muss diese Funktion aufgerufen werden,
	 * damit die Definition auf CrossReferences geprüft wird. Beim Hinzufügen oder Entfernen
	 * von UserFields macht die Definition das allein, aber wenn von einer Kennzahlformel der
	 * FormelString geändert wird, muss das der Ändernde machen.
	 */
	...
	/**
	 * @param format
	 * 		Format-UserField, für das alle Kennzahlen zurück gegeben werden sollen, die es benutzen
	 * @return
	 * 		Liste aller USerFields, die das übergebene Format-Userfield als benutzen
	 */
	public ArrayList<UserField> getFormatUser(UserField format){
