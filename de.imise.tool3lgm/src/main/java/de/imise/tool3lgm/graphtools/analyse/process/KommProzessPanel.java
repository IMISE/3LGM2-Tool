 ...
 * @author AXS
 * 
 *         Dieses Panel ist das KommunikationsprozessPanel im
 *         ElementPropertyDialog eines (Geschäfts-)Prozesses
 */
public class KommProzessPanel extends ElementDialogPanel {
	...
	/**
	 * Das {@link GraphDocument}, in dem die Teilprozessschritte visualisiert
	 * werden sollen.
	 */
	@SuppressWarnings("unused")
	private GraphDocument selectedDoc;

	/**
	 * @param pd
	 */
	public KommProzessPanel(ElementPropertyDialog pd) {
		super(pd);
	 ...
	 * COMMENTME
	 */
	private long lastDocModificationTime = System.currentTimeMillis();

	/**
	 * @param checkRowCount
	 */
	public void updateTable(boolean checkRowCount) {
		...
		for (int i = 0; i < linesCount; i++) {
			...
			if (!showTeilmodelle) {
				...
				if (me != null) { // jetzt nur noch vollständige Schritte
					...
					me = step.getStartAufgabeKonf(); // Startkonfiguration es ex. eine Startkonfiguration
					if (me != null) {
			...
			// es wird irgendwas kommuniziert
			else {
		...
		synchronized (table.getTreeLock()) {
			...
			if (i >= 0 && i < allSteps.size()) {
				...
				if (enumerateSchnittstellen) {
					...
					allSpecialInfoTargetContainer.addAll(specialInfoTargetContainer);
//					ElementContainer.writeNumberListToTartgets(selectedStep, specialInfoTargetContainer, prozessC.get3LGMLayout());
				}
...
//					ElementContainer.writeNumberListToTartgets(selectedStep, kantenContainerListe, prozessC.get3LGMLayout());
				}
	...
	private class MyTable extends JTable {
		 ...
		 * @param parent
		 */
		public MyTable(DefaultTableModel model, KommProzessPanel parent) {
			super(model);
			this.parent = parent;
	 ...
	 * @param edp
	 */
	private static final LGMAction getShowTeilModelleAction(ElementDialogPanel edp) throws ActionNotDefinedForClassException {
	 ...
	 * @param resString
	 */
	public static final LGMAction getRefreshHighlightsAndSpecialInfoAction(ElementDialogPanel edp, String resString) throws ActionNotDefinedForClassException {
		final ElementDialogPanel pane = edp;
		if (pane instanceof KommProzessPanel) {
