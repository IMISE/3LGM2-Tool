...
public class BasicGraphArea extends JComponent implements ZoomableComponent {
	...
	/**  */
	private int frame_width = page_width, frame_height = page_height;
	...
	private Insets graphBorder = new Insets(50, 50, 50, 50);
	 ...
	 * @param gdoc
	 */
	public BasicGraphArea(GraphDocument gdoc) {
		setLayout(null);
		doc = gdoc;
	 ...
	 * @param gd
	 */
	public final void setDocument(GraphDocument gd) {
		doc = gd;
		revalidate();
		repaint();
	}
	 ...
	 */
	public final void setPageSize(int width, int height) {
	 ...
	 * @param shift
	 */
	public final void setInterLayerSpace(int shift) {
	...
	/* (non-Javadoc)
	 * @see tool3lgm.imexport.ComponentAsImageExportHandler.ZoomableComponent#setZoomToMaximum()
	 */
	@Override
...
//		transformation = new AffineTransform(zoom * X_X_FACTOR, zoom * X_Y_FACTOR, zoom * y_x_factor, zoom * y_y_factor, 0, 0);
	...
	protected final void check_size() {
		if (!(this.doc instanceof Szenario))
			return;
		getExtensionSize();
		adjust_size();
	}
	...
	protected void adjust_size() {
		...
		this.setPreferredSize(new Dimension(c.right - c.left, c.bottom - c.top));
	 ...
	 * @param element
	 */
	public void elementGraphicsChanged(ElementContainer element) {
		if (element != null) {
	 ...
	 * @param element
	 */
	public void elementAdded(ElementContainer element) {
		if (element != null) {
			element.getParent().validate();
			element.getParent().repaint();
		} else {
	 ...
	 * @param value
	 */
	public void setMultiViewPitchShift(int value) {
	 ...
	 * @param value
	 */
	public void setMultiViewDegree(int value) {
		if (isMultiViewEnabled())
			setDegree(value);
		else
			old_degree = value;
