        ...
        if (firstBrowser != null) { //null if no model is opened
            if (showModelsInSeparateBrowser) {
                ...
                while (firstBrowser.getTabCount() > 1) {
                    // if the space is not enough for the only tab, there should not be shown the useless scroll buttons
                    // but don't set the wrap layout as tab policy because it centers the tab title in this case
