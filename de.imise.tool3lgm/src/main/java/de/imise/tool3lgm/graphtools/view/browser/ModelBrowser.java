...
public final class ModelBrowser extends JTabbedPaneWithCloseIconsRight implements ChangeListener, FocusListener {
    ...
    protected void addCollection(final GDCollection gdcoll) {
        ...
        addTab(subModelsBrowser.getTitle(), subModelsBrowser.getIcon(), subModelsBrowser, gdcoll.getFile() != null ? gdcoll.getFile().getAbsolutePath() : null);
    ...
    public void removeTabAt(int index) {
        if (!isInTabReordering()) {
            if (closeDocOnCloseTab) {
                ...
                // Is null if the model was closed via Close Model menu entry.
                // Is not null if the model should be closed over the Close button on the tab.
                if (doc != null) {
                    closeTab = Static.getTool().fileClose(doc); // don't close if fileClose was canceled
                }
                if (closeTab) {
        ...
        } else { // Reorder Tabs -> simple remove
            super.removeTabAt(index);
        }
    }
