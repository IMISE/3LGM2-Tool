...
public abstract class ModelElement extends UserFieldTarget {
	...
	/**
	 * Table, der von einem <code>GraphDocument</code> auf den Container des Elemtentes in diesem
	 * <code>GraphDocument</code> mappt, wenn es darin vorkommt.
	 */
	...
	/**
	 * Ein StringBuilder, der gebraucht wird, um die Namen der Elemente zusammen zu bauen.
	 * Er ist statisch, damit man ihn nicht ständig neu anlegen muss.
	 */
	...
	/**
	 * HashString des Teilmodells, mit dem das Element verknüpft ist. Diese Verknüpfung
	 * sagt einfach nur aus, dass das Element in dem Teilmodell näher berschrieben wird
	 * (z.B. duch seine Teile). Es kann, aber muss selbst nicht in diesem Teilmodell vorkommen. 
	 */
	...
	/**
	 * Liefert den Index der Ebene, auf dem das Element liegt. Diese Funktion wird von den
	 * konkreten Elementen überschrieben. Das braucht man neben den der Möglichkeit das für
	 * eine Klasse über die {@link ModelConstants} zu sagen, weil es nicht bei allen Klassen
	 * der Layer feststeht (Knickpunkte)
	 * 
	 * @return
	 */
	public int layerFor() {
	 ...
	 * @return
	 */
	public static final String getNewHashString(ModelElement me) {
	 ...
	 * @return
	 */
	public static final String getNewHashString(Class<? extends ModelElement> elementClass) {
		elementCounter++;
	 ...
	 * @param ec
	 */
	public final void setContainer(GraphDocument gd, ElementContainer ec) {
		if (gd != null)
	 ...
	 * @param gd
	 */
	public final void removeContainer(GraphDocument gd) {
	 ...
	 * @return
	 */
	public final ElementContainer getContainer(GraphDocument gd) {
		if (gd == null)
			return null;
	...
	/**
	 * Berechnet aus dem HashString das Datum, an dem das Element erstellt wurde.
	 * Lässt sich das Datum aus irgendwelchen Gründen nicht berechnen kommt new STANDARD_CREATION_DATE = new Date(0) zurück. 
	 * 
	 * @return
	 */
	...
	/**
	 * Setzt den Namen des Objektes und sortiert die Liste der {@link NodeContainer}
	 * im LayerContainer
	 * 
	 * @param name
	 */
	...
	/**
	 * Setzt den Namen des Objektes und sortiert die Liste der {@link NodeContainer}
	 * im LayerContainer, wenn sort==true
	 * 
	 * @param name
	 * @param sort
	 */
	public void setName(String name, boolean sort) {
		toStringName = null;
		if (name == null)
			return;
		...
		if (isHyperlink()) textBuf.append("<U>");
...
//				System.err.println("ModelElement.refreshText() " + this + " -> " + text + );
	...
	/**
	 * if you overwrite this methode in a sub-class nevertheless please always
	 * call this methode by super.getXMLEntities()
	 * 
	 ...
	 * @param value
	 *            Wert des Feldes (als unkodierte String)
	 ...
	 * @param value
	 *            String with the value for this field
	...
	public boolean addEdge(Kante kante) {
	...
	/**
	 * Fuegt diesem Knoten in der ArrayList connections an der Position pos die
	 * Kante kante hinzu.
	 */
	public boolean insertEdge(Kante kante, int pos) {
		if ((kante == null) || (edges.contains(kante)))
			return false;
	...
	public void setEdges(ArrayList<Kante> associations) {
		edges = associations;
	}

	/** Setzt in connections die Kante an Position pos auf kante. */
	public boolean setEdge(int pos, Kante kante) {
	...
	public final void removeEdge(Kante kante) {
		edges.remove(kante);
	}
	...
	public final ArrayList<Kante> getEdges() {
		return edges;
	}

	/** Gibt die Verbindung Nummer <i>index </i> zurueck */
	public final Kante getEdge(int index) {
	 ...
	 * @param index
	 * @return
	 * 		Knoten ueber die Verbindung Nummer <i>index </i> zurueck
	 */
	public final ModelElement getConnectedElement(int index) {
	 ...
	 * @param edge
	 * @return
	 * 		<code>true</code>, wenn Kante <i>k</i> an diesem Knoten ansetzt, sonst <code>false</code>
	 */
	public final boolean hasConnection(Kante edge) {
	 ...
	 * @param modelElement
	 * @return
	 * 		<code>true</code>, wenn eine Kante zwischen diesem und dem übergebenen Element besteht
	 */
	public final boolean isConnectedWith(ModelElement modelElement) {
	 ...
	 * @param modelElement
	 * @return
	 * 		<code>true</code>, wenn eine Kante von diesem zu dem übergebenen Element besteht
	 */
	public final boolean isConnectedTo(ModelElement modelElement) {
		for (int c = 0; c < edges.size(); c++)
	 ...
	 * @param modelElement
	 * @return
	 * 		<code>true</code>, wenn eine Kante vom übergebenen yu diesem Element besteht
	 */
	public final boolean isConnectedFrom(ModelElement modelElement) {
		for (int c = 0; c < edges.size(); c++)
	...
	/**
	 * Prüft, ob zwischen diesem und dem übergebenen Element eine Kante der angegebenen Art
	 * existiert. Die Richtung ist dabei egal.
	 * 
	 * @param modelElement
	 * 		Element zu dem die Existenz einer Verbindung geprüft werden soll
	 * @param edgeClass
	 * 		Art der Kante, die gesucht werden soll
	 * @return
	 * 		<code>true</code>, wenn eine Kante zwischen diesem und dem übergebenen Element besteht
	 */
	public final boolean isConnectedWith(ModelElement modelElement, Class<? extends Kante> edgeClass) {
		for (int c = 0; c < edges.size(); c++)
	...
	public final boolean isConnectedTo(ModelElement k, Class<? extends Kante> edgeClass) {
		for (int c = 0; c < edges.size(); c++)
	...
	public final boolean isConnectedFrom(ModelElement k, Class<? extends Kante> edgeClass) {
		for (int c = 0; c < edges.size(); c++)
	...
	/**
	 * Gibt die erste Kante vom Typ <code>edgeClass</code> zurück, die von 
	 * diesem <code>ModelElement</code> zum <code>ModelElement k</code> geht.
	 * 
	 * Führt <code>getConnectionTo(ModelElement, Class, int)</code> für alle
	 * Positionen aus.
	 * 
	 * @param modelElement
	 * @param edgeClasses
	 * @return
	 */
	public final Kante getEdgeTo(ModelElement modelElement, Class<? extends Kante> edgeClass) {
		for (int i = 0; i < edges.size(); i++) {
			Kante kante = getEdgeTo(modelElement, edgeClass, i);
			if (kante != null) // Kante gefunden
				return kante;
		}
		return null;
	}
	 ...
	 * @return
	 */
	public final Kante getEdgeTo(ModelElement modelElement, Class<? extends Kante> edgeClass, int position) {
		for (int c = 0; c < edges.size(); c++) {
	 ...
	 * @return
	 */
	public final Kante getEdgeFrom(ModelElement modelElement, Class<? extends Kante> edgeClass, int position) {
		for (int c = 0; c < edges.size(); c++) {
			if ((position != GDCommands.INVALID_EDGE_INDEX) && (position != c))
				continue;
			Kante ka = getEdge(c);
	 ...
	 * @return
	 */
	public ArrayList<Kante> getEdge(Class<? extends ModelElement> elemClass, Class<? extends Kante> edgeClass, int direction){
	 ...
	 * @return
	 */
	public final ArrayList<Kante> getEdgesWith(ModelElement modelElement) {
	 ...
	 * @return
	 */
	public final ArrayList<Kante> getEdgesWith(ModelElement modelElement, Class<? extends Kante> edgeClass) {
...
*/	
	 ...
	 * @return
	 */
	public final ArrayList<Kante> getEdgesTo(ModelElement modelElement, Class<? extends Kante> edgeClass) {
	 ...
	 * @return
	 */
	public final ArrayList<Kante> getEdgesTo(ModelElement modelElement, Class<? extends Kante> edgeClass, int position) {
	 ...
	 * @return
	 */
	public final ArrayList<Kante> getEdgesWith(ModelElement modelElement, Class<? extends Kante> edgeClass, int position, int direction) {
		ArrayList<Kante> retVal = new ArrayList<Kante>();
		if (modelElement == null)
			return retVal;
		for (int c = 0; c < edges.size(); c++) {
	...
	/**
	 * Sucht alle Kanten, die diesen Knoten mit Knoten des angegebenen Typs
	 * verbinden.
	 * 
	 * @param elementClass
	 *            Klasse der verbundenen Knoten
	 * @return ArrayList mit allen gefundenen Kanten
	 */
	public final ArrayList<Kante> getEdgesWith(Class<? extends ModelElement> elementClass) {
		return getEdgesWith(elementClass, null);
	}

	/**
	 * Sucht alle Kanten des angegebenen Typs, die diesen Knoten mit Knoten des angegebenen Typs
	 * verbinden. Wird als <code>edgeClass</code> <code>null</code> übergeben, werden alle Kantenarten zurückgegeben.
	 * 
	 * @param elementClass
	 *            Klasse der verbundenen Knoten
	 * @param edgeClass
	 * 				Kanteklasse nach der gesucht werden soll
	 * @return ArrayList mit allen gefundenen Kanten
	 */
	public final ArrayList<Kante> getEdgesWith(Class<? extends ModelElement> elementClass, Class<? extends Kante> edgeClass) {
		return getEdgesWith(elementClass, edgeClass, Doppelkante.ANY);
	}

	/**
	 * Sucht alle Kanten des angegebenen Typs, die diesen Knoten in Vorwärtsrichtung mit Knoten des angegebenen Typs
	 * verbinden. Wird als <code>edgeClass</code> <code>null</code> übergeben, werden alle Kantenarten zurückgegeben.
	 * 
	 * @param elementClass
	 *            Klasse der verbundenen Knoten
	 ...
	 * @return ArrayList mit allen gefundenen Kanten
	 */
	public final ArrayList<Kante> getEdgesTo(Class<? extends ModelElement> elementClass, Class<? extends Kante> edgeClass) {
		return getEdgesWith(elementClass, edgeClass, Doppelkante.FORWARD);
	}

	/**
	 * Sucht alle Kanten des angegebenen Typs, die diesen Knoten in Rückwärtsrichtung mit Knoten des angegebenen Typs
	 * verbinden. Wird als <code>edgeClass</code> <code>null</code> übergeben, werden alle Kantenarten zurückgegeben.
	 * 
	 * @param elementClass
	 *            Klasse der verbundenen Knoten
	 ...
	 * @return ArrayList mit allen gefundenen Kanten
	 */
	public final ArrayList<Kante> getEdgesFrom(Class<? extends ModelElement> elementClass, Class<? extends Kante> edgeClass) {
		return getEdgesWith(elementClass, edgeClass, Doppelkante.BACKWARD);
	}

	/**
	 * Sucht alle Kanten des angegebenen Typs, die diesen Knoten mit Knoten des angegebenen Typs
	 * verbinden. Wird als <code>edgeClass</code> <code>null</code> übergeben, werden alle Kanten zurückgegeben.
	 * 
	 * @param elementClass
	 *            Klasse der verbundenen Knoten
	 * @param edgeClass
	 * 				Kanteklasse nach der gesucht werden soll
	 * @param
	 * 			Richtung der Kante nach der gesucht werden soll (<code>Doppelkante.ANY</code>, <code>Doppelkante.FORWARD</code> oder
	 * 			<code>Doppelkante.BACKWARD</code>)
	 * 
	 * @return ArrayList mit allen gefundenen Kanten
	 */
	public final ArrayList<Kante> getEdgesWith(Class<? extends ModelElement> elementClass, Class<? extends Kante> edgeClass, int direction) {
	 ...
	 * diesem Knoten verbundenen Knoten der Klasse <code>searchElementClass</code>, die in
	 * doc enthalten, sind zurueck
	 * 
	 * @param searchElementClass
	 * 		Art der verbundenen Elemente, deren Container geliefert werden sollen
	 * @param doc
	 *		Knoten aus diesem Dokument
	 * @return ArrayList mit ElementContainer der gefundenen Knoten
	 */
	public final ArrayList<ElementContainer> getConnectedContainer(Class<? extends ModelElement> searchElementClass, GraphDocument doc) {
		return getConnectedContainer(searchElementClass, doc, null, Doppelkante.ANY, true);
	}

	/**
	 * Gibt eine alphabetisch sortierte Liste aller ElementContainer zurück, die mit diesem Knoten 
	 * übder die angegebene Kantenart verbundenen sind.
	 * 
	 * @param doc
	 *		Knoten aus diesem Dokument
	 * @param searchEdgeClass
	 * 			Art der zu suchenden verbindungen
	 * @return ArrayList mit ElementContainer der gefundenen Knoten
	 */
	public final ArrayList<ElementContainer> getConnectedContainer(GraphDocument doc, Class<? extends Kante> searchEdgeClass) {
	 ...
	 * @return
	 */
	public final ArrayList<ElementContainer> getConnectedContainer(Class<? extends ModelElement> searchElementClass, GraphDocument doc, Class<? extends Kante> edgeClass) {
	 ...
	 * @return
	 */
	public final ArrayList<ElementContainer> getConnectedContainer(Class<? extends ModelElement> searchElementClass, GraphDocument doc, Class<? extends Kante> edgeClass, int direction) {
	 ...
	 * diesem Knoten verbundenen Knoten des Klasse searchElementClass, die in
	 * doc enthalten, sind zurueck
	 * 
	 * @param searchElementClass
	 *            Elementklasse deren Objekte zurück gegeben werden sollen
	 * @param doc
	 *            Knoten aus diesem Dokument
	 * @param start
	 *            true = Verbindungen beginnen nicht bei diesem Knoten
	 * @param end
	 *            true = Verbindungen enden nicht bei diesem Knoten
	 * @return ArrayList mit ElementContainer der gefundenen Knoten
	 */
	@SuppressWarnings("unchecked")
    public final ArrayList<ElementContainer> getConnectedContainer(Class<? extends ModelElement> searchElementClass, GraphDocument doc, Class<? extends Kante> edgeClass, int direction, boolean alphabetical) {
	...
	/**
	 * Liefert eine Liste aller Elemente der angegebenen Art, die über irgendeine Kantenart mit den direkten und indirekten
	 * Teilelementen dieses Elementes verbunden sind. Es wird nur in dem angegebenen <code>GraphDocument</code>
	 * gesucht:
	 * 
	 * @param searchElementClass
	 * 		Elementart nach der gesucht werden soll
	 * @param doc
	 * 		<code>GraphDocument</code>, in dem nach verbundenen Elementen gesucht wird
	 * @return
	 * 		Liste mit verbundenen <code>ModelElement</code>s
	 * @see #getPartConnectedContainer(Class, GraphDocument, Class, int)
	 */
	public final ArrayList<ElementContainer> getPartConnectedContainer(Class<? extends ModelElement> searchElementClass, GraphDocument doc) {
		return getPartConnectedContainer(searchElementClass, doc, null, Doppelkante.ANY);
	}

	/**
	 * Liefert eine Liste aller Elemente die über die übergebene Kantenart mit den direkten und indirekten
	 * Teilelementen dieses Elementes verbunden sind. Es wird nur in dem angegebenen <code>GraphDocument</code>
	 * gesucht:
	 * 
	 * @param doc
	 * 		<code>GraphDocument</code>, in dem nach verbundenen Elementen gesucht wird
	 * @param searchEdgeClass
	 * 		Kantenart nach der gesucht werden soll
	 * @return
	 * 		Liste mit verbundenen <code>ModelElement</code>s
	 * @see #getPartConnectedContainer(Class, GraphDocument, Class, int)
	 */
	public final ArrayList<ElementContainer> getPartConnectedContainer(GraphDocument doc, Class<? extends Kante> searchEdgeClass) {
		return getPartConnectedContainer(ModelElement.class, doc, searchEdgeClass, Doppelkante.ANY);
	}

	
	/**
	 * Liefert eine Liste aller Elemente, die über die angegebene Kantenart mit den direkten und indirekten
	 * Teilelementen dieses Elementes verbunden sind. Es wird nur in dem angegebenen <code>GraphDocument</code>
	 * gesucht und nur in der angegebenen Richtung der Kante.
	 * 
	 * @param searchElementClass
	 * 		Elementart nach der gesucht werden soll
	 * @param doc
	 * 		<code>GraphDocument</code>, in dem nach verbundenen Elementen gesucht wird
	 * @param edgeClass
	 * 		Art der Kante, über die Elemente mit den Teilen dieses Elementes verbunden sein sollen
	 * @param direction
	 * 		Richtung, die die Kanten haben sollen, über die die verbundenen Elemente gesucht werden
	 * @return
	 * 		Liste mit verbundenen <code>ModelElement</code>s
	 */
	public final ArrayList<ElementContainer> getPartConnectedContainer(Class<? extends ModelElement> searchElementClass, GraphDocument doc, Class<? extends Kante> edgeClass, int direction) {
		//Rückgabeliste
	...
	/**
	 * Liefert eine Liste aller Elemente der angegebenen Art, die über irgendeine Kantenart mit den direkten und indirekten
	 * Oberelementen dieses Elementes verbunden sind. Es wird nur in dem angegebenen <code>GraphDocument</code> gesucht.
	 * 
	 * @param searchElementClass
	 * 		Elementart nach der gesucht werden soll
	 * @param doc
	 ...
	 */
	public final ArrayList<ElementContainer> getParentConnectedContainer(Class<? extends ModelElement> searchElementClass, GraphDocument doc) {
		return getParentConnectedContainer(searchElementClass, doc, null, Doppelkante.ANY);
	}

	/**
	 * Liefert eine Liste aller Elemente, die über die angegebene Kantenart mit den direkten und indirekten
	 * Oberelementen dieses Elementes verbunden sind. Es wird nur in dem angegebenen <code>GraphDocument</code>
	 * gesucht und nur in der angegebenen Richtung der Kante.
	 * 
	 * @param searchElementClass
	 * 		Elementart nach der gesucht werden soll
	 ...
	 * 		Liste mit verbundenen <code>ModelElement</code>s
	 */
	public final ArrayList<ElementContainer> getParentConnectedContainer(Class<? extends ModelElement> searchElementClass, GraphDocument doc, Class<? extends Kante> edgeClass, int direction) {
		//Rückgabeliste
		ArrayList<ElementContainer> connected = new ArrayList<ElementContainer>();
	 ...
	 */
	public ArrayList<ElementContainer> getDirectPartContainer(GraphDocument doc) {
	 ...
	 */
	public ArrayList<ElementContainer> getDirectParentContainer(GraphDocument doc) {
	...
	/**
	 * Prüft, ob im übergebenen GraphDocument der Container eines direkt verbundenen
	 * Parent-Elementes von <code>this</code> existiert. Wenn ja, kommt <code>true</code> zurück.
	 * 
	 * @param doc
	 * @return
	 */
	public final boolean hasDirectParentContainer(GraphDocument doc) {
	 ...
	 * @return
	 */
	public boolean hasDirectPartContainer(GraphDocument doc) {
	...
	/**
	 * Liefert ein <code>ArrayList</code> aller <code>ElementContainer</code>, deren Elemente
	 * Teil dieses Elementes sind, aber selbst keine Teile besitzen. <br>
	 * 
	 * @return Liste mit <code>ElementContainer</code>n, die die absoluten
	 *         Kindelemente sind
	 */
	public final ArrayList<ElementContainer> getAbsolutePartContainer(GraphDocument doc) {
	 ...
	 * 
	 * @param returnList
	 * 		Liste mit <code>ElementContainer</code>n
	 * @param doc
	 * 		(Teil-)Modell in dem gesucht werden soll
	 * @param parts
	 * 		Wenn <code>true</code> wird nach allen Teilen gesucht, sonst nach allen Oberelementen
	 * @param testonly
	 * 		Wenn <code>true</code> wird beim ersten gefundenen Element abgerochen und <code>true</code> zurück gegeben
	 * @return
	 * 		<code>true</code>, wenn mind. ein Element gefunden wurde, das in die Rückgabeliste gehört
	 */
	private final boolean getPartOrParentContainer(ArrayList<ElementContainer> returnList, GraphDocument doc, boolean parts, boolean testonly) {
	...
	/**
	 * Liefert alle Eltern, Kinder und Geschwister dieses Elementes und das Element selbst. 
	 * Es werden also alle Elemente gesucht, die mit diesem Element über eine beliebigen Pfad von PartOfVerbindungen
	 * zusammenhängen. 
	 * 
	 * @return
	 * 		Liste mit <code>ModelElement</code>en
	 */
	 ...
	 * 
	 * @param doc
	 * 		<code>GraphDocument</code> in dem gesucht werden soll
	 * @return
	 * 		Liste mit <code>ElementContainer</code>n
	 */
	public final ArrayList<ElementContainer> getPartAndParentContainer(GraphDocument doc){
	 ...
	 * @return
	 */
	private final ArrayList<ElementContainer> getPartOrParentContainer(GraphDocument doc, boolean addMeAsFirst, boolean parts) {
		doc = (isUnique() ? doc.getCollection().getMainGraphDocument() : doc);
	 ...
	 */
	public final ArrayList<ElementContainer> getPartContainer(GraphDocument doc, boolean addMeAsFirst) {
	 ...
	 */
	public final ArrayList<ModelElement> getPartElements(boolean addMeAsFirst) {
		if (!addMeAsFirst)
	 ...
	 * @param result
	 */
	private final void getParentElementsRecursive(Set<ModelElement> result) {
	 ...
	 * @param result
	 */
	private final void getPartElementsRecursive(Set<ModelElement> result) {
		for (Kante edge : edges) {
			if (!(edge instanceof PartOfBeziehung))
				continue;
	 ...
	 * @return Eine <code>ArrayList</code> gefüllt mit <code>ElementContainer</code>n.
	 */
	public final ArrayList<ElementContainer> getParentContainer(GraphDocument doc, boolean addMeAsFirst) {
	 ...
	 * @return Eine <code>ArrayList</code> gefüllt mit <code>ModelElement</code>s.
	 */
	public final ArrayList<ModelElement> getParentElements(boolean addMeAsFirst) {
		if (!addMeAsFirst)
	...
	/** rekursiv über alle ist-Teil-von-Beziehungen */
	public final boolean isParentOf(ModelElement me) {
		return this.getPartElements().contains(me);
	}

	/** rekursiv über alle ist-Teil-von-Beziehungen */
	public final boolean isPartOf(ModelElement me) {
		return me.getPartElements().contains(this);
	}

	/**
	 * Liefert <code>true</code>, wenn <code>this</code> und <code>me</code> direkt über
	 * eine <code>PartOfBeziehung</code> verbunden sind und <code>this</code> ein Teil
	 * von <code>me</code> ist.
	 * @param me
	 * @return
	 */
	public final boolean isDirectPartOf(ModelElement me) {
		return me.isDirectParentOf(this);
	}

	/**
	 * Liefert <code>true</code>, wenn <code>me</code> und <code>this</code> direkt über
	 * eine <code>PartOfBeziehung</code> verbunden sind und <code>me</code> ein Teil
	 * von <code>this</code> ist.
	 * @param me
	 * @return
	 */
	public final boolean isDirectParentOf(ModelElement me) {
		for (Kante edge : edges) {
			if (!(edge instanceof PartOfBeziehung))
				continue;
	...
	Braucht man vielleicht und sollte in Analogie zu den Part-OfBeziehungen (oder irgendwie über dieselbe Funktion gemacht werden
	 
	private final boolean getCompositionMasterOrSlaveContainer(ArrayList<ElementContainer> returnList, GraphDocument doc, boolean slave, boolean testonly) {
		if ((returnList == null) || (returnList.size() == 0) || (returnList.get(0) == null))
			return false;
		doc = (isUnique() ? doc.getCollection().getGraphDocument() : doc);
		ArrayList<ElementContainer> masterOrSlaves = null;
		if (slave)
			masterOrSlaves = returnList.get(returnList.size()-1).getElement().getDirectCompositionSlaveContainer(doc);
		else
			masterOrSlaves = returnList.get(returnList.size()-1).getElement().getDirectCompositionMasterContainer(doc);
		for (int i = 0; i < masterOrSlaves.size(); i++) {
			boolean found = false;
			ElementContainer ms = masterOrSlaves.get(i);
			if (returnList.contains(ms)){
				found = true;
				if (testonly)
					return true;
				break;
...
*/	
	...
	/**
	 * Liefert eine Liste aller Masterelemente dieses Elementes, also aller Elemente, die
	 * mit diesem Element über eine {@link Composition} verbunden sind, wobei das verbundene
	 * Element diesem Element übergeordnet ist.
	 * @return
	 */
	...
	/**
	 * Liefert eine Liste aller Container der Slaveelemente dieses Elementes, also aller
	 * Elemente, die mit diesem Element über eine {@link Composition} verbunden sind, wobei
	 * das verbundene Element diesem Element übergeordnet ist.
	 * 
	 * @param doc
	 * 		{@link GraphDocument} in dem die Container liegen sollen
	 * @return
	 * 
	 */
	public final ArrayList<ElementContainer> getDirectCompositionSlaveContainer(GraphDocument doc){
		return getContainer(getDirectCompositionSlaveElements(), doc);
	}

	/**
	 * Liefert eine Liste aller Container der Masterelemente dieses Elementes, also aller
	 * Elemente, die mit diesem Element über eine {@link Composition} verbunden sind, wobei
	 * das verbundene Element diesem Element übergeordnet ist.
	 * 
	 * @param doc
	 ...
	 * 
	 */
	public final ArrayList<ElementContainer> getDirectCompositionMasterContainer(GraphDocument doc){
		return getContainer(getDirectCompositionMasterElements(), doc);
	}

	/**
	 * Liefert eine Liste mit allen Containern, die die übergebenen Elemente im übergebenen {@link GraphDocument}
	 * haben. Wenn ein Element der übergebenen Collection keinen Container im {@link GraphDocument} hat, dann
	 * wird auch kein Eintrag in der Rückgabeliste hinzugefügt. Die Rückgabeliste kann also kleiner sein, als die
	 * übergebene Liste.
	 * 
	 * @param elements
	 * @param doc
	 * @return
	 */
	public static final ArrayList<ElementContainer> getContainer(ArrayList<? extends ModelElement> elements, GraphDocument doc) {
...
*/	
	...
	/**
	 * Gibt alle mit diesem <code>ModelElement</code> verbundenen <code>ModelElement</code>s des Klasse
	 * <code>searchElementOrTraceClass</code> zurueck.
	 * 
	 * @param searchElementClass
	 * @return
	 */
	public final ArrayList<ModelElement> getConnectedElements(Class<? extends ModelElement> searchElementClass) {
	...
	/**
	 * Gibt alle mit diesem <code>ModelElement</code> verbundenen <code>ModelElement</code>s des Klasse
	 * <code>searchElementClass</code> zurueck.
	 * 
	 * @param searchElementClass
	 * @param edgeClass
	 * @return
	 */
	public final ArrayList<ModelElement> getConnectedElements(Class<? extends ModelElement> searchElementClass, Class<? extends Kante> edgeClass) {
	 ...
	 * @return
	 */
	public final ArrayList<ModelElement> getConnectedElements(Class<? extends ModelElement> searchElementClass, Class<? extends Kante> edgeClass, int direction) {
	 ...
	 */
	public final ArrayList<ModelElement> getConnectedElements(Class<? extends ModelElement> searchElementClass, Class<? extends Kante> edgeClass, int direction, boolean alphabetical) {
		return getConnectedElements(searchElementClass, null, edgeClass, direction, alphabetical);	
	}

	/**
	 * Gibt alle mit diesem <code>ModelElement</code> verbundenen <code>ModelElement</code>s der Klasse
	 * <code>searchElementClass</code> zurueck.
	 * 
	 * @param searchElementClass
	 * @param doc
	 * 		<code>GraphDocument</code> in dem verbundene Elemente gesucht werden sollen. Wird <code>null</code>
	 * 		übergeben, werden alle verbundenen Elemente zurück gegeben, was der Suche im Hauptmodell entspricht.
	 * @param edgeClass
	 * @param direction
	 * @param alphabetical
	 ...
	 */
    @SuppressWarnings("unchecked")
    public final ArrayList<ModelElement> getConnectedElements(Class<? extends ModelElement> searchElementClass, GraphDocument doc, Class<? extends Kante> edgeClass, int direction, boolean alphabetical) {
	...
	/**
	 * Gibt alle mit diesem <code>ModelElement</code> verbundenen <code>ModelElement</code>s der Klasse
	 * <code>searchElementClass</code> zurueck oder deren <code>ElementContainer</code>.
	 * 
	 * @param searchElementClass
	 * @param doc
	 * 		<code>GraphDocument</code> in dem verbundene Elemente gesucht werden sollen. Wird <code>null</code>
	 * 		übergeben, werden alle verbundenen Elemente zurück gegeben, was der Suche im Hauptmodell entspricht.
	 * 		Will man aber <code>ElementContainer</code> aus dem Hauptmodell haben, muss man ein gültiges Haupt-
	 * 		<code>GraphDocument</code> übergeben. 
	 * @param edgeClass
	 * @param direction
	 * @param container
	 * 		wenn <code>true</code>, werden die <code>ElementContainer</code> der gefundenen Elemente zurück
	 * 		gegeben; sonst die Elemente selbst
	 * @param alphabetical
	 * @return ArrayList mit allen verbundenen <code>ModelElement</code>s oder <code>ElementContainer</code>n
	 */
	private final ArrayList<?> getConnected(Class<? extends ModelElement> searchElementClass, GraphDocument doc, Class<? extends Kante> edgeClass, int direction, boolean container, boolean alphabetical) {
	...
	/**
	 * Gibt alle <code>ModelElement</code>s zurück, die mit diesem <code>ModelElement</code>
	 * im angegebenen <code>GraphDocument</code> verbunden sind.
	 * 
	 * @param edgeClass
	 *            alle Kanten des Types
	 * @return ArrayList mit den verbundenen ModelElementen
	 */
	public ArrayList<ModelElement> getConnectedElementsByEdge(Class<? extends Kante> edgeClass) {
	 ...
	 * @return ArrayList mit den verbundenen ModelElementen
	 */
	public ArrayList<ModelElement> getConnectedElementsByEdge(Class<? extends ModelElement> targetElementClass, Class<? extends Kante> edgeClass) {
	...
	/**
	 * Liefert einen Eigenschafts-Dialog für dieses Element. Wenn bereits einer existiert,
	 * wird dieser zurück gegeben, sonst wird ein neuer Dialog angelegt. Der Dialog wird
	 * sofort angezeigt oder wenn er bereits angezeigt wird in den Vordergrund gebracht.  
	 * 
	 * @param gdcoll
	 * 		GDCollection, in der sich das Element befinden sollte
	 * @return
	 */
	public ElementPropertyDialog getPropertyDialog(GDCollection gdcoll) {
	 ...
	 * Die  übergebene Klasse muss gleich der zurückzugebenen Kantenklassen oder eine
	 * Oberklasse davon sein.
	 * 
	 * @param edgeClass
	 * 		Klasse der zu suchenden Kanten
	 * @return
	 */
	public final ArrayList<Kante> getEdges(Class<? extends Kante> edgeClass) {
	 ...
	 */
	public final int countConnections(Class<? extends Kante> edgeClass) {
	 ...
	 * 		Type of edges to count
	 * @param direction
	 * 		Direction of the edge in relation to this
	 * @return
	 * 		Number of edges with the specified type
	 */
	private final int countConnections(Class<? extends Kante> edgeClass, int direction) {
		int retVal = 0;
		for (Kante edge : edges)
			if (edgeClass.isAssignableFrom(edge.getClass())) {
				if (direction == Doppelkante.FORWARD) {
	 ...
	 * 		Number of edges with the specified type
	 */
	public final int countConnectionsFromThis(Class<? extends Kante> edgeClass) {
	 ...
	 * 		Number of edges with the specified type
	 */
	public final int countConnectionsToThis(Class<? extends Kante> edgeClass) {
	...
	/**
	 * Reicht die selbe Methode die von <code>Tool3lgmConstants</code> einfach
	 * nur durch.
	 * 
	 * @param key
	 * @return
	...
	/**
	 * Ueberprueft, ob der Knoten zur Menge der UNIQUE_KNOTS gehoert, ob er also
	 * ein Knoten ist, der im gesamten Modell nur einmal vorkommt
	 */
	...
	/**
	 * @return
	 * 		<code>true</code>, wenn die Elementart eine PartOfBeziehung hat
	 */
     ...
     * @return
     */
    public final boolean isForwardLinkable(ModelElement me, Class<? extends Kante> edgeClass, boolean testCardinality) {
	...
	/**
	 * join Element properties without connections this.hashstring = other.hashstring
	 * other will be not changed
	 */
	public boolean join(ModelElement other, boolean overwriteHashstringAndExtIDs) {
		if (other.getClass() != this.getClass() || this==other)
			return false;

		if (overwriteHashstringAndExtIDs)
			this.hashstring = other.getHashString();

		String joined = Tool3lgmConstants.getResString("joined");
		if (!this.name.trim().equals(other.name.trim()))
			this.name = this.name.concat("\n-" + joined + "-\n" + other.name);
		//es gibt keine Beschreibung bei this
		if (this.descr.trim().equals("")) {
		...
		//es gibt eine Beschreibung bei this
			...
			if (!other.descr.trim().equals("") || !this.descr.trim().equals(other.descr.trim()))
				this.descr = this.descr.concat("\n-" + joined + "-\n" + other.descr);
		}
		this.htmlName = HTMLConverter.getHTMLString(name).replaceAll("&#10;", "<BR>").replaceAll("\\\\&#45;", "-<BR>");
		this.refreshText();
	 ...
	 * @param document
	 */
	public void setAssociatedDoc(String document) {
	 ...
	 * @return
	 */
	public String setExternalID(String key, String value) {
		if (externalIDs == null)
	 ...
	 * @return
	 */
	public boolean hasExternalID(Object key, String extID) {
		if (externalIDs == null)
			return false;
