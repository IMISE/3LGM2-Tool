...
public abstract class Kante extends ModelElement {
	...
	/**
	 * Name des <code>Class&lt;? extends ModelElement&gt;[]</code>-Arrays, mit dem jede
	 * Kantenklasse alle seine Startklassen beschreibt. Über diesen Namen werden die
	 * Startklassen per Reflection ermittelt.
	 */
	public static final String START_CLASS_FIELD_NAME = "stcl";
	
	/**
	 * Name des <code>Class&lt;? extends ModelElement&gt;[]</code>-Arrays, mit dem jede
	 * Kantenklasse alle seine Endklassen beschreibt. Über diesen Namen werden die
	 * Endklassen per Reflection ermittelt.
	 */
	public static final String END_CLASS_FIELD_NAME = "etcl";
	
	/**
	 * Name des <code>int[]</code>-Arrays, mit dem jede Kantenklasse die Kardinalitäten
	 * beschreibt, mit denen Elemente der Startklasse für Elemente der Endklasse vorhanden
	 * sein müssen.<br />
	 * Über diesen Namen werden die Kardinalitäten per Reflection ermittelt. Das Array sollte
	 * immer nur 2 int-Werte für Minimum und Maximum enthalten.
	 */
	public static final String START_CARDINALITY_FIELD_NAME = "scard";
	
	/**
	 * Name des <code>int[]</code>-Arrays, mit dem jede Kantenklasse die Kardinalitäten
	 * beschreibt, mit denen Elemente der Endklasse für Elemente der Startklasse vorhanden
	 * sein müssen.<br />
	 * Über diesen Namen werden die Kardinalitäten per Reflection ermittelt. Das Array sollte
	 * immer nur 2 int-Werte für Minimum und Maximum enthalten.
	 */
	...
	/**
	 * Auch für Kanten muss angegeben, welche Elementarten sie verbinden können, damit die Vererbung
	 * bei der Definition der MetaPfade funktioniert, die getStartElementClass() und getEndElementClass()
	 * aufruft.
	 */
	...
	/**
	 * The two elements wchich are connected by this edge. <code>k1</code> is the start element
	 * and <code>k2</code> the end element of this edge.
	 */
	 ...
	 * @param knot2
	 */
	public Kante(ModelElement knot1, ModelElement knot2) {
	 ...
	 */
	public Kante(ModelElement knot1, ModelElement knot2, boolean registerInKnots) {
		super();
	...
	/**
	 * liefert true, wenn beide Knoten, die die Kante verbindet identisch sind und die Kante von derselben Art ist
	 * (Richtung ist egal)
	 * 
	 * @param kante
	 * @return
	 */
	public boolean isEqualTo(Kante kante) {
...
*/	}
	 ...
	 * @param _k2
	 */
	public void setKnots(ModelElement _k1, ModelElement _k2) {
...
*/		
	...
	/**
	 * Setzt fuer die Kante die Knoten Anfang=_k1 und Ende=_k2 und fügt die
	 * Kante bei _k1 an Position _k1EdgePos und bei _k2 an Position _k2EdgePos ein.
	 * 
	 * @param _k1
	 *            Knoten
	 * @param pos
	...
	/**
	 * @param _k1
	 *          Startelement der Kante 
	 * @param _k1EdgePos
	 * 			Postion der Kante in der Kantenliste von _k1. Wenn der Wert größer oder kleiner als die aktuelle 
	 * 			Liste ist, dann wird die Kante hinten angefügt.
	 * @param _k2
	 * 			Endelement der Kante 
	 * @param _k2EdgePos
	 * 			Postion der Kante in der Kantenliste von _k2. Wenn der Wert größer oder kleiner als die aktuelle 
	 * 			Liste ist, dann wird die Kante hinten angefügt.
	 */
	public void setKnotsAndInsert(ModelElement _k1, int _k1EdgePos, ModelElement _k2, int _k2EdgePos) {
		k1 = _k1;
		k2 = _k2;
	 ...
	 * @param start
	 */
	public void setStartAndInsert(ModelElement start) {
		k1 = start;
		if (k1 != null)
			k1.addEdge(this);
	}
	 ...
	 * @param end
	 */
	public void setEndAndInsert(ModelElement end) {
		k2 = end;
		if (k2 != null)
			k2.addEdge(this);
	}
	 ...
	 * @return
	 */
	public boolean isStart(ModelElement me){
		return getStart() == me;
	}
	 ...
	 * @return
	 */
	public boolean isEnd(ModelElement me){
		return getEnd() == me;
	}
	
	/**
	 * Wenn das übergebene Element durch diese Kante mit einem anderen Element verbunden ist, kommt das andere Element
	 * der Kante zurück, sons <code>null</code>.
	 * 
	 * @param me
	 * @return
	 */
	public ModelElement getOther(ModelElement me){
		if (isStart(me))
			return getEnd();
		if (isEnd(me))
			return getStart();
	...
	/**
	 * Wenn die übergebene Elementklasse durch eine Kante der angegebenen Art mit anderen Elementen
	 * verbunden sein kann, dann wird die Elementklasse dieser anderen Elemente zurück gegeben.
	 * Passen Kante und Elementklasse nicht zusammen, kommt <code>null</code> zurück.
	 * 
	 * @param edgeClass
	 * 		Kantanklasse, von der die andere verbundene Elementklasse zurück gegeben werden soll
	 * @param meClass
	 * 		Elementklasse der Kante, deren Gegenelementklasse zurück gegeben werden soll 
	 * @return
	 * 		die andere Elementklasse der Kante, als die übergebene Klasse oder <code>null</code>, wenn
	 * 		die Klasse gar nicht passt
	 */
	public static Class<? extends ModelElement> getOther(Class<? extends Kante> edgeClass, Class<? extends ModelElement> meClass){
	 ...
	 * @return
	 */
	public final boolean decodeHashStrings(GraphDocument doc) {
	 ...
	 * @return
	 */
	public boolean isConnecting(ModelElement _k1, ModelElement _k2) {
	...
	/**
	 * Prüft die Validität der Kanten und stellt sie wenn möglich her. Die Prüfung betrifft
	 * die Art der Kantenelemente
	 * 
	 * @return
	 * 		<code>true</code>, wenn die Kante vollständig richtig ist
	 */
	 ...
	 * @return
	 */
	private static final Class<? extends ModelElement> getStartEndClass(Class<? extends Kante> edgeClass, boolean start) {
	 ...
	 * @return
	 */
	protected static final int[] getStartEndCardinality(Class<? extends Kante> edgeClass, boolean start) {
...
//		System.err.println(getClass().getSimpleName() + ": " +fieldName + " (" + elementClass.getSimpleName()+")");
     ...
     * @return
     */
    public static final Class<? extends ModelElement> getStartClass(Class<? extends Kante> edgeClass) {
	 ...
	 * @return
	 */
	public final static Class<? extends ModelElement> getEndClass(Class<? extends Kante> edgeClass) {
	...
	/**
	 * Liefert <code>true</code>, wenn die übergebene Klasse die Startklasse der Kante oder eine Ober- oder Unterklasse
	 * davon ist.
	 * 
	 * @param edgeClass
	 * @param elementClass
	 * @return
	 */
	public static final boolean isStartClass(Class<? extends Kante> edgeClass, Class<? extends ModelElement> elementClass){
	 ...
	 * @return
	 */
	public final boolean isStartClass(Class<? extends ModelElement> elementClass){
		return isStartClass(getClass(), elementClass);
	}

	/**
	 * Liefert <code>true</code>, wenn die übergebene Klasse die Endklasse der Kante oder eine Ober- oder Unterklasse
	 * davon ist.
	 * 
	 * @param edgeClass
	 * @param elementClass
	 * @return
	 */
	public static final boolean isEndClass(Class<? extends Kante> edgeClass, Class<? extends ModelElement> elementClass){
	 ...
	 * @return
	 */
	public final boolean isEndClass(Class<? extends ModelElement> elementClass){
		return isEndClass(getClass(), elementClass);
	}

	/**
	 * Liefert <code>true</code>, wenn die übergebene Klasse die Start- oder Endklasse der Kante oder eine 
	 * Ober- oder Unterklasse davon ist.
	 * 
	 * @param edgeClass
	 * @param elementClass
	 * @return
	 */
	public static final boolean isStartOrEndClass(Class<? extends Kante> edgeClass, Class<? extends ModelElement> elementClass){
	 ...
	 * @return
	 */
	public static final boolean isConnecting(Class<? extends Kante> edgeClass, Class<? extends ModelElement> elementClass1, Class<? extends ModelElement> elementClass2) {
	...
	/**
	 * Liefert <code>true</code>, wenn die übergebene Kantenklasse Elemente der angegebenen Arten in Vorwärtsrichtung miteinander 
	 * verbindet. Also <code>startElementClass</code> die Startklasse der Kantenklasse oder eine Unterklasse davon ist und
	 ...
	 * @return
	 */
	public static final boolean isConnectingForward(Class<? extends Kante> edgeClass, Class<? extends ModelElement> startElementClass, Class<? extends ModelElement> endElementClass) {
	 ...
	 * @return
	 */
	public static final int getMinCardinality(Class<? extends ModelElement> elementClass, Class<? extends Kante> edgeClass){
	 ...
	 * @return
	 */
	public static final int getMaxCardinality(Class<? extends ModelElement> elementClass, Class<? extends Kante> edgeClass){
		if (Kante.isStartClass(edgeClass, elementClass))
	 ...
	 * @return
	 */
	public static final int getMinStartToEndCardinality(Class<? extends Kante> edgeClass){
	 ...
	 * @return
	 */
	public static final int getMaxStartToEndCardinality(Class<? extends Kante> edgeClass){
	 ...
	 * @return
	 */
	public static final int getMinEndToStartCardinality(Class<? extends Kante> edgeClass){
	 ...
	 * @return
	 */
	public static final int getMaxEndToStartCardinality(Class<? extends Kante> edgeClass){
	 ...
	 * @return
	 */
	public boolean reconnect(GDCollection coll) {
		if (k1 == null || k2 == null)
			return false;
		...
		if (k1 == null || k2 == null || coll.getMainGraphDocument().findElementCoded(this.getHashString()) == null)
			return false;
	...
	/* (non-Javadoc)
	 * @see tool3lgm.graphtools.elements.ModelElement#isUnique()
	 */
	@Override
	...
	/* (non-Javadoc)
	 * @see tool3lgm.graphtools.elements.ModelElement#isUnpaintable()
	 */
	@Override
