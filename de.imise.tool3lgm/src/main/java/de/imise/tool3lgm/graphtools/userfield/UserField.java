...
public class UserField implements Cloneable, Comparator<ModelElement> {
	...
	public static enum Style {
		SEPARATOR {
			@Override
            int compare(UserField uf, ModelElement me1, ModelElement me2) {
	            return 0;
            }
		},
		SINGLE_LINE {
...
*/		
		...
		/**
		 * Vergleicht die beiden UserFields hinsichtlich ihres Wertes bezüglich des
		 * Modelelements.<p>
		 * 
		...
		private static int alphabeticalCompare(UserField uf, ModelElement me1, ModelElement me2) {
			String v1 = uf.getValue(me1);
			String v2 = uf.getValue(me2);
			if (v1 == null)
				return v2 == null ? 0 : -1;
		...
		private static int numberCompare(UserField uf, ModelElement me1, ModelElement me2) {
			String v1 = uf.getValue(me1);
			String v2 = uf.getValue(me2);
			if (v1 == null)
				return v2 == null ? 0 : -1;
...
//	public static enum FUNC {SUM, MULT, TWSUM, AVG, INDI, REF, MIN, MAX};
	...
	/**
	 * Ein konstant leerer <code>String</code>. <code>UserFieldTarget</code>s, die keinen
	 * Wert für dieses <code>UserField</code> liefern können, geben diesen <code>String</code> zurück.
	 */
	...
	/**
	 * Dieses Set beinhaltet Strings, die Formel-UserFields als Werte annehmen, wenn sie sich
	 * nicht berechnen lassen oder irgendwelche anderen Probleme auftreten. <code>FORMAT_ERROR</code>
	 * und <code>EMPTY_STRING</code> können auch bei Kennzahl-UserFields auftreten. 
	 * beinhaltet:
	 */
	...
	/**
	 * Dieses Set enthält alle Strings, die Werte von UserFields sein können, die bei Berechnungen
	 * ignoriert werden sollen. D.h. wenn eine Summen-/Differenz/Divisions-/Multiplikationsfunktion
	 * auf ein UserField mit einem <code>String</code> aus diesem Set als value trifft, wird dafür 
	...
	/**
	 * Dieses Set beinhaltet Strings, die interne Verrechnungsfunktionen kennzeichen 
	 * beinhaltet:
	 */
	...
	/**
	 * Dieser String wird im Format den Nachkommastellen vorangestellt, damit man ihn
	 * von anderen Formatinformationen unterscheiden kann.
	 */
	private static final String FORMAT_DECIMAL_PLACES_PREFIX = "d";
	
	/**
	 * Dieser String wird im Format der Eiheit vorangestellt, damit man ihn
	 * von anderen Formatinformationen unterscheiden kann.
	 */
	...
	/**
	 * Ist der <code>String</code>, durch den das <code>UserField</code> eindeutig
	...
	/**
	 * Gibt an, von welcher Form das <code>userField</code> sein soll.
	 * /
	private int style = -1;

	/**
	 * Art dieses {@link UserField}s. Immer ein Wert aus {@link Style}
	 */
	...
	/**
	 * Beinhaltet alle Einträge, die in UserFields mit den Styles ComboBox, RadioButton und
	 * CheckBox auftauchen sollen.
	 */
	...
	/**
	 * Die Definition in der sich dieses UserField befindet. Wird gebraucht, um z.B.
	 * das Format-<code>UserField</code> zu finden.
	 */
	private UserFieldDefinitions definitions;
	
	/**
	 * UserField mit dem Style <code>FORMAT_STYLE</code>, das vorgibt, wie der Zahlenwert dieses
	 * UserFields formatiert werden soll. 
	 */
	 ...
	 * @param style
	 */
	public UserField(Style style, UserFieldDefinitions definitions) {
	 ...
	 * @param style
	 */
	public UserField(Class<? extends UserFieldTarget> targetClass, Style style, UserFieldDefinitions definitions) {
	 ...
	 * @param targetClass
	 */
	public UserField(Class<? extends UserFieldTarget> targetClass, UserFieldDefinitions definitions) {
	 ...
	 * @param hashCode
	 */
	public UserField(Class<? extends UserFieldTarget> targetClass, String hashCode, UserFieldDefinitions definitions) {
	 ...
	 * @param definitions
	 */
	public UserField(String hashCode, UserFieldDefinitions definitions) {
	 ...
	 * @param fieldName 
	 * 		Der Name der zu belegenden Variable des userFieldes
	 * @param value
	 * 		Der Wert, mit der die Variable belegt werden soll. 
	 */
	public boolean putXMLFieldString(String fieldName, String value) {
	...
	/**
	 * Gibt den aktuellen <code>style</code> des <code>UserField</code> s
	 * zurück
	 * 
	 ...
	 * @param style
	 * @return
	 * 		<code>true</code>, wenn der übergebene {@link Style} der selbe ist, wie der dieses UserFields
	 */
	...
	/**
	 * @return
	 * 		<code>true</code>, wenn das UserField nicht für ein Element definiert ist sondern global fürs Modell oder ein Format ist
	 */
	 ...
	 * @return
	 */
	public static boolean isClassificationStyle(Style style){
	 ...
	 * @return
	 */
	public static boolean isListValueStyle(Style style){
	...
	/**
	 * Prüft, ob dieses {@link UserField} ein UserField ist, das einen Wert in sich tragen kann (also kein Format-, kein Separator- und kein Panel-USerfield ist).
	 * @param userField
	 * @return
	 */
	 ...
	 * @param formulaString
	 * @return boolean
	 * 		true: Wenn es sich bei dem <code>userField</code> um eine sekundäre Kennzahl handelt, 
	 * 		d.h. eine Kennzahl, die Kennzahlformeln repräsentiert - ansonsten false
	 */
	public boolean setFormula(String formulaString){
	 ...
	 * @param definitions
	 */
	public void setDefinitions(UserFieldDefinitions definitions) {
		this.definitions = definitions;
	}
	 ...
	 * @return
	 */
	public void setFormatUserField(UserField formatUserField) {
	...
	/**
	 * Liefert den <code>String</code>, der in Modeldatein oder in der Export von <code>UserField</code>s
	 * geschrieben wird.
	 * @return
	 */
	private String getFormatExportString(){
		...
		if (this.numberFormat!=null){
			sb.append(this.numberFormat.getMinimumFractionDigits());
			sb.append(" ");
		}
		sb.append(FORMAT_UNIT_PREFIX);
		if (this.formatUnit!=null){
			sb.append(this.formatUnit);
		}
		return sb.toString();
	}
	...
	/**
	 * Setzt die Nachkommastellenanzahl und die Einheit des Formates.
	 * 
	 * Es wird ein String der Form "d### uXXX" erwartet, wobei d für den String {@link #FORMAT_DECIMAL_PLACES_PREFIX}
	 * und u für {@link #FORMAT_UNIT_PREFIX} steht. "###" steht für eine Zahl und "XXX" für eine beliebige Zeichenkette.
	 * Vor dem 'u' können beliebig viele Whitespaces (auch keins) stehen. 
	 * 
	 * @param formatStringWithPrefixes
	 */
	private void setFormatFractionDigitsCountAndUnit(String formatStringWithPrefixes) {
	...
	/**
	 * Liefert das <code>NumberFormat</code> des <code>UserField</code>s. Ist das <code>UserField</code>
	 * selbst ein Format, gibt es seine eigenen Nachkommastellen zurück, ist es ein UserField, dem ein 
	 * Format zugewiesen ist (was für ein Format selbst nie zutreffen kann), dann gibt es die Nachkommstellen
	 * des Formates zurück.
	 * 
	 * @param definitions
	 * 		die <code>UserFieldDefinitions</code>, in denen das Format-<code>UserField</code> dieses
	 * 		<code>UserField</code>s defniert ist oder <code>null</code>, wenn man diese Information
	 * 		direkt für ein Format-<code>UserField</code> abfragen möchte.
	...
	/**
	 * Liefert die Anzahl der Nachkommastellen. Ist das UserField selbst ein Format, gibt es seine
	 * eigenen Nachkommastellen zurück, ist es ein UserField, dem ein Format zugewiesen ist (was für
	 * ein Format selbst nie zutreffen kann), dann gibt es die Nachkommstellen des Formates zurück.
	 * 
	 * @return die Anzahl der Nachkommastellen des Formates. Wenn kein Format eingestellt ist, kommt -1 zurück;
	 */
	 ...
	 */
	public void setFormatFractionDigits(int fractionDigits){
	 ...
	 * @param unitString
	 */		
	public void setFormatUnit(String unitString){
		if (style != Style.FORMAT)
			return;
		if (unitString.trim().equals(""))
			this.formatUnit = null;
		else
			this.formatUnit = unitString;
	}

	/**
	 * Gibt die Einheit des <code>UserField</code>s zurück.
	 * Ist das UserField selbst ein Format, gibt es seine eigene Einheit zurück,
	 * ist es ein UserField, dem ein Format zugewiesen ist (was für ein Format
	 * selbst nie zutreffen kann), dann gibt es die Nachkommstellen des Formates 
	 * zurück.
...
//	    assert false : "hashCode not designed";
	 ...
	 *
	 * @param b
	 * 		<code>true</code>  --> nur noch postive Werte erlaubt
	 * 		<code>false</code> --> positive und negative Werte erlaubt
	 */
	public void setPositiveOnly(boolean b) {
		this.positiveOnly = b;
	}

	/**
	 * @param treeVisibility
	 *            The treeVisibility to set.
	 */
	 ...
	 * 
	 * @param target
	 *            Kennzahlwertträger, für das der Wert zurück gegeben
	 *            werden soll
	 * @return
	 */
	public String getValue(UserFieldTarget target) {
	 ...
	 * 
	 * @param me
	 *          Modellelement, für das der formatierte Wert zurück gegeben
	 *          werden soll
	 * @return
	 */
	public String getFormatedValue(ModelElement me) {
	 ...
	 *          werden soll
	 * @param appendUnit
	 * 			wenn <code>true</code> wird auch die Einheit an den Rückgabewert angehängt
	 * @return
	 */
	public String getFormatedValue(ModelElement me, boolean appendUnit) {
	 ...
	 * @return
	 */
	public String getFormatedValue(Object o, boolean appendUnit) {
	...
	/**
	 * Vergleicht die beiden spezifizierten Modelelemente hinsichtlich ihrer Werte für dieses 
	 * UserField.<p>
	 * 
	 * Bedingungen für die Vergleichbarkeit sind: 
	 * 	<li> Beide Modelemente sind Instanzen der selben Klasse</li>
	 * 	<li> Das UserField ist für diese Klasse definiert</li>
	 * <br><br>
	 * 
	 * @param o1
	 * @param o2
	...
	/**
	 * Führt, wenn das übergebene <code>Object</code> ungleich <code>null</code> ist die
	 * <code>toString()</code>-Methode aus und ersetzt in diesem String alle Punkte gegen
	 * Kommas, wenn der DecimalSeparator ein Komma ist und umgekehrt.
	 * 
	 * @param string
	 */
	public String replaceWrongDecimalSeparator(Object objectToString) {
	...
	/**
	 * Ersetzt alle in <code>objectToString.toString()</code> enthaltenen Kommas und Punkte
	 * durch <code>decimalSeparator</code>.
	 * Mit <code>replaceAll = true</code> wird nur das letzte Treenzeichen ersetzt
	 * @param objectToString
	 * @param decimalSeparator
	 * @return
	 */
	public String replaceWrongDecimalSeparator(Object objectToString, char decimalSeparator){
	...
	/**
	 * Prüft, ob <code>this</code> das übergebenene <code>UserField</code> <code>other</code>
	 * benutzt. Möglich ist als Format oder innerhalb einer Formel.
	 */
	public boolean uses(UserField possibleUsedField){
	 ...
	 * @return
	 */
	public boolean isUserFieldFor(Class<? extends ModelElement> elementClass) {
	 ...
	 * @return
	 * 		true, wenn sich der übergebene Wert in der <code>errorMap</code> befindet ansonsten false
	 */
	 ...
	 * 		true, wenn sich der übergebene Wert in der <code>errorMap</code> befindet ansonsten false
	 */
	public static boolean isIgnoreableErrorString(String value){
