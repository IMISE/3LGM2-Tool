 ...
 * wird ein <code>JPanel</code> eingefügt. In dem <code>JPanel</code> werden
 * die Eingabemöglichkeiten für die UserField dargestellt. Die grafischen
 * Elemente werden durch <code>JComponent </code>, egal ob <code>JLabel</code>,
 * <code>JTextfield</code> oder <code>JComboBox</code> repräsentiert. Es
 * wird eine <code>ArrayList</code> angelegt, die Objekte vom Typ
 * <code>AttributeComponent</code> enthält. Eine
 * <code>AttributeComponent</code> enthält als Objektattribute ein Attribut
 * einer Element- bzw. Kantenklasse und die zugehörige GUI-Komponente.
 * 
 * Für die Datenübernahme ist die Methode <code>commit()</code> zuständig.
 * Sie prüft iterativ die Eingaben in GUI-komponenten in der
 * <code>ArrayList</code> und vergleicht Sie mit den bisherigen Werten der
 * UserField. Bei einem Unterschied wird das Tool3lgm-Kommando
 * <code>SET_USER_FIELD_VALUE</code> aufgerufen und somit der Wert eines
 * Attributes einer Element- bzw. Kanetenklasse geändert.
 * 
...
public class PropertyDialogUserFieldPanel extends ElementDialogPanel {
	...
	private ArrayList<AttributeComponent> fields = new ArrayList<AttributeComponent>();

	/**
	 * @param pd
	 */
	public PropertyDialogUserFieldPanel(ElementPropertyDialog pd) {
		super(pd);
		create();
...
//				String tmp_str = field.getFormatedValue(modelElement, true);
	...
	/* (non-Javadoc)
	 * @see tool3lgm.graphtools.dialog.panel.ElementDialogPanel#commit()
	 */
	@Override
	public void commit() {
	...
	private class AttributeComponent {
		...
		private UserField attribute;

		AttributeComponent(UserField attribute, JComponent component) {
			comp = component;
