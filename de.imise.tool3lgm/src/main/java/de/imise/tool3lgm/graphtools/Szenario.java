...
public class Szenario extends LGMGraphDocument {
	 ...
	 * @param title
	 */
	public Szenario(GDCollection _gdcoll, String title, String description, String hashString) {
		super(_gdcoll);
	 ...
	 * @return
	 */
	public ElementContainer addContainerCopy(ElementContainer ec) {
	 ...
	 * @param sourceDoc
	 */
	private final void updateSlaveContainers(Composition com, GraphDocument sourceDoc) {
...
*/		
	 ...
	 * @return
	 */
	public boolean endsAreMine(Kante t) {
		return ((isMyElement(t.getStart())) && (isMyElement(t.getEnd())));
	}

	/* (non-Javadoc)
	 * @see tool3lgm.graphtools.GraphDocument#isMyElement(tool3lgm.graphtools.elements.ModelElement)
	 */
	@Override
	...
	public String toXMLString() {
		...
		if (getFrame() != null) {
			try {
				xmlString.append("<view><selected>" +
						getFrame().isSelected() + "</selected><x>" +
						getFrame().getScrollPane().getViewport().getViewPosition().x + "</x><y>" +
						getFrame().getScrollPane().getViewport().getViewPosition().y + "</y><zoom>" +
						getFrame().getInputGraphArea().getZoomFactor() + "</zoom><degree>" +
						getFrame().getInputGraphArea().getMultiViewDegree() + "</degree><shift>" + 
						getFrame().getInputGraphArea().getMultiViewPitchShift() + "</shift><pageSizeFactor>" +
						getPageSizeFactor() + "</pageSizeFactor><activeLayer>" +
						gdcoll.getActiveLayer() + "</activeLayer><multiView>" + 
						getFrame().getInputGraphArea().isMultiViewEnabled() + "</multiView></view>"
					);
			} catch (Exception ex) {
	 ...
	 * @param _view
	 */
	public void setViewParameter(ViewParameter _view) {
		viewParam = _view;
