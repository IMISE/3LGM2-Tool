...
/**
 * AXS: 25.06.2020: Nichts hiervon wird genutzt! Das kann man aber evtl.
 * gebrauchen, wenn man das Draggen umsetzt, um die draggbaren Elemente zu
 * definieren und was mitkommen soll, wenn man sie draggt. Ansonsten wird das
 * hier angedachte (insbesondere die CopyDependencies) bereits über die ganz
 * normalen CopyDependencies des Metamodells erledigt. Auf der rekursive
 * Auschluss in beide Richtungen erfolgt in resolveCopyDependencies() beim
 * Kopieren von einem Modell (z.B. einem Template) zu einem anderen (z.B. zu
 * einem Zielmodell. <br>
 * Defines the elements and their relations which can be applied from a template
 * to a regular model.
 *
...
public class TemplateUsageDefinition {
    ...
    /**
     * If <code>true</code> every element of a template that can exist without
     * other elements can be applied from a template to a model. If
     * <code>false</code> only elements which are stated in
     * {@link #copyableElements} can be applied. Elements which need other
     * elements to exist means that these elements need at least one edge to
     * another element with a mininmum cardinality of 1 or greater. Default is
     * <code>true</code>.
     */
    public boolean allowApplyForAllElements = true;

    /**
     * All element types which can be applied direct from a template to model.
     * This is only relevant if {@link #allowCopyForAllElements} is
     * <code>false</code>.
     */
    private final Set<Class<? extends ModelElement>> appliableElements = new HashSet<>();

    /**
     * Maps from a element type to the edge type which must be copied too an
     * element of the given type will be apllied from the template to a model.
     */
    ...
    /**
     * Initialize the defintion. Should be overwritten in subclasses. Default
