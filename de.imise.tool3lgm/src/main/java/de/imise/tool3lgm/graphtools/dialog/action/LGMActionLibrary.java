 ...
 * <code>LGMAction</code> s bereit.
 * 
 * Panels erzeugen und verwenden diese Actions um Funktionen wie etwa das
...
public class LGMActionLibrary {
	 ...
	 * !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	 * 
	 * zu getAddElementAction(...) und getRemoveElementAction(...):
	 * 
	 * Bei Panels mit mehr als zwei Trees, kann es dazu kommen, dass Elemente
	 * falsch verschoben werden. Um dieses Problem zu beheben, sollte einfach
	 * der Wert von switchTree geändert werden.
	 * 
	 * !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	 */
	 ...
	 * von Elementen aus dem <code>srcTree</code> in den
	 * <code>targetTree</code> realisiert.
	 * 
	 * Diese <code>LGMAction</code> sollte an die "addButtons" der Panels
	 * angefügt werden.
	 * 
	 * @param srcTree
	 * @param targetTree
	 ...
	 */
	public static final LGMAction getAddElementAction(JTree srcTree, JTree targetTree, final ElementDialogPanel edp, boolean switchTree) throws ActionNotDefinedForClassException {
			...
			if (edp instanceof StructurePanel || edp instanceof NConnectionPanel || edp instanceof ETNTPanel2 || edp instanceof ETNTPanel || edp instanceof OTAPanel || edp instanceof KomPanel) {
				...
				return new LGMAction("", Tool3lgmConstants.getIcon("arrow_left2.gif")) {
					...
					public void execute(final EventObject eo) {
						...
						// Falls der hinzuzufügende awb auf einen bereits hinzugefügten awb "gedroped" wird,
						...
						if (leftC.getElement() instanceof Anwendungsbaustein || leftC.getKnoten() instanceof PhysischerDVBaustein) {
							...
							//this.execute(eo);
							this.actionPerformed(new ActionEvent(eo.getSource(), 0, ""));
						}
	 ...
	 * <code>targetTree</code> realisiert.
	 * 
	 * Diese <code>LGMAction</code> sollte an die "removeButtons" der Panels
	 * angefügt werden.
	 * 
	 * @param srcTree
	 * @param targetTree
	 ...
	 * @throws ActionNotDefinedForClassException
	 */
	public static final LGMAction getDisconnectAction(JTree srcTree, JTree targetTree, ElementDialogPanel edp, boolean switchTree) throws ActionNotDefinedForClassException {
			...
			return new LGMAction("", Tool3lgmConstants.getIcon("arrow_right2.gif")) {
				...
				public void execute(final EventObject eo) {
					...
					for (int i = 0; i < path.length; i++) {
						...
						// ist nur ein Baustein ausgewählt, bleibt die Konfiguration bestehen (außer es ist der letzte Baustein) 
	 ...
	 * Mouse-Aktionen in Trees reagiert.
	 * 
	 * @param jTree
	 * @param edp
	 * @return
	 */
	public static final LGMAction getMouseAction(JTree jTree, ElementDialogPanel edp) {
	 ...
	 * @return
	 */
	public static final LGMAction getTreeSelectionAction(JTree jTree, ElementDialogPanel edp) {
	...
	/**
	 * Methode liefert eine neue LGMAction zurück.
	 * 
	 * Diese LGMAction verwaltet das Initialisieren von DragNDrop in einem
	 * Panel.
	 * 
	 * Alle Panels, die DragNDrop-Funktionalität bieten wollen, müssen diese
	 * Action über einen MouseListener an ihre Trees anfügen. Dabei sollte diese
	 * Action sowohl bei mousePressed als auch bei mouseEntered aufgerufen
	 * werden.
	 * 
	 * @param dndActionChains
	 */
	public static final LGMAction getDragNDropInitAction(DragNDropInitializer.DragNDropActionChain[] dndActionChains) {
		...
		return new LGMAction() {
			...
			/**
			 * Sammlung aller <code>DragNDropActionChain</code>s, die bei
			 * einem DragNDrop-Ereignis ausgeführt werden können
			 */
			private DragNDropActionChain[] dndActionChains = chains;
			 ...
			 * <code>true</code>, wenn gerade eine DragNDrop-Aktion
			 * ausgeführt wird =<code>false</code>, sonst
			 */
			 ...
			 * werden Uneindeutigkeiten beim DragNDrop vermieden.
			 * 
			 * Solang die Mousetaste gedrückt bleibt, ist das Initialisieren
			 * einer neuen DragNDrop-Aktion deaktiviert, um das Auführen der
			 * aktuellen DragNDrop-Aktion nicht zu behindern.
			 * 
			 * @param me
			 */
			private void mousePressed(MouseEvent me) {

				this.blockDragNDropInitializing = true;
				...
				int n = this.dndActionChains.length;
				
				if (n>2) {
					for (int i=0; i<n; i++) {
						LGMDragNDropTree tree = this.dndActionChains[i].getSrcTree();
						if (tree != focusedTree) {
				...
				this.blockDragNDropInitializing = false;
				this.mouseEntered(me);
			}

			/**
			 * Methode ruft
			 * <code>activateDragNDrop(LGMDragNDropTree focusedTree)</code>
			 * auf, falls sich die Mouse über einen der Trees des Panels
			 * befindet.
			 * 
			 * @param me
			 */
			private void mouseEntered(MouseEvent me) {
				...
				if (blockDragNDropInitializing == false && me.getSource() instanceof LGMDragNDropTree) {
					this.activateDragNDrop((LGMDragNDropTree) me.getSource());
				}
			}
			 ...
			 * @param focusedTree
			 */
			private void activateDragNDrop(LGMDragNDropTree focusedTree) {

				if (this.lastEnteredTree == focusedTree)
					return;

				this.lastEnteredTree = focusedTree;

				for (int i = 0; i < this.dndActionChains.length; i++) {
					DragNDropActionChain dndAC = this.dndActionChains[i];
	...
	/**
	 * Methode liefert eine <code>LGMAction</code> zurück, die das Erzeugen
	 * eines neuen Elements realisiert.
	 * 
	 * @param panel
	 * @param elementClass
	 * 
	 ...
	 * @throws ActionNotDefinedForClassException
	 */
	public static final LGMAction getSplitAction(JTree jTree, ElementDialogPanel edp, Class<? extends ModelElement> elementClass) throws ActionNotDefinedForClassException {
			...
			return new LGMAction(Tool3lgmConstants.getResString("abkonfSplitButtonText")) {
				...
				public void execute(final EventObject eo) {
					...
					if (selpaths == null || selpaths.length == 0) {
						...
						//es ist etwas selektiert -> alle selektierten OEs an eine neue AufOrgKombination
						...
						//trenne jede selektierte OE von der originalen AufOrgKombination und verbinde sie
					...
					for (int i = 0; i < newAufOrgs.size(); i++) {
						...
						for (int j = 0; j < abKonfs.size(); j++) {
							...
							//verbinde diese neue ABKonfiguration mit der aktuellen neuen AufOrgKombination
							...
							//erstelle eine Liste aller Anwendungsbausteine, die die ABKonfiguration enthält
...
//					doc.finish_transaction()
	 ...
	 * @throws ActionNotDefinedForClassException
	 */
	public static final LGMAction getMoveDownAction(JTree tree, ElementDialogPanel edp) throws ActionNotDefinedForClassException {
						...
						if (pos1 < lroot.getChildCount() - 1) {
							...
							if (en != null) {
								...
								while (en.hasMoreElements()) {
									ltree.expandPath(en.nextElement()); //seine Unterknoten auch expandieren
								}
	 ...
	 * @throws ActionNotDefinedForClassException
	 */
	public static final LGMAction getMoveUpAction(JTree tree, ElementDialogPanel edp) throws ActionNotDefinedForClassException {
	 ...
	 * ermittelt.
	 * 
	 * Dabei wird das Element, was sich dem Mouse-Zeiger am nächsten befindet,
	 * als SelectionPath des targetTrees gesetzt.
	 * 
	 * Damit ist es möglich Elemente an eine bestimmte Position innerhalb eines
	 * Trees zu verschieben.
	 * 
	 ...
	 * @param jTree
	 */
	private static final LGMAction getDragNDropLocateElementAsTargetAction(JTree jTree) {
	 ...
	 * @param edp
	 */
	public static final LGMAction getWindowClosedAction(ElementDialogPanel edp) {
	 ...
	 * @param edp
	 */
	public static final LGMAction getComponentShownAction(ElementDialogPanel edp) {
	...
	/**
	 * Gibt das <code>ModelElement</code> des
	 * <code>ElementPropertyDialog</code> s wieder, in dem sich der
	 * <code>tree</code> befindet.
	 * 
	 * @param tree
	 * 
	 * TODO: diese Funktion hat jetzt das {@link LGMDragNDropPanel}, so dass das hier irgendwann mal weg kann
	 */
	private static ModelElement getTopLevelModelElement(JTree tree) {
