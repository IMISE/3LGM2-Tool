...
public class RedundancyChecker extends WindowAdapter {
	 ...
	 */
	public static final void getReport(GDCollection gdcoll) {
		...
		//1.) Alle Inkonsistenzen, die die normale Konsistenzprüfung für Elemente liefert, die die bezüglich ihrer Redundanz zu untersuchenden
		//		oder auf dem verbindenden Pfad dazwischenliegenden Elemente betrifft, müssen beseitigt werden.
		//2.) Jedes der Elemente, die am Ende als redundant ausgegeben werden sollen (in unserem speziellen Fall sind das AWB) darf nur einen
		//		einzigen Pfad zu der anderen Elementart haben (in unserem Fall zu Aufgaben).
		//3.) Für alle auf dem zu untersuchenden Gesamtpfad liegenden Elemente, die in Teil-Von-Beziehung stehen können, dürfen nur die 
		//		Blattelemente mit jeweils anderen Elementen auf diesem Pfad  verbunden sein.

		//alle allgemeinen Konsistenzfehler suchen, die bei Klassen auftreten, die für die Redundanzanalyse relevant sind
		...
		//eigentlich sollte hier immer schon dieselbe GDCollection selektiert sein, aber zur Sicherheit wird mal dahin gewechselt
		...
		if (errors.size() > 0) {
			...
			Object[] options = {Tool3lgmConstants.getResString("ana_fr_resolve_errors"),
								Tool3lgmConstants.getResString("ana_fr_ignore_errors"),
			                    Tool3lgmConstants.getResString("cancel")};
			int answer = JOptionPane.showOptionDialog(Tool3lgm.tool,
				Tool3lgmConstants.getResString("ana_fr_error_message"), 
				Tool3lgmConstants.getResString("ana_fr_error_message_title"),
	 ...
	 * 
	 * @param doc
	 *            Modell für das die XMLAnalyse durchgeführt wurde
	 * @param redundancyAnalysisResults
	 */
	private static void showResult(ArrayList<RedundancyAnalysisResult> redundancyAnalysisResults) {
		...
		for (RedundancyAnalysisResult result : redundancyAnalysisResults) {
			...
			// Anzahl aller AWB
			int fullAWBCount = result.exclusiveAWB.size() + result.moreNeededAWB.size() + result.uselessAWB.size() + result.moreUselessAWB.size() /*+ result.notSupportingAWB.size()*/;
	 ...
	 * @return
	 */
	private static String getElementName(ModelElement me) {
	 ...
	 * @param asJava
	 */
	public static void printData(int[][] matrix, boolean asJava) {
		if (!asJava) {
			if (matrix.length == 0) {
				System.out.println("[]");
				return;
	...
	private static final class RedundancyThread extends Thread {
		 ...
		 */
		private ArrayList<RedundancyAnalysisResult> resultList;

		/**
		 * @param resultsToFill
		 *            Liste mit RedundancyAnalysisResult, die gefüllt werden
		 *            sollen
		 */
		public RedundancyThread(ArrayList<RedundancyAnalysisResult> resultsToFill) {
			super();
			this.resultList = resultsToFill;
		}
