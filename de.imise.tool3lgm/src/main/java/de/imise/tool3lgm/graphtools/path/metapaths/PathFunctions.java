...
public class PathFunctions {
    ...
    static final boolean isCreatable(final ModelElement startElement, final ModelElement endElement, final MetaPath metaPath, final boolean checkConsistency) {
        ...
        if (metaPath instanceof ElementaryMetaPath) {
            ...
            //                            return false;
            //                        }
            //                        if (creatableMetaPathEndClass && Modifier.isAbstract(mp.getEndClasses().iterator().next().getModifiers())) {
            //                            creatableMetaPathEndClass = false;
            //                        }
            //                        if (creatableNextMetaPathStartClass && Modifier.isAbstract(nextMetaPath.getStartClasses().iterator().next().getModifiers())) {
    ...
    static final PathConnectionState getPathConnectionState(final ModelElement startElement, final ModelElement endElement, final MetaPath metaPath, final boolean searchParents, final boolean searchParts) {
        ...
        boolean self = PathFunctions.isConnected(startElements, endElements, metaPath);
        boolean parent = false;
        boolean part = false;

        if (searchParents) {
            startElements = startElement.getParentElements(false);
            parent = PathFunctions.isConnected(startElements, endElements, metaPath);
        }
        if (searchParts) {
            startElements = startElement.getPartElements(false);
            part = PathFunctions.isConnected(startElements, endElements, metaPath);
        }
        if (self && parent && part) {
    ...
    public static final ModelElement createNodeWithContainerAndDependents(final LGMGraphDocument doc, final ModelElement startElement, final ElementaryMetaPath pathToNewElement, final ElementaryMetaPath pathFromNewElement, final int pid) {
        ...
        Direction nextEdgeClass2CreateDirection = pathFromNewElement != null ? pathFromNewElement.getDirection() : FORWARD;
        return PathFunctions.createNodeWithContainerAndDependents(doc, startElement, edgeClass2Create, edgeClass2CreateDirection, newElementClass, nextEdgeClass2Create, nextEdgeClass2CreateDirection, pid);
    }
    ...
    private static final ModelElement createNodeWithContainerAndDependents(final LGMGraphDocument doc, final ModelElement startElement, final Class<? extends Edge> edgeClassToNewElement, final Direction directionToNewElement,
        ...
        if (startElement != null && MetaPathFunctions.isCompositionFromMasterToSlave(edgeClassToNewElement, directionToNewElement)) {
            ...
            createdContainer = doc.createNodeAndContainer(newElementClass, pid);
        ...
        Class<? extends Edge>[] edgeTypes = metaModel.getEdgeTypes(newElementClass);
        ...
        for (int i = 0; i < edgeTypes.length && !interrupted; i++) {
            ...
            if (CoreMetaModel.isStartClass(edgeType, newElementClass)) {
                ...
                if (minCardinalityForwardToOther > 0) {
                    ...
                    Class<? extends ModelElement> endClass = Edge.getEndClass(edgeType);
                    ...
                    while (minCardinalityForwardToOther - edgesForwardToCount > 0) {
                        ...
                        ModelElement created = createNodeWithContainerAndDependents(doc, createdDependent, edgeType, FORWARD, endClass, null, null, pid);
                        if (created == null) {
                            interrupted = true;
                            break;
                        }
                ...
                if (minCardinalityBackwardToOther > 0) {
                    ...
                    Class<? extends ModelElement> startClass = Edge.getStartClass(edgeType);
                    //wenn weitere Kanten angelegt werden müssen
                    while (minCardinalityBackwardToOther - edgesBackwardToCount > 0) {
                        ...
                        //geht, ist die edgeClassFromNewElement null.
                        ModelElement created = createNodeWithContainerAndDependents(doc, createdDependent, edgeType, BACKWARD, startClass, null, null, pid);
                        if (created == null) {
                            interrupted = true;
                            break;
                        }
