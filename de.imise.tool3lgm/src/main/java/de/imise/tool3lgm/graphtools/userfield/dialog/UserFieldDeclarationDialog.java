 ...
 * 
 * Dialog to create, edit, remove, import and export user-definied
 * property-fields for model-elements
 */
public final class UserFieldDeclarationDialog extends JDialog implements ActionListener, ListSelectionListener, MouseListener {
	...
	/** list with the defined userFields
	 * Es wird hier keine AplphabeticalJList genutzt, 
	 * weil die Elemente in der Reinhenfolge angezeigt werden sollen, 
	...
	private GDCollection gdcol;
	
	/** Speichert alle defnierten benutzerdefinierten Eigenschaftsfelder */
	private UserFieldDefinitions definitions;

	/** 
	 * Clone der Definitionen vor allen Änderungen. Wird beim Abbrechen auf diese
	 * Defnition zurück gesetzt.
	 */ 
	private UserFieldDefinitions oldUserFieldDefionitions;
	...
	/**
	 * Liste mit allen UserFields, die gelöscht wurden. Wird der Dialog mit OK verlassen,
	 * werden bei allen <code>UserFieldTarget</code>s die Werte dieser UserFields *unwiederbringlich*
	 * gelöscht.
	 */
	private ArrayList<UserField> removedUserFields = new ArrayList<UserField>();
	 ...
	 */
	private UserFieldDeclarationDialog(Frame owner, GDCollection gdcol) throws HeadlessException {
		super(owner, Tool3lgmConstants.getResString("userfields"), true);
		this.gdcol = gdcol; 
		this.definitions = gdcol.getUserFieldDefinitions();
	 ...
	 * @param doc
	 * 			actual which give the context of <code>GDCollection</code> and <code>UserFieldDefinitions</code>
	 ...
	 */
	public static int showDialog(Frame owner, GDCollection gdcol) {
	...
	/**
	 * Initialiert die <code>ComboBox</code> mit den Klasseneinträgen, für die
	 * <code>UserField</code> s definiert werden können.
	 */
	...
	/**
	 * Setzt in Abhängigkeit von der ausgewählten Klasse der <code>classComboBox</code>
	 * nur die Arten von neu anlgebaren benutzerdefinierten Eigenschaftenfeldern, die man
	 * für die jeweilige Klasse anlegen kann.<br>
	 * Für Kantenklassen kann man als einziges Verteilungsgewichte anlegen. Für Kanten sind
	 * aber keine Kennzahlen oder Kennzahlformeln vorgesehen. Im Moment auch nicht für Assoziationsklassen 
	 * (z. B. <code>KommBeziehung</code>), da man das dür das Umsetzen von Ansgars Kostenmodell
	 * nicht braucht.
	 */
	...
	/**
	 * Gibt in Anghänigkeit des übergebenen userfields den style dessen als
	 * String zurück.
	 * 
	 * @param u
	 * @return String, der den ausgeschirebenen Style enthält.
	 */
	private static final String getUserFieldStyle(UserField u) {
...
*/		
	 ...
	 * @param userField
	 */
	private void addFieldListEntry(UserField userField){
	 ...
	 * @param index
	 */
	private void addFieldListEntry(UserField userField, int index){
    ...
    public void actionPerformed(ActionEvent e) {
		if (e.getActionCommand().equals("ok")) {
			...
			if (showWarningForDeletingUserFields) {
				String[] frage = { Tool3lgmConstants.getResString("dontShowAgain") };
