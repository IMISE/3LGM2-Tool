...
public class StructurePanel extends LGMDragNDropPanel{ 
	 ...
	 * 
	 */
	private LGMDragNDropTree lotree, lutree;
	 ...
	 * COMMENTME
	 */
	private ArrayList<ElementContainer> childrenToExcludeFromRtree = new ArrayList<ElementContainer>(100);
	 ...
	 * @param pd
	 */
	public StructurePanel(ElementPropertyDialog pd) {
		super(pd);
		...
		try {
			this.addUeberAction = LGMActionLibrary.getAddElementAction(this.rtree,this.lotree,this,false);
			this.removeUeberAction = LGMActionLibrary.getDisconnectAction(this.lotree,this.rtree,this,false);
			this.addUnterAction = LGMActionLibrary.getAddElementAction(this.rtree,this.lutree,this,true);
			this.removeUnterAction = LGMActionLibrary.getDisconnectAction(this.lutree,this.rtree,this,true);
		}
		catch (Exception e) {
			e.printStackTrace();
		}
	...
	/**
	 * Hier werden alle möglichen DragNDrop-Aktionen zwischen den Trees des Panels 
	 * zumsammengefasst und als Attribut in der Oberklasse abgespeichert.
	 */
	@Override
	protected DragNDropInitializer.DragNDropActionChain[] collectDragNDropActionChains() {
		 ...
		 */
		DragNDropInitializer.DragNDropActionChain tac1 = 
			DragNDropInitializer.createNewDragNDropActionChain(rtree, lotree, addUeberAction);
		DragNDropInitializer.DragNDropActionChain tac2 =
			DragNDropInitializer.createNewDragNDropActionChain(rtree, lutree, addUnterAction);
		DragNDropInitializer.DragNDropActionChain tac3 =
			DragNDropInitializer.createNewDragNDropActionChain(lotree, rtree, removeUeberAction);
		DragNDropInitializer.DragNDropActionChain tac4 =
			DragNDropInitializer.createNewDragNDropActionChain(lutree, rtree, removeUnterAction);
		
		/*
		 * alle Aktionen zwischen lotree <-> lutree
		 */
		DragNDropInitializer.DragNDropActionChain tac5 = DragNDropInitializer.createNewDragNDropActionChain(new LGMDragNDropTree[]{lotree,rtree,lutree},
				new LGMAction[]{removeUeberAction, addUnterAction});
		DragNDropInitializer.DragNDropActionChain tac6 = DragNDropInitializer.createNewDragNDropActionChain(new LGMDragNDropTree[]{lutree,rtree,lotree},
				new LGMAction[]{removeUnterAction, addUeberAction});
		
		
		DragNDropInitializer.DragNDropActionChain[] allDndActionChains = 
			new DragNDropInitializer.DragNDropActionChain[] { tac1, tac2, tac3, tac4, tac5, tac6 };
	...
	public LGMDragNDropTree[] getAllDragNDropTrees() {
		return new LGMDragNDropTree[] { rtree, lotree, lutree };
	}
