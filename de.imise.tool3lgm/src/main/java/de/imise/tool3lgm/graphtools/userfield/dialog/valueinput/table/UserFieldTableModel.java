 ...
 * <p>
 * Über die statischen Methoden können vorgefertigte <code>UserFieldTableModel</code>s abgerufen
 ...
 * sondern über setDataVector(java.lang.Object[][], java.lang.Object[],java.lang.Object[]). 
 * Die Anwendung der alten Methode kann dann dazu führen, dass die im Table dargstellten 
 * Daten nicht mehr den Modeldaten entsprechen !!!
 * 
 * @author fstephan
 */
public class UserFieldTableModel extends DefaultTableModel {
	...
	/**
	 * Beinhaltet alle UserFields in einer HashMap, die für Knoten, Kanten und das Modell deklariert und 
     * definiert wurden. 
	 */
	 ...
	 * Daten des tables
	 * 
	 * Beinhaltet im Gegensatz zum {@link DefaultTableModel#dataVector} die aus <code>UserField</code>
	 * und anzuzeigendem Wert bestehenden <code>NamedObjectContainer</code>.
	 * 
	 * Mittels der <code>UserField</code>s ist es dann für den <code>TableCellRenderer</code>
	 * möglich, formatierte Werte darstellen zu können.
	 */
	 ...
	 * @param doc
	 */
	protected UserFieldTableModel(GraphDocument doc) {
		super();
		this.doc = doc;
	 ...
	 */
	private void setDataForCNTable(Class<? extends ModelElement> elementClass, boolean showTopLevel, boolean showInner, boolean showLeafs,  UserField.Style userFieldStyle) {
...
//TODO:FST,XHB. Wenn die Kante PDVBKAWBVerb übergeben wurde, bleibt allModelElements leer. Ist auch richtig,solange es keine Soclhe Verbindung gibt. 
	 ...
	 * @param field
	 * @param direction
	 *            Richtung in der die ausgwählte Kante zu lesen ist. In der
	 *            Tabelle sthen die Startklassen der Kante in den Zeilen, wenn
	 *            <code>DoubleTrace.FORWARD</code> übergeben wurde. Bei
	 *            <code>DoubleTrace.BACKWARD</code> stehen die
	 *            Endklassenelemente in den Zeilen.
	 */
	private void setDataForDWTable(Class<? extends Kante> edgeClass, int direction, UserField field) {
	...
	private void setDataForMVTable() {
		...
		Object[] columnIdentifiers = new Object[] {"Wert"};
	 ...
	 * 
	 * @param elementClass
	 * 			in der <code>nodeBox</code> des <code>ClassificationNumberEditorPanel</code>s
	 * 			ausgewählte Klasse
	 * @param doc
	 * @param elementTypesToShow
	 * 			Typ der anzuzeigenden Elemente (siehe Konstanten) 
	 * @return
	 */
	public static final UserFieldTableModel createClassificationNumberModel(Class<? extends ModelElement> elementClass, GraphDocument doc, boolean showTopLevel, boolean showInner, boolean showLeafs) {
	 ...
	 * @return
	 */
	public static final UserFieldTableModel createClassificationModelVariableModel(GraphDocument doc) {
		UserFieldTableModel model = new UserFieldTableModel(doc);
	 ...
	 * @param elementClass
	 * @param direction
	 *            Richtung in die die übergebene Kantenklasse gelesen werden
	 *            soll. (<code>Doppelkante.FORWARD</code> oder
	 *            <code>Doppelkante.BACKWARD</code>)
	 * @param field
	 * @param doc
	 * @return
	 */
	public static final UserFieldTableModel createClassificationWeightingModel(Class<? extends Kante> edgeClass, int direction, UserField field, GraphDocument doc) {
		UserFieldTableModel model = new UserFieldTableModel(doc);
	...
	/**
	 * Erzeugt ein Model für einen Table, der für alle ModelElemente die berechneten
	 * Kennzahlen anzeigt. Ein Editieren dieses Tables ist nicht möglich.
	 * @param elementClass
	 * @param doc
	 * @param elementTypesToShow
	 * @return
	 */
	public static final UserFieldTableModel createClassificationNumberFormulaModel(Class<? extends ModelElement> elementClass, GraphDocument doc, boolean showTopLevel, boolean showInner, boolean showLeafs) {
		UserFieldTableModel model = new UserFieldTableModel(doc) {
			/* (non-Javadoc)
	...
	public String getColumnToolTip(int columnIndex) {
		return this.columnIdentifiers.get(columnIndex).toString();
	}
	...
	public String getRowToolTip(int rowIndex) {
		return this.rowIdentifiers.get(rowIndex).toString();
	}
	
	/**
	 * Überschreibt die Methode der Superklasse, sodass in Abhängigkeit der Daten über die
	 * Editierbarkeit der Zelle an der Stelle (<code>row</code>,<code>column</code>) entschieden wird.
	 * 
	...
	public boolean isCellEditable(int row, int column) {
		...
		if (column >= 0 && this.getValueAt(row, column) == null)
			return false;
	 ...
	 */
	public void setDataVector(Object[][] data, Object[] columnIdentifiers, Object[] rowIdentifiers) {
		...
		this.dataField = new Object[data.length][data[0].length];
		this.dataField = new Object[rowIdentifiers.length][columnIdentifiers.length];
		for (int i=0; i<this.dataField.length;i++) {
			for (int j=0; j<this.dataField[0].length;j++) {
				this.dataField[i][j] = data[i][j];
			}
		}
		super.setDataVector(data, columnIdentifiers);
	...
	/**
	 * Methode gibt den <code>NamedObjectContainer</code> zurück, der das <code>UserField</code>
	 * und den Wert an der Stelle (row,col) enthält.
	 * 
	 * Überschreibt die Methode der Superklasse, damit der CellRenderer das Format
	 * feststellen kann.
	 * 
	 ...
	 * @return
	 */
	public Object[][] getValues(int firstRow, int lastRow, int firstColumn, int lastColumn) {
	...
	/**
	 * Übernimmt einen im Table eingegebenen Wert in dieses Model.
	 * 
	 * Dabei wird ein neuer <code>NamedObjectContainer</code> aus <code>value</code>
	 * und dem bereits in {@link #dataField} enthaltenen <code>UserField</code> erstellt und
	 * als neuer Wert in {@link DefaultTableModel#dataVector} und {@link #dataField} gesetzt.
	 * 
	 * @param value
	 * @param row
	 * @param col
    ...
    public void setValueAt(Object value,int row, int col) {
		...
		UserField field = ((NamedObjectContainer<UserField>)this.dataField[row][col]).getObject();
		...
		// dataField update
		this.dataField[row][col] = newValue;
     ...
     * @param row
     * 			Zeilenindex dieser Zelle
     * @param col
     * 			Spaltenindex dieser Zelle
     */
    ...
    /**
     * Setzt die Modeldaten beginnend bei (<code>firstRow</code>,<code>firstColumn</code>)
     * auf die Werte in <code>values</code>.<br>
     * Wird dabei die Größe des Models überschritten, gibt diese Methode <code>false</code>
     * zurück.<br>
     ...
     * 			neue Werte
     * @param firstRow
     * 			Zeilenindex, ab dem die Werte eingefügt werden sollen 
     * @param firstColumn
     * 			Spaltenindex, ab dem die Werte eingefügt werden sollen 
     * @return
     */
    public boolean setValuesAt(Object[][] values, int firstRow, int firstColumn) {
	...
	public boolean hasColumnIdentifiers() {
		return (this.columnIdentifiers != null);
	}
	...
	public boolean hasRowIdentifiers() {
		return (this.rowIdentifiers != null);
	}
	...
	public boolean hasData() {
		return (this.dataField != null && dataField.length > 0);
	}
	...
	/**
	 * Methode setzt das Attribut {@link #dataChanged} und bestimmt damit, ob sich
	 * Daten geändert haben oder nicht.
	 * <br>
