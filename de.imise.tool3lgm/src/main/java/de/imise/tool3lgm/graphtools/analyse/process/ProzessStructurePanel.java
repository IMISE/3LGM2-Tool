 ...
 * 
 */
public class ProzessStructurePanel extends ElementDialogPanel implements TreeWillExpandListener {
	 ...
	 * COMMENTME
	 */
	private JPanel addRemoveControl;
	...
	/**
	 * werden gebraucht, um die Bäume zu aktualisieren, wenn sich die Anzahl der Aufgaben, Objekttypen oder
	 * die Kanten zwischen diesen beiden ändert während dieses Panel offen ist, NUR DANN muss aktualisiert werden,
	 * sonst nicht
	 */
	 ...
	 * @param pd
	 */
	public ProzessStructurePanel(ElementPropertyDialog pd) {
		super(pd);
		...
		/*
		 * / erstmal wieder raugenommen, weil sich das beim Abbrechen nicht
		 * zurücknehmen lässt JButton clearBut = new
		 * JButton(Tool3lgmConstants.getIcon("clear.gif"));
		...
		try {
			upAction = LGMActionLibrary.getMoveUpAction(ltree, this);
			downAction = LGMActionLibrary.getMoveDownAction(ltree, this);
		...
		try {
			...
			LGMAction verificationCheckAction = getVerfikationAction(this, this.ltree);
	...
	/**
	 * Sorgt dafuer, dass die rechte Seite des Panels angezeigt oder verborgen
	 * wird. Es werden KEINE Daten aktualisiert sondern lediglich die
	 * Komponenten hinzugefuegt oder entfernt.
	 * 
	 * @param visible
	 *            boolean
	...
	public void setRightSideVisible(boolean visible) {
		...
		if (visible && !labelRechts.isVisible()) {
			...
			// rechte Seite nur invisible setzen, wenn sie es nicht schon ist
		} else if (!visible && labelRechts.isShowing()) {
			addRemoveControl.setVisible(false);
			labelRechts.setVisible(false);
			spRechts.setVisible(false);
			// this.viewButton.setIcon(Tool3lgmConstants.getIcon("auf.gif"));
	 ...
	 */
	private void appendObjectTypes(LGMTreeNode aufgabenContainerNode) {
...
//		System.out.println("actionPerformed command: " + e.getActionCommand());
		...
		if (str.equals("aufklappen")){
			if (verify)	check3LGMBuisnessProcess();
		}
			...
			synchronized (ltree.getTreeLock()){
				...
				if (selPath != null){
					...
					if (node.getUserObject() instanceof NodeContainer){
						//seinen Container holen (diese muessen links im Baum geaddet werden)
		...
		if (str.equals("removeueber")) {
			synchronized (rtree.getTreeLock()){
				...
				if (selPath != null){
					LGMTreeNode node = (LGMTreeNode)selPath.getLastPathComponent();
					//prüfen, ob eine Aufgabe selektiert ist
					Object knot = node.getUserObject(); //auf keinen Fall hier gleich auf ElementContainer casten, weil es auch ein String sein kann !!!
		...
		if (str.equals("moveup")){
			...
			if (selPath != null && selPath.getPathCount()==2){
				...
				if (pos1>0){ 
					...
					LGMTreeNode node = (LGMTreeNode)lroot.getChildAt(pos1-1);	//den oberen Knoten holen
					lmodel.removeNodeFromParent(node);						//ihn entfernen
					lmodel.insertNodeInto(node, lroot, pos1);				//ihn einen tiefer als vorher einfügen
					//wenn die selektierte Aufgabe expandiert war
					if (en!=null){
						expandFullPath = true;	//muss sein wegen treeWillExpand, damits auch wirklich expandiert wird
...
/*	Das hier würde sich bei drücken auf Abbrechen nicht zurücknehmen lassen, da das UNDO für link nicht gesetzt wurde
*/	/*	
	if (str.equals("clear")){
...
/*
			for (int i=ltree.getRowCount()-1; i>=0; i--){
				...
				doc.exec("UNLINK " + prozess.getHashString() + " " + knotCont.getHashString(), dialog.getID());
				lroot.remove(i);
			}		  	
*//*
			return;
...
/**/	//}
	...
	public void valueChanged(TreeSelectionEvent e) {
		...
		/*
		 * for (int pos = 0; pos < doc.selectedContainer.size(); pos++) {
		 * ElementContainer ec = (ElementContainer)
		 * doc.selectedContainer.get(pos); if (doc.selectedContainer.get(pos) !=
		...
		// es ist etwas selektiert
			...
			if (clickedRow != -1) {
				...
				if (((LGMTreeNode) tree.getPathForRow(clickedRow).getLastPathComponent()).getUserObject() instanceof String)
					return;
				// wenn auf einen nicht selektierten Eintrag geklickt wurde ->
				// diesen Eintrag selektieren
	...
	public final void mouseClicked(MouseEvent e) {
		...
		if (e.getClickCount() == 1 || (pathToExpandOrCollapse != null && pathToExpandOrCollapse.getPathCount() > 2)) {
			...
			if (sourceName.equals("righttree"))
				this.addAction.execute(e);
			else if (sourceName.equals("lefttree"))
				this.removeAction.execute(e);
			willExpand = false;
			willCollapse = false;
			pathToExpandOrCollapse = null;
		}
	 ...
	 * @param row
	 */
	public void setSelectionRow(JTree destinationTree, int row) {
		if (row == -1)
	 ...
	 * @param b
	 */
	public void expandFullPath(boolean b) {
		this.expandFullPath = b;
	}
	...
	protected DragNDropActionChain[] collectDragNDropActionChains() {
		DragNDropActionChain dndAC1 = DragNDropInitializer.createNewDragNDropActionChain(this.rtree, this.ltree, this.addAction);
		DragNDropActionChain dndAC2 = DragNDropInitializer.createNewDragNDropActionChain(this.ltree, this.rtree, this.removeAction);

		DragNDropActionChain dndAC3 = DragNDropInitializer.createNewDragNDropActionChain(new LGMDragNDropTree[] { this.ltree, this.rtree, this.ltree }, new LGMAction[] { this.removeAction, this.addAction });

		return new DragNDropActionChain[] { dndAC1, dndAC2, dndAC3 };
	...
	private void initDragNDrop() {
		...
		LGMDragNDropTree[] trees = this.getAllDragNDropTrees();

		for (int i = 0; i < trees.length; i++) {
			trees[i].addMouseListener(ml);
			trees[i].addMouseMotionListener(ml);
		}
	 ...
	 */
	private static final LGMAction getVerfikationAction(ElementDialogPanel edp, JTree tree) throws ActionNotDefinedForClassException {
	 ...
	 * @throws ActionNotDefinedForClassException
	 */
	private static final LGMAction getFehlerAction(ElementDialogPanel edp) throws ActionNotDefinedForClassException {
		final ElementDialogPanel pane = edp;
		if (edp instanceof ProzessStructurePanel) {
