...
public class InputGraphArea extends BasicGraphArea implements MouseListener, MouseMotionListener, MouseWheelListener {
	 ...
	 */
	private int[] xreal = new int[ModelConstants.LAYERS.length], yreal = new int[ModelConstants.LAYERS.length];
	 ...
	 */
	private int[] lastXreal = new int[ModelConstants.LAYERS.length], lastYreal = new int[ModelConstants.LAYERS.length];
	 ...
	 * @param gdoc
	 */
	public InputGraphArea(GraphDocument gdoc) {
		super(gdoc);
	...
	@Override
    public void setMultiViewEnabled(boolean b) {
		multi_view = b;
	 ...
	 * @param k
	 */
	public final void setNextKnot(Knoten k) {
		if (k == null)
			return;
		next_knot = k;
	 ...
	 * @param b
	 */
	public final void setMouseMakesKnot(boolean b) {
		if (b)
	 ...
	 * @param b
	 */
	public final void setMouseMakesTrace(boolean b) {
		if (b)
	 ...
	 * COMMENTME
	 */
	public final void computeRealCoordinates(boolean initLastAsReal) {
		if (!initLastAsReal){
			lastXreal[0] = xreal[0];
			lastYreal[0] = yreal[0];
			lastXreal[2] = xreal[2];
	...
	/**
	 * Berechnet in jeder Richtung die minimalen und maximalen Koordinaten von der übergebenen 
	 * {@link Rectangle} und dem {@link ElementContainer}. Wird als {@link Rectangle} <code>null</code>
	 * übergeben, dann kommt ein neues {@link Rectangle}-Objekt zurück, ansonsten wird das bestehende
	 * zurück gegeben.
	 ...
	 */
	private static final Rectangle getIncludingRectangle(Rectangle rect, ElementContainer ec) {
	 ...
	 * @return
	 */
	private final static Rectangle getIncludingRectangle(Rectangle rect, int x1, int y1, int x2, int y2){
		if (rect == null)
	...
	/**
	 * Füllt die 3 {@link Rectangle} {@link #grabbedElementsFullRect}, {@link #grabbedElementsRasteredRect} und
	 * {@link #grabbedElementsRealRect} mit den Koordinaten in abhängigkeit von der Selektion. Wenn eine 
...
*/		
		...
		//sonst Eigenschaftsdialog anzeigen
		else if (ka != null)
...
//									doc.getCollection().removeBendpoint(kpc, TransactionManager.STANDARD_PID);
	 ...
	 * @return
	 */
	private static final NodeContainer chooseResizable(LayerContainer lc, int x, int y) {
		if (lc == null)
			return null;
	 ...
	 * @return
	 */
	private final ElementContainer chooseObject(LayerContainer lc, int x, int y) {
		if (lc == null)
			return null;
		int counter;
