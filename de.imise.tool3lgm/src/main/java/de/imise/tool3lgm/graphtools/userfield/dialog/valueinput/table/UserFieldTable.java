...
/**
 * Klasse repräsentiert einen speziellen <code>JTable</code>, der zur Eingabe und Darstellung von Kennzahlen, Verteilungsgewichten und Modelvaribalen verwendet werden kann.
 * <p>
 * Es besteht die Möglichkeit, diese Tabelle in eine Komponente mit speziellem Layout einzubetten(im Allgem. ein <code>JScrollPane</code>). Die grafische Darstellung des Tables ist dann über
 * <code>getLayoutContainer()</code> zu erreichen.
 * <p>
 * Über eine <code>TableEditCondition</code> kann die Editierbarkeit einer jeden Zelle speziell festgelegt werden. Über die Konstanten können spezielle AutoResizeModes aktiviert werden.
 * <p>
 ...
 * Es ist möglich mittels Gedrückthalten der "Shift" oder "Strg" - Taste, mehrere Zellen auszuwählen und ebenso auch die Selektierung wieder aufzuheben. Änderungen durch Werteeingabe werden auf alle
 * ausgewählten Zellen übertragen.
 * <p>
 * Undo/Redo: <br>
 * Mit den Tastenkombinationen <b>Ctrl+Z</b> bzw. <b>Ctrl+Y</b> ist und Undo bzw. Redo möglich.
 * <p>
 * Cut/Copy/Paste: <br>
 * Mit den Tastenkombinationen <b>Ctrl+X</b>, <b>Ctrl+C</b>, <b>Ctrl+V</b> ist das Ausschneiden und das Kopieren von Werten aus dem Table in die Systemzwischenablage möglich, sowie das Einfügen von
 * Werten aus der Zwischenablage in den Table.
 * 
 * @author fstephan
 */
public class UserFieldTable extends JTable implements ContentExchanger {
	 ...
	 */
	private JScrollPane layoutContainer;
	...
	/**
	 * Bestimmt das Resize-Verhalten des Tables bei kleinem Fenster (Table kann nicht vollständig in seiner<code>preferredSize</code> dargestellt werden).
	 */
	...
	/**
	 * Konstruktor Setzt <code>uftm</code> als <code>dataModel</code>, <code>uftc</code> als <code>tableController</code> und bettet diesen Table in eine durch <code>uftl</code> erzeugte
	 * Layout-Komponente ein.
	 * 
	 * @param uftm
	 * @param uftl
	 * @param tec
	 * @return
	 */
	public UserFieldTable(UserFieldTableModel uftm, UserFieldTableLayout uftl, UserFieldTableController uftc) {
		super(uftm);
		this.tableController = uftc;
		this.tableLayout = uftl;
		this.layoutContainer = tableLayout.createLayoutContainer();
		init();
	}
	 ...
	 * @param uftl
	 */
	public UserFieldTable(UserFieldTableLayout uftl) {
		this(null, uftl, null);
	}

	/**
	 * Initialisiert diesen Table. <br>
	 * Es wird das AutoResize-Verhalten und das Focus-Verhalten festgelegt, ein {@link ContentExchangeListener} hinzugefügt sowie Undo bzw. Redo für die Tastenkombinationen <b>Ctrl+Z</b> bzw.
	 * <b>Ctrl+Y</b> gesetzt.
	 */
	private void init() {
	 ...
	 *            sagt aus, ob der Zellzwischenraum auch neu gezeichnet werden soll
	 */
	public void repaintCellAt(int row, int column, boolean includeSpacing) {
	...
	/*
	 * (non-Javadoc)
	 * @see tool3lgm.util.clipboard.ContentExchanger#copy()
	 */
	@Override
	public void copy() {
	...
	/*
	 * (non-Javadoc)
	 * @see tool3lgm.util.clipboard.ContentExchanger#cut()
	 */
	@Override
	public void cut() {
		copy();
	...
	/*
	 * (non-Javadoc)
	 * @see tool3lgm.util.clipboard.ContentExchanger#paste()
	 */
	@Override
	public void paste() {
		...
		if (leadingPoint.x == -1 || leadingPoint.y == -1) // keine gültige Selektion
			return;
	 ...
	 * @param key
	 */
	private void showWarningMessage(String key) {
	...
	public void undo() {
		...
		if (n < 1) // keine rückgängigmachbaren Änderungen vorhanden
			return;
	...
	public void redo() {
		...
		if (n < 1) // keine rückgängigmachbaren Undos vorhanden
			return;
	...
	private boolean saveValues() {
		...
		if (data == null) // keine Daten vorhanden, also auch kein Speichern möglich
			return false;
	 ...
	 * @param uftc
	 */
	public void setModel(UserFieldTableModel uftm, UserFieldTableController uftc) {

		super.setModel(uftm);

		if (uftc != null)
			this.setUserFieldTableController(uftc);
	 ...
	 * @param uftl
	 */
	public void setUserFieldTableLayout(UserFieldTableLayout uftl) {
		if (this.tableLayout != null)
			this.tableLayout.removeFrom(this);
		this.tableLayout = uftl;
	}
	 ...
	 */
	public void setUserFieldTableController(UserFieldTableController uftc) {
		if (this.tableController != null)
			this.tableController.removeFrom(this);
		this.tableController = uftc;
	}
	 ...
	 * @param smallMode
	 *            AutoResizeMode bei kleinem Fenster
	 * @param largeMode
	 *            AutoResizeMode bei großem Fenster
	 */
	public void setAutoResizeMode(int smallMode, int largeMode) {

		if (smallMode == largeMode)
			setAutoResizeMode(smallMode);
		else {
			setAutoResizeMode(-1);
			this.superResize = false;
	...
	public void setValueAt(Object value, int row, int col) {
		...
		if (!saveValues()) // Kein Datenarray vorhanden
			return;
	 ...
	 * @param col
	 */
	public void clearValueAt(int row, int col) {
	 ...
	 * @param tableCells
	 */
	void setTableCells(UserFieldTableCell[][] tableCells) {
	 ...
	 * @param width
	 *            bevorzugte Breite des Tables
	 * @param height
	 *            bevorzugte Höhe des Tables
	 */
	...
	/**
	 * Gibt den LayoutContainer dieses Tables zurück. Alle Klassen, die den Table darstellen wollen, sollten stattdessen das hier zurückgegebene ScrollPane darstellen. Die Modifizierungen des
	 * RowHeaders sind sonst nicht sichtbar.
	 * 
	 * @return container
	 */
	...
	/**
	 * Gibt den Renderer für die Zelle an Position (<code>row</code>,<code>column</code>) wieder. Der zurückgegebene Renderer sorgt für die formatierte Darstellung der Werte in dieser Zelle.
	 * 
	 * @param row
	 * @param column
	 */
	...
	/**
	 * Gibt den Editor für die Zelle an Position (<code>row</code>,<code>column</code>) wieder. Der zurückgegebene Editor stellt den tatsächlichen unformatierten Wert dar.
	 * 
	 * @param row
	 * @param column
	 */
	 ...
	 * 
	 * @param firstRow
	 *            erste Zeile, aber der die Werte zurückgegeben werden sollen
	 * @param lastRow
	 *            letzte Zeile, bis zu der die Werte zurückgegeben werden sollen
	 * @param firstColumn
	 *            erste Spalte, aber der die Werte zurückgegeben werden sollen
	 * @param lastColumn
	 *            letzte Spalte, bis zu der die Werte zurückgegeben werden sollen
	 * @param replaceIgnorableErrors
	 *            gibt an, ob Werte aus {@link UserField#IGNOREABLE_ERROR_SET} durch <code>""</code> ersetzt werden soll
	 * @param replaceErrors
	 *            gibt an, ob Werte aus {@link UserField#ERROR_SET} durch <code>""</code> ersetzt werden soll
	 * @return
	 */
	private Object[][] getValues(int firstRow, int lastRow, int firstColumn, int lastColumn, boolean replaceIgnorableErrors, boolean replaceErrors) {
	...
	/**
	 * Gibt den <code>Viewport</code> des {@link #layoutContainer}s wieder, das heißt den Bereich des {@link #layoutContainer}s, der die Zellen des Tables darstellt.
	 * 
	...
	public boolean isCellEditable(int row, int col) {
		if (this.tableController != null)
			return this.tableController.isEditable(row, col);
		return super.isCellEditable(row, col);
	}

	/**
	 * Überschreibt die Methode von {@link JTable}, sodass der Wert von {@link UserFieldTableController#isCellSelected(int, int)} an der Stelle (<code>row</code>,<code>column</code>) zurückgegeben
	 * wird.
	 */
	@Override
	public boolean isCellSelected(int row, int column) {
		if (this.tableController != null)
			return (this.tableController.isCellSelected(row, column) || (getEditingRow() == row && getEditingColumn() == column));
	 ...
	 */
	public boolean isAnchorPoint(Point p) {
		if (tableController != null)
	...
	public void removeSelectionListener(ListSelectionListener l) {
		...
		this.getColumnModel().getSelectionModel().removeListSelectionListener(l);
	}
	 ...
	 */
	@Override
	public void addContentExchangeListener(ContentExchangeListener l) {
	 ...
	 */
	@Override
	public void removeContentExchangeListener(ContentExchangeListener l) {
		removeKeyListener(l);
	}
	...
	public void removeFromLayoutContainer() {
		clearSelection();
		this.layoutContainer.setViewportView(null);
		this.layoutContainer.setRowHeaderView(null);
		this.layoutContainer.repaint();
	}
	 ...
	 * 
	 * @param originTable
	 *            Table, dessen Model-Werte angezeigt werden sollen
	 * @param owner
	 *            Frame, in dem der Table angezeigt werden soll
	 */
	public static void showModelValueTable(UserFieldTable originTable, Frame owner) {
	 ...
	 * 
	 * @param originTable
	 *            Table, dessen Editor-Werte angezeigt werden sollen
	 * @param owner
	 *            Frame, in dem der Table angezeigt werden soll
	 */
	public static void showEditorValueTable(UserFieldTable originTable, Frame owner) {
	 ...
	 * 
	 * @param originTable
	 *            Table, dessen Renderer-Werte angezeigt werden sollen
	 * @param owner
	 *            Frame, in dem der Table angezeigt werden soll
	 */
	public static void showRendererValueTable(UserFieldTable originTable, Frame owner) {
	 ...
	 * 
	 * @param originTable
	 *            Table, dessen Selektionszustand für die Zellen angezeigt werden sollen
	 * @param owner
	 *            Frame, in dem der Table angezeigt werden soll
	 */
	public static void showSelectionStateTable(UserFieldTable originTable, Frame owner) {
	 ...
	 * @param type
	 */
	private static void showInternalValueTable(UserFieldTable originTable, Frame owner, INTERNAL_VALUE_TYPE type) {
