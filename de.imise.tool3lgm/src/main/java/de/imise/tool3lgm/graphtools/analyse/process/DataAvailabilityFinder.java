...
public class DataAvailabilityFinder {
	 ...
	 */
	private ShortestCommunicationPathFinder commPathFinder;
	
	/**
	 * @param doc
	 */
	public DataAvailabilityFinder(GraphDocument doc){
		super();
		this.doc = doc;
...
//		System.err.println(elements.size());
		HashSet allMe = new HashSet(elements);
		for (int i=0; i<elements.size(); i++){
			ModelElement me = (ModelElement)elements.get(i);
			
			if (me.getConnectionsFrom(Anwendungsbaustein.class, PdvbPdvbVerbindung.class).size()>0 ||
				me.getConnectionsFrom(Anwendungsbaustein.class, RawbRawbVerbindung.class).size()>0);
				allMe.add(me);
		}
/*		System.err.println("Anwendungsbaustein: " + allMe.size());
		elements = mainDoc.getAllModelElements(Bausteinschnittstelle.class);
...
/*		int b=0; int i=0;
		...
		ArrayList elements = mainDoc.getModelItmes(AufObjVerbindung.class);
		for (int k=0; k<elements.size(); k++){
			Doppelkante kante = (Doppelkante)elements.get(k);
			if (kante.getDirection()==Doppelkante.FORWARD){
				if (kante.getStart() instanceof Aufgabe)
					b++;
				else
					i++;
			}else if(kante.getDirection()==Doppelkante.BACKWARD){
				if (kante.getStart() instanceof Aufgabe)
					i++;
				else
					b++;
...
*/		
	 ...
	 * @param objecttype
	 * @return
	 * 		<code>true</code>, wenn der übergebenene Objekttyp irgendwo gespeichert wird, sonst <code>false</code> 
	 */
	public boolean hasStoringSystem(Objekttyp objecttype){
	 ...
	 * @param objecttype
	 * @return
	 * 		<code>true</code>, wenn der übergebenene Objekttyp einen Master-Speicher besitzt, sonst <code>false</code> 
	 */
	public boolean hasMasterSystem(Objekttyp objecttype){
		ModelAnalyzerCache mac = commPathFinder.getAnalyzerCache();
	 ...
	 * @param objecttype
	 * @return
	 * 		<code>true</code>, wenn der übergebenene Objekttyp irgendwo gespeichert wird und er
	 * 		einen Master-Speicher besitzt, sonst <code>false</code> 
	 */
	public boolean isStored(Objekttyp objecttype){
		return hasStoringSystem(objecttype) && hasMasterSystem(objecttype);
	}
	
	/**
	 * Liefert eine Liste von <code>SameTypePair</code>s, mit allen Master-Anwendungssystemen des Objekttyps,
	 * zwischen denen untereinander kein gerichteter Kommunikationsweg existiert, über den der Objekttyp
	 * übertragen werden kann. Das erste Objekt jedes <code>SameTypePair</code> ist Sender und das zweite
	 * Empfänger des Objekttyps.<br>
	 * Es kann sein, dass gar keine Kommunikation zwischen 2 Master Anwendungsystemen möglich ist. Dann
	 * sind 2 <code>SameTypePair</code>s in der Liste, die jeweils die beiden Master als Objekte enthalten,
	 * aber beide in jeweils umgekehrter Reihenfolge. Ist nur eine kommunikationsrichtung nicht gegeben,
	 * dann ist nur das entsprechend gerichtete <code>SameTypePair</code> in der Liste.
	 * 
	 * @param objecttype
	 * @return
	 */
	public Collection<SameTypePair<ModelElement>> getMissingMasterToMasterCommunicationLinks(Objekttyp objecttype){
		ModelAnalyzerCache mac = commPathFinder.getAnalyzerCache();
	 ...
	 * @return
	 */
	public Collection<SameTypePair<ModelElement>> getMissingMasterToStorageCommunicationLinks(Objekttyp objecttype){
		ModelAnalyzerCache mac = commPathFinder.getAnalyzerCache();
		Set<ModelElement> masters = mac.getDirectMasterApplicationSystems(objecttype);
		Set<ModelElement> storage = mac.getDirectStorageApplicationSystems(objecttype);
	 ...
	 * @return
	 */
	public Collection<SameTypePair<ModelElement>> getMissingCommunicationLinks(Collection<ModelElement> sender, Collection<ModelElement> receiver, Objekttyp objecttype){
	...
	/**
	 * Gibt den minimalen Kommunikationsweg zwischen den Anwendungssystemen der Konfiguration der Aufgabe und den speichernden
	 * Anwendungssystemen des Objekttyps zurück. Wenn die Aufgabe keine Anwendungssysteme in ihrer Konfiguration 
	 * 
	 * @param function
	 * @param objecttype
	 * @return
	 */
	public ShortestCommunicationPathFinder.MinimalCommunicationPath getMinimalInterpretingPath(Aufgabe function, Objekttyp objecttype){
	 ...
	 * Kommunikation vom Master-Anwendungssystem des Objekttyps besteht. 
	 * Dies kann daran liegen, dass der Objekttyp kein Master-Anwenungssystem besitzt oder keine
	 * Kommunikationsverbindung zwischen dem Master- und dem speichernden Anwendungssystem besteht. 
	 * 
	 * @param objekttyp
	 * @return
	 * 		alle Anwendungssysteme, die den Objekttyp inkonsistent speichern
	 * 
	 * TODO: implementieren
	 */
	public Set<ModelElement> getInconsistentStorageSystems(Objekttyp objekttyp){
		return null;
	}
...
//das hier war nur der Testaufruf des alten funktionierenden Codes, um noochmal zu prüfen, wie es richtig laufen müsste			
//			ShortestCommunicationPathFinderOld scpfo = new ShortestCommunicationPathFinderOld(ot.getPartAndParentElements(), mainDoc.getCollection());
