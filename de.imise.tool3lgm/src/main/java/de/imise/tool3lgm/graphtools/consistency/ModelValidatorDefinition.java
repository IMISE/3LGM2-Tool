 ...
 * Eigenschaftsdialog) zu lenken, als den Dialog des Elementes, bei dem bei
 * einer Edge ein Kardinalitätsfehler besteht, direkt den Reiter für diese
 * fehlerhafte Edge zu öffnen. <br>
 * ATTNENTION: This class will be instanciated by
 * {@link Constructor#newInstance(Object...)}. So don't make it abstract!
 *
 * @author AXS
 */
public class ModelValidatorDefinition implements MetaModelSpecific {
     ...
     * <ol>
     * <li>Wenn für den Fehler eine <code>ErrorSolution</code> gefunden wird,
     * die einen gültigen <code>MetaPath</code> beschreibt, über den ausgehend
     * vom Element des übergebenen Fehlers verbundene Elemente gefunden werden,
     * dann kommen genau diese verbundenen Elemente zurück.</li>
     * <li>Wenn die gleichen Vorbedingungen gelten, wie eben, aber keine
     * verbundenen Elemente gefunden werden, dann kommt <code>null</code>
     * zurück.</li>
     * <li>Wenn für den Fehler eine <code>ErrorSolution</code> gefunden wird,
     * diese aber keinen <code>MetaPath</code> enthält, so kommt eine Liste mit
     * dem ModelElement des Fehlers als einzigem Element zurück</li>
     * <li>Wenn für den Fehler keine <code>ErrorSolution</code> gefunden wurde,
     * kommt eine leere Liste zurück</li>
     * </ol>
     * Zusäztlich dazu wird auch <code>null</code> zurück gegeben, wenn der
     * übergebene Fehler selbst <code>null</code> ist. Das kann man aber vorher
     * ausschließen, so dass die eindeutige Unterscheidung der einzelnen
     * Fehlerarten möglich ist.
     *
     * @param error
     * @return
     */
     ...
     * verbundenen Element hinterlegt ist, das aber nicht erreichtbar ist, weil
     * auch die Verbindung zu diesem Element fehlt. Somit kann der Fehler
     * nirgends behoben werden und man kann das Element löschen. In Metamodell
     * 2.7 heißt das: Anwendungsbaustein-Konfigurationen ohne einen
     * Anwendungsbautein könnte man im Dialog der Aufgaben an der mit der
     ...
     * Verbindung, dann kann man die Konfiguration löschen. ACHTUNG:
     * MissingPathErrors werden hier doch nicht nach der obigen Beschreibung
     * behandelt, d.h. sie werden igoriert, da nicht klar ist, warum die über
     * einen längeren Pfad nicht vorhandenen Elemente fehlen, also an welcher
     * Stelle der Pfad unterbrochen ist und an welchen Stellen bzw. in welchen
     * Eigenschaftsdialogen welcher Elemente im Pfad man den den Fehler beheben
     * könnte. Die Aussage der MissingPathErrors bezieht sich immer nur auf die
     * letzte Kante im Pfad. Wenn aber davor schon etwas nicht verbunden ist,
     * kommt der Fehler auch und es gibt das Element überhaupt nicht, dessen
     * Eigenschaftsdialog zur Fehlerbehebung man öffnen sollte. Deswegen darf
     * das als fehlerhaft geltende Element aber trotzdem nicht einfach gelöscht
     * werden.
     */
