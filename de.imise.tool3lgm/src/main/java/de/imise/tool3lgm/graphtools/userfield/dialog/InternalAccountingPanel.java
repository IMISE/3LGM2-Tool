...
/**
 * Panel mit dem für Kennzahlen ausgewählt werden kann, wie sie 
 * innerhalb einer Hierarchie - also elementartintern verrechnet
 * werden können.
 * 
 * Dieses Panel wird Moment nicht benutzt, da die interne Verrechnung in Hierarchien jetzt (erstmal) durch
 * die Definition der Formeln erledigt wird. 
 * 
 * @author AXS
 *
 */
public class InternalAccountingPanel extends AbstractInputPanel implements ActionListener{
	...
	private JRadioButton noAccountingBut = new JRadioButton(Tool3lgmConstants.getResString("non_accounting"));
	
	private JRadioButton sumAccountingBut = new JRadioButton(Tool3lgmConstants.getResString("summe"));
	
	private JRadioButton twsumAccountingBut = new JRadioButton(Tool3lgmConstants.getResString("teilwertsumme"));
	
	private AlphabeticalComboBox dirComboBox = new AlphabeticalComboBox();

	private AlphabeticalComboBox vgComboBox = new AlphabeticalComboBox();
	
	/**
	 * "Gleichverteilt" in der gewählten Loacle. Wird angezeigt, wenn als Verteilungsgewicht bei einer
	 * Verrechnung über die Teilwertsumme kein spezielles Verteilungsgewicht genutzt werden soll.
	 */
	 ...
	 */
	public InternalAccountingPanel(UserFieldDefinitions definitions, UserField userField) {
...
*/
	 ...
	 * 
	 * @return
	 * 		Richtung in der eine verrechnet werden soll
	 */
	...
	@Override
	public void commit() {
	/*	String selectedFuntion = getSelectedInternalFuntionName();
		if  (UserField.INTERNAL_ACCOUNTING_FUNCTION_SUM.equals(selectedFuntion)){
		...
		}else if (UserField.INTERNAL_ACCOUNTING_FUNCTION_TWSUM.equals(selectedFuntion)){
			userField.setInternalAccountingFunction(UserField.INTERNAL_ACCOUNTING_FUNCTION_TWSUM);
			userField.setInternalAccountingDirection(getSelectedInternalAccountingDirection());
			userField.setInternalAccountingWeightUserFieldHash(getSelectedInternalAccountingWeigthHash());
		}else {
			userField.setInternalAccountingFunction(null);
			userField.setInternalAccountingDirection(null);
			userField.setInternalAccountingWeightUserFieldHash(null);
		} 
	*/
	}
