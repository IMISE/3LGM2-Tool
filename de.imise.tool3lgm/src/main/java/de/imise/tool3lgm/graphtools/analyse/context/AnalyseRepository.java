 ...
 * AnalyseRepository verwenden, über Änderungen im Repository informiert.
 * 
...
public class AnalyseRepository  {
	 ...
	 */
	public static boolean addAnalyse(XMLAnalyse toadd) {
		if (toadd == null)
			return false;
		if (xmlAnalysen == null)
	 ...
	 * @param newAbfragen	eine ArrayList der neuen Abfragen. Jeder Eintrag des ArrayList ist eine XMLAnalyse.
	 */
	public static boolean setXMLAnalysen(List<XMLAnalyse> newXMLAnalysen) {
	 ...
	 */
	public static List<AbstractAnalyse> getAnalysenFuerKnoten(String elementClassName) {
	 ...
	 * @return false, wenn der Name in der Liste der Analysen gar nicht enthalten ist oder
	 * nur die einzeln übergebnen XMLAnalyse diesen Namen besitzt.
	 */
	public static boolean containsName(List<XMLAnalyse> analysen, XMLAnalyse xMLAnalyse, String name) {
		if (analysen == null) return false;
	 ...
	 * Ist bereits eine Datei geladen oder gespeichert worden, kommt diese zurück. Sonst wird
	 * der Reihe nach folgendes geprüft:<br>
	 * 
	 ...
	 * 
	 * Ex. diese Datei nicht, dann wird im Installationsverzeichnis des Bauskastens nach einer
	 * Datei mit dem Namen "Tool3lgm.analysis" gesucht. Wenn diese beschreibbar ist, wird sie 
	 * zurückgeliefert. Wenn sie nicht beschreibbar ist, dann wird diese Datei ins 
	 * Anwendungsdatenverzeichnis des Benutzers mit den Namen "Tool3lgm_Analys.res3" kopiert
	 * und zurückgegeben.<br>
	 * 
	 * Wenn auch im Installationsverzeichnis keine Analysendatei gefunden wurde, wird aus den
	 * Resourcen die Standarddatei geladen. Zuerst wird versucht, sie unter dem Namen "Tool3lgm.analysis"
	 * ins Installationsverzeichnis des Baukastens zu kopieren. Geht das gut, wird sie zurückgegeben.
	 * Geht das nicht gut, wird die Standarddatei ins Benutzerverzeichnis mit dem Namen 
	 * "Tool3lgm_LOCALECODE.analysis" kopiert und diese neue Datei zurückgegeben.<br>
	 * 
	 * Geht das auch nicht gut, wird die Original Resourcendatei zurückgegeben. Da sie in einem
	 * jar-Paket liegt, ist sie dann sicher nicht beschreibbar.
	 * 
	 * Der ganze Aufwand hängt damit zusammen, dass der Benutzer immer auf einer Datei arbeitet,
	 * die er auch möglichst beschreiben kann. Das Speichern der Analysendatei im Installationsverzeichnis
	 * hat den Vorteil, dass die Analysen allen Benutzern dieser Installation zu Verfügung stehen.
	 * @return
	 */
	public static File getRepositoryFile() {
		...
		if (file.exists()) {
			//die Datei aus dem Installationsverzeichnis ins Benutzerverzeichnis kopieren (lesen sollte
			//man file können,  sonst könnte man den Baukasten im gleichen Verzeichnis auch nicht ausführen)
			if (!file.canWrite()) {
		...
		//es ex. keine Analysendatei im Installationsverzeichnis -> Standardresourcendatei dahin oder, wenn
		//das nicht geht, ins Benutzerverzeichnis kopieren
		}else {
			try {
				//wenn es die Datei nicht im Installpfad gibt und man sie auch nicht dahin schreiben kann
	 ...
	 * @return
	 */
	public static List<XMLAnalyse> loadAnalyseFile(File file) {
		try {
	...
	/**
	 * Gibt eine Liste aller Analysen zurück, die sich in der Datei mit der übergebenen URL befinden.
	 * @param f
	 * @return
	 */
	public static List<XMLAnalyse> loadAnalyseFile(URL url) {
		String line = "";
	...
	/**
	 * Speichert die Analysen in die übergebene Analysedatei. Die übergebene Datei muss nicht existieren,
	 * aber erzeugbar und beschreibbar sein.
	 * @param f	die Datei, in die die Analysen gespeichert werden sollen.
	 */
	public static void saveAnalyseFile(File f, List<XMLAnalyse> analysen) {
