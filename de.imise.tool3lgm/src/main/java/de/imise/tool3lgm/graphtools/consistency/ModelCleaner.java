...
public class ModelCleaner {
	...
	private boolean PRINT_ERRORS = false;
	 ...
	 * @param gdcoll
	 */
	public ModelCleaner(GDCollection gdcoll) {
		super();
		this.gdcoll = gdcoll;
	}
	...
	public void cleanModel(){
		...
		//Es kann vorkommen, dass einige Kanten einen Container auf den Layern im Hauptmodell und einem 
		//Teilmodell haben, aber in ihrem ContainerTable nur ein Container für das Hauptmodell eingetragen
		//ist, der aber eigentlich der Container aus dem Teilmodell ist. Das ist absoluter Mist und wird hier berichtigt.
		for (GraphDocument doc : docs) {
			for (LayerContainer lc : doc.getLayers()) {
				...
				for (ElementContainer layerElemCont : allEc) {
					...
					//wenn der Container aus dem ContainerTable des Modellelements für das doc ein anderer ist als der auf dem Layer des docs
					...
					if (doc instanceof Szenario) {
						//wenn das Element zwar in einem Teilmodell, aber nicht im Gesamtmodell vorkommt -> Container im 
						//Gesamtmodell erzeugen und adden (dieser Fehler trat noch nie auf, aber sicher ist sicher)
						...
						//falls das Element unique ist, aber aus irgendwelchen Gründen auch in einem Teilmodell einen Container hat -> im Teilmodell löschen
						//(dieser Fehler sollte eigentlich unkritisch sein und trat im UKL-KIS-Modell bei PdvbPdvbkVerbindung, OrgAufOrgVerbindung, 
		...
		//Jetzt ist sicher gestellt, dass alle Container, die bei den Layern eingetragen sind auch bei allen Elementen eingetragen sind.
		//Nun für alle Modellelemente alle ihre Container überprüfen, ob es diese überhaupt geben darf und wenn ja, ob sie auch korrekt
		//auf den Layern eingetragen sind.
		//Dieser Fehler trat bisher noch nicht auf, aber es ist besser, das noch einmal explizit sicher zu stellen!
		...
		//Alle ABKonfigurationen löschen, die keiner AufOrgKombination zugeordnet sind. Davon gibt es in alten
...
//		removeInconsistentElements(gdcoll, ABKonfiguration.class, AufOrgKombination.class, null);
		...
		//Alle Knickpunkte löschen, die keiner Kante zugeordnet sind. So etwas trat in alten Modellen auf
		...
		for (int i = 0; i < gdcoll.getNumberOfSzenarios(); i++) {
			for (ElementContainer kc : gdcoll.getSzenario(i).getElementContainer(Knickpunkt.class)) {
				...
				// wenn der Owner null ist oder der Knickpunktcontainer nicht richtig in der KnickpunktContainerListe
...
//					System.err.println(gdcoll.getSzenario(i));
		...
		//Alle inkonsistenten Kanten löschen = alle Kanten, denen Start- oder Endelement fehlen oder die
		...
		for (int i = 0; i < edges.size(); i++) {
			...
			for (Kante edge2 : edge.getStart().getEdgesTo(edge.getEnd(), edge.getClass())) {
				if (edge != edge2) {
					...
					//edge2 befindet sich auf jeden Fall hinter edge in der Liste edges, sonst wäre vorher edge2 schon mal edge gewesen und edge
					//wäre dann edge2 gewesen. ALso kann man edge2 einfach hinten aus der Liste entfernen
					edges.remove(edge2);
				}
			}
		}
		...
		//die ganzen irgendwann beim Zusammenführen mal sinnlos reingekommenen LeerZeichen und Leeerzeilen
		//sowie das "-ZUSAMMENGEFÜHRT-" oder "-JOINED-" löschen
		final String[] superflousStrings = {"-ZUSAMMENGEFÜHRT-", "-JOINED-"};
		for (ModelElement me : gdcoll.getMainGraphDocument().getModelItems(ModelElement.class, true)) {
			...
			//alle Radiobuttons, Comboboxes und Kennzahlen, die beim Zusammenführen irgendwelche komischen Werte zusammengeführt bekommen haben,
			//wieder berichtigen
			...
			for (Class<? extends ModelElement> elementClass : ModelConstants.ALL_ELEMENTS_SET) {
				for (UserField uf : definitions.getUserFields(elementClass)) {
					//eigentlich haben Checkboxen keine Listenwerte, da sie nur true oder false für eine einzelne Box darsellen, aber falls
					//aus der einzelnen Checkbox mal eine ButtonGroup mit mehreren Checkboxes gemacht wird, haut das hier gleich hin
		...
		//Sicher stellen, dass bei allen berichtigten Kanten und Knickpunkten die Container richtig positioniert sind
		//also einfach nochmal pauzschal alle Kanten-Container initialisieren (das wird und muss bereits einmal nach
	 ...
	 * löscht entfernt alle "-ZUSAMMENGEFÜHRT-" Eintragungen, die im Grunde dieselben String trennen.
	 * Wenn dieselben Informationen mehrfach durch "-ZUSAMMENGEFÜHRT-" getrennt im übergebenen String stehen,
	 * dann kommt die Information nur noch einmal zurück. Wenn verschiedene String durch "-ZUSAMMENGEFÜHRT-"
	 * getrennt im Ausgangstring stehen, dann kommt der Ausgangstring zurück. (also das "-ZUSAMMENGEFÜHRT-"
	 * bleibt erhalten)
	 * 
	 * @ param sourceString
	 * 		String der bereinigt werden soll
	 * @param newDelimiter
	 * 		Yeichenkette, die als neuer Trenner im Rückgabestring eingebaut werden soll, wenn 2 verschiedene Zeichenketten
	 * 		duruch einen der 'alten' Delimiter aus delimiter getrennt waren
	 * @param delimiter
	 * 		Strings, die eventuell identische Tokens untereinander Aufteilen. Beim Zusammenführen von Elementen besteht der 
	 * 		Name evtl 2 Mal aus derselben Zeichenkette, die durch "-ZUSAMMENGEFÜHRT-" voneinander getrennt sind.
	 * 		
	 * @return
	 */
	public static final String getCleanString(String sourceString, String newDelimiter, String... delimiter) {
		//alle Leerzeichen und Zeilenumbrüche am Anfang und Ende entfernen
		sourceString = sourceString.trim();
		//Liste mit allen Tokens des Ausgangsstring, die durch "-ZUSAMMENGEFÜHRT-" voneinander getrennt im Ausgangsstring stehen
...
//		ArrayList<String> subStringDelimiterStringList = new ArrayList<String>();
		...
		//man muss immer nach dem deutschen "-ZUSAMMENGEFÜHRT-" und dem englischen "-JOINED-" suchen
		loop: for (int superflousStringIndex = 0; superflousStringIndex < delimiter.length; superflousStringIndex++) {
			...
			if (endIndex != -1) {
				...
				if (startIndex < endIndex) {
					//merke den aktuellen TeilString zwischen startIndex und dem Beginn von "-ZUSAMMENGEFÜHRT-"
					// in der Tokenliste
...
//					subStringDelimiterStringList.add(superflousStrings[superflousStringIndex]);
				...
				//(solange ein "-ZUSAMMENGEFÜHRT-" oder "-JOINED-" gefunden wurde, wiederholt er diese
		...
		if (subStringList.size() == 0)
			return "";
		//Der Ausgangsstring hatte gar kein "-ZUSAMMENGEFÜHRT-" oder "-JOINED-" in sich oder diese Zeichenketten
		...
		loop: for (int i = 0; i < subStringList.size(); i++) {
			...
			//der aktuelle Token muss dem RückgabeString angefügt werden, wenn davor schon was steht,
	...
	void removeInconsistentAWBConfigurationsWithoutAWB(boolean showResultDialog) {
	...
	/**
	 * Löscht alle <code>AufOrgKombination</code>en, die mit keiner <code>Aufgabe</code> oder 
	 * keiner <code>Organisationseinheit</code> verbunden sind.
	 *  
	 * @param showResultDialog wenn <code>true</code>, wird ein Dialog mit demErgebnis angezeigt
	 */
	void removeInconsistentAufOrgKombinations(boolean showResultDialog) {
		removeInconsistentElements(gdcoll, AufOrgKombination.class, Organisationseinheit.class, "clean_result_inconsistent_auforg");
	}

	/**
	 * Löscht alle inkonsitenten Physischen Datenverarbeitungsbaustein-Konfigurationen
	 * aus dem Gesamtmodell.<br>
	 * Inkonsitente Physischen Datenverarbeitungsbaustein-Konfigurationen sind mit keinem 
	 * physischen Datenverarbeitungsbaustein verbunden.
	 * 
	 * @param showResultDialog wenn <code>true</code>, wird ein Dialog mit demErgebnis angezeigt
	 */
	void removeInconsistentPDVBConfigurations(boolean showResultDialog) {
	...
	/**
	 * Hängt der Aufgabe eine AufOrgKombination</code> unter mit den gleichen <code>ABKonfiguration</code>en
	 * und AWB. Die <code>AufOrgKombination</code> und die <code>ABKonfiguration</code>en werden neu
	 * angelegt mit allen Assoziationen, die sie zu anderen Elementen haben.
	 * @param gdcoll
	 * @param auf
	 * @param aufOrg
	 */
	private static final void cloneConfig(GDCollection gdcoll, Aufgabe auf, AufOrgKombination aufOrg, int transactionID) {
		//eine neue AufOrgKombi anlegen, diese mit der Aufgabe verbinden und Name und Beschreibung der alten AufOrgKombi
		...
		for (Kante kante : aufOrg.getEdges()) {
			...
			if (elemToConnect instanceof ABKonfiguration) {
				...
				//Alle Verbindungen der Konfiguration auch klonen (außer die zur Originalen AufOrgKombination)
			...
			//alle anderen Verbindungen der AufOrgKombination zu Organisationseinheiten sein ebenfalls klonen
	...
	private static final void cloneConfig(GDCollection gdcoll, Aufgabe auf, AufOrgKombination aufOrg, int transactionID) {
		//eine neue AufOrgKombi anlegen, diese mit der Aufgabe verbinden und Name und Beschreibung der alten AufOrgKombi
		//für die neue übernehmen
		ModelElement newAufOrg = gdcoll.createKnotenWithContainer(AufOrgKombination.class, aufOrg.getName(), transactionID).getElement();
		gdcoll.link(AufAufOrgVerbindung.class, auf, newAufOrg, transactionID);
		newAufOrg.setDescription(aufOrg.getDescription());
		...
		for (Kante kante : aufOrg.getEdges()) {
			...
			//jeden AWB neu angelegt werden
			if (elemToConnect instanceof ABKonfiguration) {
				ModelElement newKonfig = gdcoll.createKnotenWithContainer(ABKonfiguration.class, elemToConnect.getName(), transactionID).getElement();
				gdcoll.link(AwbkAufOrgVerbindung.class, newAufOrg, newKonfig, transactionID);
				newKonfig.setDescription(elemToConnect.getDescription());
				//Alle Verbindungen der Konfiguration auch klonen (außer die zur Originalen AufOrgKombination)
				for (Kante edge : elemToConnect.getEdges()) {
	...
	/**
	 * Prüft, ob die übergebene <code>Aufgabe</code> Konfigurationen besitzt, die sie an 
	 * vorhandene Teilaufgaben weitergeben kann. 
	 * @param auf
	 * @return
	 */
	public boolean hasCloneableConfigs(Aufgabe auf) {
	 ...
	 */
	public final void cloneConfigsToParts(Aufgabe auf) {
	 ...
	 * @param transactionId
	 */
	private boolean cloneConfigsToParts(Aufgabe auf, int transactionId) {
		//hole alle AufOrgKombinationen der Aufgabe
		ArrayList<ModelElement> aufOrgs = auf.getConnectedElementsByEdge(AufAufOrgVerbindung.class);
		//wenn die Aufgabe nichts zu vererben hat -> nächste Aufgabe
		if (aufOrgs.size()==0)
...
//			mainDoc.removeElements(hashesToDelete, transactionId);
	...
	/**
	 * Klont von Aufgaben sämtliche AufOrgKombinationen mit sämtlichen AWB-Konfigurationen
	 * an die Teilaufgaben und entfernt die Originale von der Oberaufgabe.
	 * 
	 * @param showResultDialog wenn <code>true</code>, wird ein Dialog mit demErgebnis angezeigt
	 */
	public final void copyAufOrgKombinationsToAufLeafs(boolean showResultDialog) {
		int transactionId = GraphDocument.createTransactionId();
	 ...
	 */
    @SuppressWarnings("unchecked")
	public static final void removeInconsistentElements(GDCollection gdcoll, Class<? extends ModelElement> searchElementClass, Class<? extends ModelElement> connectedElementClass, String resultStringKey) {
		@SuppressWarnings("rawtypes")
		Class[] connectedElementClasses = {connectedElementClass};
	...
	/**
	 * Löscht alle Elemente der Art <code>searchType</code> aus dem Modell <code>doc</code>, das keine
	 * Verbindung zu einem der in <code>connectedTypes</code> angegebenen Elementarten besitzt.<br>
	 * 
	 * Das Ergebnis wird ausgegeben, wenn ein nicht leerer <code>resultStringKey</code> angegeben wird,
	 * der aus den Resourcen einen String lädt, wlcher beschreibt, was gelöscht wurde. Die Anzahl der
	 * gelöschten Elemente wird diesem String in der Ausgabe vorangestellt.
	 * 
	 * @param doc
	 * @param searchElementClass
	 ...
	 * @param resultStringKey
	 */
	public static final void removeInconsistentElements(GDCollection gdcoll, Class<? extends ModelElement> searchElementClass, Class<? extends ModelElement>[] connectedElementClasses, String resultStringKey) {
	...
	/**
	 * Liefert alle Elemente der Art <code>searchType</code>, die im angegebenen Modell nicht 
	 * mit Elementen der Art <code>connectedTypes</code> verbunden sind.
	 * @param doc Modell in dem gesucht wird
	 * @param searchElementClass Art der Elemente, die gesucht werden sollen
	 * @param connectedTypes Arten der Elemente, mit denen die gesuchten Elemente nicht verbunden sein dürfen
	 * @return Liste aller gefundenen Elemente
	 */
	private static HashSet<ModelElement> getNotConnected(GraphDocument doc, Class<? extends ModelElement> searchElementClass, Class<? extends ModelElement>[] connectedElementClasses) {
