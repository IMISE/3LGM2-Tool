...
public class LayoutEditor extends JDialog implements ActionListener {
	...
	private NodeContainer[] knoten;
	private JButton[] form_trigger, farbe_trigger, font_trigger;
	private JPopupMenu farbe_menu, form_menu, font_menu;
	private JMenuItem[] farbe, form, name;
	private Mapping my_mapping;
	private Insets insets;
	private int wieviele = 0, offset, c, counter, akt_x, akt_y, aktuelles = -1;
	private JButton uebernehmen, abbrechen, beenden;
	private GraphDocument doc, mydoc;
	private int x_abstand = 160, y_abstand = 140;
	...
	public LayoutEditor(JFrame f, GraphDocument document) {
		super(f);
		...
		wieviele = ModelConstants.ALL_NODES_SET.size() - GraphViewConstants.getUnpaintableCount() + 4; // Sicherheit geht vor!
	 ...
	 * @param z
	 */
	private void setAktuelles(int z) {
		aktuelles = z;
	}
