...
public class TableToolBar extends UnfloatableToolBar implements ChangeListener, ActionListener {
	 ...
	 */
	private JLabel colElementLabel;
	...
	/**
	 * Selektor, der die Comboboxen und die Metapfadauswahlliste zur Festlegung des
	 * darzustellenden Metapfades bereitstellt
	 */
	private MetaPathSelector metaPathSelector;
	
	/**
	 * Chekcbox über die eingestellt werden kann, ob nur absolute Kindelemente (also Elemente ohne eigene Teilelemente)
	 * angezeigt werden soll. Diese Box ist disabled, wenn weder in den Zeilen noch in den Spalten Elemente angezeigt
	 * werden, die in Teil-Von-Beziehung stehen können. 
	 */
	private JCheckBox showPartsOnlyCheckBox;

	/**
	 * Über diesen <code>boolean</code> wird sich die zuletzt gewählte Option aller geöffneten
	 * <code>showPartsOnlyCheckBox</code>es gemerkt, so dass eine neue Checkbox gleich mit diesem
	 ...
	 */
	private TableInternalFrame controlledFrame;
	
	/**
	 * @param controlledFrame
	 * 		Frame dessen Darstellung durch diese Toolbar beeinfluss wird.
	 */
	public TableToolBar(TableInternalFrame controlledFrame) {
		super();
		...
		JComponent[] child1 = {rowLabel, metaPathSelector.getClass1ComboBox(), colLabel, metaPathSelector.getClass2ComboBox(), null, showPartsOnlyCheckBox};
		setGridBagLayout(choicePanel, child1, 2);

		JComponent[] child2 = { new JLabel(Tool3lgmConstants.getResString("zeile") + ":"), rowElementLabel, new JLabel(Tool3lgmConstants.getResString("spalte") + ":"), colElementLabel };
	 ...
	 */
	private static final GridBagConstraints setGridBagLayout(JComponent owner, JComponent[] childs, int columns) {
		...
		for (int i = 0; i < childs.length; i++) {
			...
			/*
			 * erste Spalte immer so klein wie möglich aller anderen können
			 * gestreckt werden
			 */
			if (i % columns == 0)
				constraints.weightx = 0;
	...
	/**
	 * Aktualisiert die Matrix und die Einträge in der Legende (gewählter Pfad und 
	 * Bedeutung der Markierungen in der Matrix)
	 */
	private void update() {
		//Tabelle updaten
	...
	/**
	 * Aktualisiert die Positionslabels, die die Elemente der Zeile und Spalte darstellen.
	 * (Wird aufgerufen, wenn sich die Mausposition in der Matrix ändert)
	 * 
	 * @param col
	 ...
	 */
	public void positionChanged(ModelElement col, ModelElement row) {
	...
	public void stateChanged(ChangeEvent e) {
		if (e.getSource()==metaPathSelector.getClass2ComboBox()){
			...
			if (metaPathSelector.getSelectableMetaPathes()!=null && metaPathSelector.getSelectableMetaPathes().length>1){
				Object[] msg = {Tool3lgmConstants.getResString("text_path"), metaPathSelector.getMetaPathJList()};
