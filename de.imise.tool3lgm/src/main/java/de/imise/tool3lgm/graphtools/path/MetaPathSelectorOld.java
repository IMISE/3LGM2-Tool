 ...
 * @create 13.11.2007
 */
public class MetaPathSelectorOld implements ActionListener {
     ...
     */
    private Collection<AbstractMetaPath> selectableMetaPathes;
     ...
     */
    private static MetaPathSelectorOld dialogMetaPathSelecor;

    /**
     *
     */
    private MetaPathSelectorOld() {
        super();

        class1ComboBox = new AlphabeticalComboBox();
        for (Class<? extends ModelElement> elementClass : PathFinderOld.getElementClassesInPathes()) {
     ...
     */
    public static final MetaPathSelectorOld createComponents() {
        return new MetaPathSelectorOld();
    }
    ...
    public void actionPerformed(final ActionEvent e) {
        if (e.getSource() == class1ComboBox) {
            ...
            for (Class<? extends ModelElement> elementClass : PathFinderOld.getElementClassesInPathes()) {
                if (!PathFinderOld.getMetaPathes(elementClass, class1BoxSelection).isEmpty()) {
            ...
            metaPathJList.removeAllElements();
            selectableMetaPathes = PathFinderOld.getMetaPathes(class1BoxSelection, class2BoxSelection);
            if (!selectableMetaPathes.isEmpty()) {
                for (AbstractMetaPath selectableMetaPath : selectableMetaPathes) {
                    metaPathJList.addItem(selectableMetaPath, selectableMetaPath.getFullName());
                }
     ...
     */
    public static final MetaPathSelectorOld showDialog() {
     ...
     * @return einen <code>MetaPathSelector</code> der durch die Einträge aus einem angezeigten Dialog gefüllt wurde
     */
    public static final MetaPathSelectorOld showDialog(final String class1Label, final String class2Label, final String metaPathListLabel) {
        if (dialogMetaPathSelecor == null) {
            dialogMetaPathSelecor = new MetaPathSelectorOld();
        }
     ...
     */
    public MetaPathOld getSelectedMetaPath() {
        Object o = metaPathJList.getSelectedObject();
        if (o != null) {
            return (MetaPathOld) o;
        }
        return null;
    }
     ...
     */
    public Collection<AbstractMetaPath> getSelectableMetaPathes() {
