...
/**
 * Oberklasse für alle Klassen, denen man benutzerdefinierte 
 * Eigenschaften geben kann.
 * 
 * Hinweis: Beim Löschen eines UserFields bleibt sein Hash und Value-Paar in der HashMap. Nimmt man das Löschen zurück, 
 * 			indem man im <code>UserFieldDeclarationDialog</code> auf abbrechen geht, wird die beim Öffnen des Dialogs
 * 			erstellte Kopie der <code>UserFieldDefinitions</code> zurück gesetzt und alles ist wie vorher. 
 * 
 * 
 * @author AXS
 *
...
public class UserFieldTarget implements Cloneable {
	...
	/**
	 * Mappt von den für das Modell definierten <code>UserField</code>s
	 * auf die vom Benutzer eingegebenen Werte.
	 */
	private HashMap<UserField, String> userFieldToInputValuesMap = null;
	
	/**
	 * Mappt von den für das Modell definierten <code>UserField</code>s
	 * auf die berechneten Werte. 
	 * Die Map wird nur initialisiert, wenn das <code>UserFieldTarget</code> mind. einen
	 * berechneten Wert hat.
	 */
...
//			retVal.setUserFieldInputValue(key, new String(getUserFieldInputValue(key)));
	...
	/**
	 * Liefert den vom Benutzer für das <code>UserField</code> mit dem übergebenen HashCode
	 * eingegebenen Wert. Wurde noch keiner eingegeben oder gehört das betreffende <code>UserField</code>
	 * gar nicht zu diesem <code>UserFieldTarget</code>, kommt <code>null</code> zurück.
	 * 
	 * @param userField
	 * @return
	 * 		den eingebenen Wert oder <code>UserField.EMPTY_STRING</code>
	 * 
	 */
	public String getUserFieldInputValue(UserField userField) {
	 ...
	 * @param value
	 */
	public void setUserFieldInputValue(UserField userField, String value) {
		...
		if ((value == null) || 
			...
			(value.equalsIgnoreCase("__3LGM_DELETE__")))
			return;
		if (value.length()>0)
	 ...
	 * @return
	 */
	public Object removeUserField(UserField userField) {
	...
	/**
	 * Hängt an den übergebenen <code>StringBuilder</code> für jedes <code>UserField</code>
	 * einen XML-Eintrag an.
	 * @param sb <code>StringBuilder</code>, an den die Einträge gehängt werden
	 */
	protected void appendUserFieldXMLString(StringBuilder sb) {
	...
	/**
	 * Liefert den berechneten Wert eines <code>UserField</code>s, wenn es ihn gibt. Gibt es
	 * ihn nicht, weil er noch nicht berechnet wurde oder das userField gar nicht zu dieser
	 * Klasse gehört, kommt <code>null</code> zurück.
	 * 
	 * ACHTUNG: Diese Funktion sollte nur die Klasse <code>UserField</code> aufrufen. Kein anderer!
	 * Möchte man den berechneten oder eingegebenen Wert eines <code>UserField</code>s für ein Element
	 * abfragen, sollte das immer über die Funktion <code>UserField.getValue(ModelElement)</code> 
	 * geschehen, da diese ggf. die Neuberechnung von Kennzahlformeln anstößt.
	 * 
	 * @param userField
	 * 		<code>UserField</code> für das der berechnete Wert dieses Elementes zurück gegeben werden soll
	 * @see #_getUserFieldInputValue(String)
	 * @see #_getUserFieldInputValue(UserField)
	 * @see UserField#getValue(ModelElement)
	 * 
	 * @return 
	 * 		den für den übergebenen HashString eines Formel-UserFields vermerkten berechenten Wert oder
	 * 		<code>EMPTY_STRING</code>, wenn kein Wert vermerk ist
	 * 	 
	 * 
	 */
	protected String getCalculatedUserFieldValue(UserField userField) {
	 ...
	 */
	protected void setCalculatedUserFieldValue(UserField userField, String value) {
		if (userFieldToCalculatedValuesMap==null)
