...
public class TransactionManager {
	 ...
	 */
	private ArrayList<TransactionListener> listenerList = new ArrayList<TransactionListener>(3);

	/**
	 * Liste aller durchgeführten Transaktionen, die sich auch rückgängig machen
	 * lassen sollen.
	 */
	private Transaction trans_q[];
	 ...
	 * Diese Konstante ist nachträglich eingeführt worden um die Magic-Number 0,
	 * die an vielen Stellen im Code als Prozess-ID aufgetaucht ist, zu
	 * beseitigen. In welchen Fällen man 0 als Prozess-ID übergeben darf, ist
	 * nicht ganz klar.
	 */
	 ...
	 * @param doc
	 */
	private void startTransaction(int pid, GraphDocument doc) {
		if (is_doing)
			return;
	 ...
	 * @param doc
	 */
	public final void startTransaction(String undoCommand, String redoCommand, int pid, GraphDocument doc) {
		if (is_doing)
			return;
	 ...
	 * 
	 * @param command
	 *            hinzuzufügendes Kommando
	 * @param pid
	 *            ID der Transaktion
	 * @param undo
	 *            Wenn <code>true</code> wird das Kommando als Undo-Kommando
	 *            hinzugefügt, sonst als Redo-Kommando
	 * 
	 */
	private final void addUndoOrRedoCommand(String command, int pid, boolean undo) {
	 ...
	 * @param pid
	 */
	public final void addRedoCommand(String command, int pid) {
	 ...
	 * @param pid
	 */
	public final void addUndoCommand(String command, int pid) {
	 ...
	 * jeden Zwischenschritt zu specihern. Für das Redo benötigt man immer nur
	 ...
	 * demselben <code>commandpre</code> in dieser Trasaktion vorkommt.
	 * 
	 * 
	 * Diese Funktion ist gedacht, um z. B. bei Verschiebeoperationen nicht
	 ...
	 * @param pid
	 */
	public final void addUndoCommandIfNotExist(String commandPrefix, String commandArguments, int pid) {
		commandPrefix = getValidCommand(commandPrefix);
		if (commandPrefix == null)
			return;
		commandArguments = commandArguments.trim();
	 ...
	 * @param index
	 */
	private final void destroyTransaction(int index) {
		if (is_doing)
			return;
	 ...
	 * @param pid
	 */
	public final void finishTransaction(int pid) {
		if (is_doing)
			return;
		int j = getTransactionIndexForPID(pid, true);
		if (j == INVALID_POS)
	 ...
	 * @param pid
	 */
	public final void addPreSelectionItem(String string, int pid) {
		if (is_doing)
			return;
		int j = getTransactionIndexForPID(pid, true);
		if (j != INVALID_POS)
//			System.err.println("addPreSelectionItem " + string);
	 ...
	 * @param pid
	 */
	public final void addPostSelectionItem(String string, int pid) {
		if (is_doing)
			return;
		int j = getTransactionIndexForPID(pid, true);
		if (j != INVALID_POS)
	 ...
	 * 
	 * @param pid
	 *            ID der Transaktion, die gesucht werden soll. Wird
	 *            <code>UNSPECIFIC_PID</code> übergeben, kommt der Index der
	 *            letzten Transaktion zurück.
	 * @param undo
	 *            bei <code>true</code> werden inklusive des aktuellen
	 ...
	 */
	private int getTransactionIndexForPID(int pid, boolean undo) {
	 ...
	 * @return
	 */
	public final boolean undo(int pid) {
		if (is_doing)
			return true;
		if (!isUndoAvailable())
			return true;
	 ...
	 * @return
	 */
	public final boolean redo(int pid) {
		if (is_doing)
			return true;
		if (!isRedoAvailable())
			return true;
...
//			System.err.println("12345AXS " + trans_q[j].getRedoCommand(i2));
	 ...
	 * 
	 * @param entryCount
	 *            Anzahl der Elemente ab dem ersten, die ausgegeben werden
	 *            sollen
	 */
	public final void printQueue(int entryCount) {
	 ...
	 */
	public final String getQueue(int entryCount) {
	...
	/**
	 * Liefert <code>true</code>, wenn wenigstens eine offene Transaktion
	 * existiert.
	 * 
	 * @return
	 * 		<code>true</code>, wenn eine Transaktion offen ist
	 */
