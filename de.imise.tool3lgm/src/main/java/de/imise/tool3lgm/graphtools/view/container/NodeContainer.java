...
public class NodeContainer extends ElementContainer/* implements GraphDocumentListener, InTransactionListener*/ {
	 ...
	 * @param gd
	 */
	public NodeContainer(Knoten neu, GraphDocument gd) {
		super(neu, gd);
		init();
	}
	 ...
	 * @param gd
	 */
	public NodeContainer(Knoten neu, GraphElementLayout l, GraphDocument gd) {
		super(neu, l, gd);
		init();
	}
	 ...
	 * @param gd
	 */
	public NodeContainer(NodeContainer alt, GraphDocument gd) {
		super(alt, gd);
		init();
	}
...
//			specialInfoTargets.addAll(me.getConnectedContainer(Aufgabe.class, doc, null, Doppelkante.FORWARD, false));
	 ...
	 * @param h
	 */
	public final void setCoordinates(int x, int y, int w, int h) {
		if (layout == null)
			return;
		setLocation(x, y);
		setSize(w, h);
...
*/		
	 ...
	 * @param ImageTable
	 */
	public void setIcon(String name, Hashtable<String, byte[]> ImageTable) {
		if (me.isUnpaintable())
			return;
...
//		System.err.println(w + " " + h);
	 ...
	 * @param h
	 */
	public final void setSizeForButtons(int w, int h) {
		if (me.isUnpaintable())
			return;
		layout.width = w;
		layout.height = h;
...
//	}
	...
	/* (non-Javadoc)
	 * @see tool3lgm.graphtools.view.container.ElementContainer#set3LGMLayout(tool3lgm.graphtools.view.graph.GraphElementLayout)
	 */
	@Override
	 ...
	 * @return
	 */
	public boolean hasParent(GraphDocument doc) {
		return me.hasDirectParentContainer(doc);
	}
...
*/		
	 ...
	 * @param g
	 */
	public final void paintSuperComponent(Graphics g){
	 ...
	 * @param node
	 */
	public void setTreeNode(LGMTreeNode node) {
		treeNode = node;
	}
...
//### Ende GraphDocumentListener #######################################################################################
	 ...
	 * @param linked
	 */
	public void linkWithSzenario(boolean linked) {
		if (linked) {
			if (linkLabel == null) {
...
//			setToolTipText("asfasfjhasf");
		}
	}

	/* (non-Javadoc)
	 * @see tool3lgm.graphtools.view.container.ElementContainer#toString()
	 */
	@Override
	...
	public void setAdditionalTextRightDown(String string) {
		if (string==null){
