...
public class SearchDialog extends JDialog implements ActionListener, ListSelectionListener, WindowListener, ItemListener {
	 ...
	 */
	private JCheckBox elementName_cb = new JCheckBox(Tool3lgmConstants.getResString("SEARCH_DIALOG_USERFIELD_CBText"), SearchDialog.ignoreCaseInName);
	
	/**
	 * Checkbox für ignore case Beschreibung
	 */
	private JCheckBox elementDescription_cb = new JCheckBox(Tool3lgmConstants.getResString("SEARCH_DIALOG_USERFIELD_CBText"), SearchDialog.ignoreCaseInDescription);
	
	/**
	 * Checkbox für ignore case Benutzerdef Eigenschaften
	 */
	private JCheckBox elementUserField_cb = new JCheckBox(Tool3lgmConstants.getResString("SEARCH_DIALOG_USERFIELD_CBText"), SearchDialog.ignoreCaseInUserField);
	 ...
	 */
	private AlphabeticalComboBox elementClassBox = new AlphabeticalComboBox();
	
	/**
	 * Combobox Modell
	 */
	private AlphabeticalComboBox modelBox = new AlphabeticalComboBox();
	
	/**
	 * Combobox Teilmodell
	 */
	private AlphabeticalComboBox subModelBox = new AlphabeticalComboBox();
	
	/**
	 * Suchknopf
	 */
	private JButton searchButton = new JButton(Tool3lgmConstants.getResString("SEARCH_DIALOG_USERFIELD_Button"));
	...
	/**
	 * Die zentrale Suchmethode die aufgerufen wird.
	 * Prinzip: 
	 * Alle Elemente des Teilmodels landen in <code>searchSet</code>
	 * Nicht erfüllte Suchkriterium werden herausgefiltert mittels <code>searchSet.remove</code>
	 * @param e - übergebener ActionEvent
	 */
	private void callSearch(ActionEvent e) {
		...
		// beim aufruf des fensters nicht suchen (listener feuern aber beim öffnen des fensters bereits)
		...
		for (int i = searchSet.size() - 1; i >= 0; i--) {
			...
			// re3 muss gesetzt werden, sonst wird nie etwas removed, wenn z.b. auf checkboxen eingeschränkt wird
				...
				if (me.getUserFieldInputValueKeys().size() == 0)
					...
					for (UserField key : me.getUserFieldInputValueKeys()) {
						...
						if ((userFieldTypeComboBox.getSelectedObject()).equals(Tool3lgmConstants.getResString("SEARCH_DIALOG_USERFIELD_all")) || ((UserField.Style) userFieldTypeComboBox.getSelectedObject()).equals(UserField.Style.CHECK_BOX)
							...
							boolean nameOfCheckBoxMatched = false;// (+ zusätzlich muss label stimmen)
	 ...
	 * @return DefaultTableModel
	 */
	private JTable initialiseTable(DefaultTableModel mod) {
		...
		// -> wenn 0: nur id; wenn 1: name switchen, modell bleibt wie vorher, wenn 2: modell switchen, name bleibt wie vorher;
