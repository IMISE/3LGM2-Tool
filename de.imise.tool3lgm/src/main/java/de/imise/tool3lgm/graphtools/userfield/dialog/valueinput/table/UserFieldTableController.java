 ...
 * <p>
 * Über die statischen Methoden können vorgefertigte Instanzen dieser Klasse abgerufen werden, die auf Tabels für primäre und sekundäre Kennzahlen, Verteilungsgewicht oder Modelvariablen zugeschnitten
 * sind.
 * 
 * @author fstephan
 */
public abstract class UserFieldTableController {
	...
	/**
	 * Ändert und überwacht {@link #selectionChangeMode} in Abhängigkeit davon, ob Shift oder Strg gedrückt wurde bzw. ein MouseDragging erfolgt ist. Damit wird das korrekte Selektionsverhalten dieses
	...
	/**
	 * Array, das für jede Zelle wiedergibt, ob ihre Selektion durch die Methoden {@link #clearRangeSelections()} und {@link #clearSingleSelections()} aufgehoben werden darf.
	 */
	 ...
	 */
	private UserFieldTableController(boolean[][] editMatrix, boolean[][] initialSelectionMatrix) {
	 ...
	 */
	boolean isLeadingPoint(Point p) {
	 ...
	 * @param table
	 */
	void enableMultipleSelection(UserFieldTable table) {
	...
	/**
	 * Gibt, wenn die aktuelle Selektion zusammenhängend ist, ein Punkte-Paar wieder. Der erste Punkt repräsentiert dabei die markierte Zelle mit dem kleinsten Zeilen- und Spaltenindex und der zweite
	 * Punkt die mit dem größten Zeilen- und Spaltenindex. <br>
	 * Ist die Selektion nicht zusammenhängend, wird <code>null</code> zurückgegeben.
	 */
	Pair<Point, Point> getCoherentlySelection() {

		if (!isMultipleSelectionEnabled) // keine Mehrfachselektion aktiviert
		...
		if (maxRow == -1) // keine Zelle markiert
			return null;
	 ...
	 * @param col
	 */
	private void shiftModeSelectionChange(int row, int col) {
	...
	/**
	 * Hält bisherige Selektionen fest und beginnt eine neue, indem der {@link #anchorPoint} auf den gerade ausgewählten Punkt (<code>row</code>,<code>col</code>) gesetzt wird.
	 * 
	 * @param row
	 * @param col
	 */
	private void controlModeSelectionChange(int row, int col) {
	 ...
	 * @param col
	 */
	private void defaultModeSelectionChange(int row, int col) {
		anchorPoint = new Point(row, col);
		leadingPoint = new Point(row, col);
		clearAllSelections();
	...
	/**
	 * Löscht alle durch das Keyboard und MouseDragging erzeugten Selektionen, bis auf die, die durch {@link #ignoreClearingMatrix} davor geschütz sind.
	 */
	 ...
	 * @param table
	 */
	void removeFrom(UserFieldTable table) {
		disableMultipleSelection(table);
	}
	 ...
	 * @return
	 */
	public static UserFieldTableController getNewUserFieldTableController(boolean[][] editMatrix) {
		return new UserFieldTableController(editMatrix, new boolean[editMatrix.length][editMatrix[0].length]) {
			...
			public boolean isEditable(int row, int col) {
				return this.editMatrix[row][col];
			}
		};
	}
	 ...
	 * @param uftm
	 */
	public static UserFieldTableController getNewClassificationNumberTableController(UserFieldTableModel uftm) {
	...
	/**
	 * Methode gibt eine <code>TableEditCondition</code> für Tables, die Verteilungsgewichte enthalten und <code>uftm</code> als Model besitzen zurück.
	 * 
	 * @param uftm
	 */
	public static UserFieldTableController getNewDistributionWeightTableController(UserFieldTableModel uftm) {
	...
	/**
	 * Methode gibt eine <code>TableEditCondition</code> für Tables, die berechnete Kennzahlen enthalten und <code>uftm</code> als Model besitzen zurück.
	 * 
	 * @param uftm
	 */
	public static UserFieldTableController getNewClassificationNumberFormulaTableController(UserFieldTableModel uftm) {
	 ...
	 * @param uftm
	 */
	public static UserFieldTableController getNewModelVariableTableController(UserFieldTableModel uftm) {
	...
	/**
	 * Methode gibt in Abhängigkeit der übergebenen Parameter eine neue Instanz dieser Klasse wieder: <code>uftm</code> ist das zum Table gehörige Model <code>firstRowEditable</code> gibt an, ob die
	 * oberste Reihe editierbar sein soll <code>firstColumnEditable</code> gibt an, ob die linke Spalte editierbar sein soll <code>dependsOnModelValue</code> gibt an, ob die Editierbarkeit der Zellen
	 * zusätzlich noch vom ModelValue der Zellen abhängen soll
	 * 
	 * @param uftm
	 ...
	 */
	private static UserFieldTableController getNewUserFieldTableController(final UserFieldTableModel uftm, boolean firstRowEditable, boolean firstColumnEditable, final boolean dependsOnModelValue) {
	...
	private class SelectionModeHandler extends MouseAdapter implements KeyListener {
		 ...
		 *            zu überwachender Table
		 */
		public SelectionModeHandler(UserFieldTable table) {
			this.table = table;
		}
		...
		public void keyPressed(final KeyEvent e) {
			...
			if (isDragging == true) // Falls ein MouseDragging erfolgt, werden KeyEvents ignoriert
				return;
			...
			if (e.isControlDown()) { // Strg gedrückt
				...
				if (e.isShiftDown()) // nur Shift gedrückt
					selectionChangeMode = SHIFT_MODE;
			}
		}
		 ...
		 * @param e
		 */
		private final void setSelectionChangeMode(InputEvent e) {
			if (e.isControlDown()) {
				if (e.isShiftDown())
					selectionChangeMode = DEFAULT_MODE;
				else
	 ...
	 * Ist nur eine Zelle markiert, wird nach Drücken von Enter die Markierung um Eins nach unten verschoben, bzw. in die nächste Spalte, falls das Spaltendende überschritten wurde.
	 * <p>
	...
	private class KeySelectionChangeHandler extends KeyAdapter {
		 ...
		 *            zu überwachender Table
		 */
		public KeySelectionChangeHandler(UserFieldTable table) {
			this.table = table;
		}
		...
		public void keyPressed(KeyEvent e) {
			...
			if (isArrowOrEnterPressed(e)) {// Enter oder Pfeiltaste wurde gedrückt
				...
				if (isEnterPressed(e) && hasMultipleSelection()) // mehrfache Selektion + Enter -> setze Markierung um 1 zurück
				...
				return;
			} else if (e.isControlDown() && e.getKeyCode() == KeyEvent.VK_A) // Alles markieren, bei Strg-A
				selectAll();
		}
		 ...
		 * @return
		 */
		private boolean isArrowOrEnterPressed(KeyEvent e) {
		 ...
		 * @return
		 */
		private boolean isArrowPressed(KeyEvent e) {
			int k = e.getKeyCode();
	...
	/**
	 * Listener, der die Selektion im <code>table</code> auch dann der Mouse folgen lässt, wenn sich der Cursor über dem <code>rowHeader</code> befindet.
	 * 
	 * @author fstephan
	 */
	private class RowHeaderSelectionListener implements MouseMotionListener {
		 ...
		 * @param rowHeader
		 */
		public RowHeaderSelectionListener(UserFieldTable table, JList rowHeader) {
			this.table = table;
