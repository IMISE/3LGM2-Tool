...
public class ModelPropertyDialogOld extends PropertyDialog {
     ...
     */
    public ModelPropertyDialogOld(final GDCollection gdcoll) throws HeadlessException {
        super(gdcoll);
    ...
    public boolean removeTab(final GraphDocument doc) {
    ...
    public void addTab(final GraphDocument doc) {
        scrollPane = new MyScrollPane(doc);
        tabbedPane.add(scrollPane);
    }

    public void renameTab(final GraphDocument doc) {
        if (removeTab(doc)) {
            addTab(doc);
        }
    }
    ...
    public void actualizeFrameTitle() {
        if (tabbedPane.getSelectedIndex() > 0) {
            setTitle(Tool3lgmConstants.getResString("description") + " - " + gdcoll.getName() + " - " + gdcoll.getSzenario(tabbedPane.getSelectedIndex() - 1).getTitle());
        } else {
            setTitle(Tool3lgmConstants.getResString("description") + " - " + gdcoll.getName() + " - " + Tool3lgmConstants.getResString("uebersicht"));
        }
    }

    void tabbedPane_stateChanged(final ChangeEvent e) {
        actualizeFrameTitle();
        lastActiveDoc.setDescription(textPane.getText());
    ...
    private class MyScrollPane extends JScrollPane {
        ...
        private MyScrollPane(final GraphDocument doc) {
            super();
            this.doc = doc;
        }
