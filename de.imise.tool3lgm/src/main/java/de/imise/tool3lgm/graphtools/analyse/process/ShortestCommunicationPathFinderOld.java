...
public class ShortestCommunicationPathFinderOld {
	...
	/** 
	 * Kostenmatrix für die Kommuniaktionswegberechnungen
	 * Indizes: 1. Objekttyp, 2. Zeilenschnittstelle, 3. Spaltenschnittstelle<br>
	 * Wert: Kosten, die Kommunikation von Zeilenschnittstelle zu Spaltenschnittstelle verursacht. Gibt
	 * es keine Kommunikationsweg, sind die Kosten <code>INFINITY</code> 
	 */
	private int [][][] costMatrix;

	/** 
	 * Pfadmatrix für die Kommuniaktionswegberechnungen
	 * Indizes: 1. Objekttyp, 2. Zeilenschnittstelle, 3. Spaltenschnittstelle<br>
	 * Wert: Pfad über alle Schnittstellen von Zeilenschnittstelle zu Spaltenschnittstelle, der
	 * durchlaufen werden muss, um den Objekttyp zu übgertragen (also Liste von Schnittstellen,
	 * die nacheinander durchlaufen werden müssen)
	 */
	private ArrayList<ModelElement> [][][] pathMatrix;
	
	/** 
	 * Mapppt von einem Anwendungsbausteins auf eine Liste aller seiner
	 ...
	 * 
	 * Dies wird immer ein Hauptdokument, da die vollständige XMLAnalyse nur im Hauptdokument
	 * sinnvoll ist.
	 */
	...
	/**
	 * @param objekttypList
	 * 		Liste von Objekttypen, deren Kommunizierbarkeit untersucht werden soll
	 */
	public ShortestCommunicationPathFinderOld(ArrayList<ModelElement> objekttypList, GDCollection gdcoll) {
		super();
		if (objekttypList!=null && objekttypList.size()>0){
			mainDoc = gdcoll.getMainGraphDocument();
			this.objectTypes = objekttypList;
	 ...
	 * 		Liste von Objekttypen, deren Kommunizierbarkeit untersucht werden soll
	 */
	public ShortestCommunicationPathFinderOld(ArrayList<ModelElement> objekttypList, ModelAnalyzerCache analyzerCache) {
		super();
		if (objekttypList!=null && objekttypList.size()>0){
...
//		printAllSchnittstellenOfAllBaustein();
	...
	/**
	 * Liefert alle Anwendungsbausteine, die mit dem übergebenen in irgendeiner Teil-Von-Beziehung stehen.
	 * 
	 * @param awb
	 * @return
	 */
	public ArrayList<ModelElement> getEqualsAWB(Anwendungsbaustein awb){
		return awbToSameAWBList.get(awb);
	}

	
	/**
	 * Prüft, ob die Schnittstelle eine Kommunikationsbeziehunge besitzt, über die
	 * irgendein Objekttyp gesendet oder empfangen wird.
	 * @param bss
	 * @return
	 * 		<code>true</code> wenn die Schnittstelle nicht kommunizieren kann
	 */
	public boolean isUseless(Bausteinschnittstelle bss){
	 ...
	 * 
	 * @param o
	 * 		Index des Objekttyps
	 * @param z
	 * 		Index der Sendeschnittstelle
	 * @param s
	 * 		Index der Empfängerschnittstelle
	 * @return
	 */
	private boolean connected(int o,int z, int s) {
...
//	static long fullFloydTime=0l;
	 ...
	 */
	private void findShortestPath(int o) {
...
/* Das hier ist noch falsch, weil jetzt auch OTs rausfallen, deren Eltern Kommuniziert werden können		
		//alle Objekttypen entfernen, die man nicht kommunizieren kann = die in keiner Repräsentationsform
...
/*
		...
		//alle Schnittstellen entfernen, die keine KommBez haben oder deren KommBezen nichts senden/empfangen
		loop: for (Iterator schnittstellenIt = schnittstellen.iterator(); schnittstellenIt.hasNext();){
...
//		System.err.println("ShortestCommunicationPathFinder.initPathAndCostMatrix()1: " + (end-start));


//		start = System.currentTimeMillis();
		init();
//		end = System.currentTimeMillis();
//		System.err.println("ShortestCommunicationPathFinder.initPathAndCostMatrix()2: " + (end-start));
		...
		//der Objekttyp von der ersten zur zweiten Schnittstelle geschickt werden kann. Wird ein Objekttyp mit
		//Teil-Objekttypen über eine Kommunikationsbeziehung geschickt, so gilt das auch für alle seine Teile
...
*/		
//		System.out.println("In dem Teilmodell " + doc.getTitle() + " gibt es " + allBssBssKanten.size() + " Kommunikationsbeziehungen.");
		

//		start = System.currentTimeMillis();

		//für jede Kommunikationsbeziehung = Kanten von einer Bss zu einer Bss (das sind alles Doppelkanten)
				...
				for (ModelElement nt : ntAndDtOfKante) {
					...
					for (ModelElement nachrichtentyp : ntOfEtnt) {
						...
						for (ModelElement objekttyp : objekttypes) {
							...
							//wenn in der Liste an der Stelle des entsprechenden OT noch kein Schnittstellenpaar steht
...
//		end = System.currentTimeMillis();
//		System.err.println("ShortestCommunicationPathFinder.initPathAndCostMatrix()3: " + (end-start));
...
//			System.out.println(objektTypes.get(o) + " ########################");
			...
			for (int z=0; z<N; z++){
				//wird die Liste aller Schnittstellen, die die jeweilige Schnittstelle mit OT erreichen kann
				...
				if  (sendReceiveSSListForOt!=null){
					...
					//2.)füge zu allConnectedFromSSWithOT alle Schnittstellen hinzu (wenn sie nicht schon drin sind), die mit ss 
					//		-auf dem selben Baustein liegen oder
					//		-auf einem seiner Unterbausteine oder Oberbausteine oder Geschwister
					//das jetzt folgende geht davon aus, dass eine Schnittstelle IMMER zu GENAU EINEM Baustein gehört
				...
				//Kostenmatrix erzeugen und Pfadmatrix initialisieren (der Zwischenschritt der Verbindungsmatrix ist hier implizit enthalten)
//				System.out.println(ss+ " kommt an " + al.size() + " Schnittstellen");
				for (int s=0; s<N; s++){

					//alle Schnittstellen, die die aktuelle Schnittstelle mit dem aktuellen OT erreichen kann 
			...
			//jetzt die vollständige Pfadmatrix erzeugen (hiernach stehen ist pathMatrix die Pfadmatrix und costMatrix die
			//Distanzmatrix)
...
//		System.err.println("ShortestCommunicationPathFinder.initPathAndCostMatrix()4: " + (end-start));
//		System.err.println("ShortestCommunicationPathFinder.initPathAndCostMatrix()Floyd: " + fullFloydTime);
...
/*
	private class PathMatrixItem{
		
		Bausteinschnittstelle s1;
		Bausteinschnittstelle s2;
		ArrayList path;
		int costs;
		
		private PathMatrixItem(Bausteinschnittstelle s1, Bausteinschnittstelle s2, ArrayList path, int costs){
...
*/	
	...
	public class MinimalCommunicationPath {
		 ...
		 * @param objekttyp
		 */
		private MinimalCommunicationPath(Collection<ModelElement> startAWBs, Collection<ModelElement> endAWBs, Objekttyp objekttyp){
			...
			//wenn keine Kommunikation nötig ist, weil Star- und Endbaustein auf demselben Gesamtbaustein liegen
			if (sameAWBInStartEnd!=null){
				...
				if (otPosition>=0){
					//den erst besten kürzesten Pfad von einer Startschnittstelle zu einer Endschnittstelle suchen
					loop: for (ModelElement startSS : startSchnittStellen) {
						...
						for (ModelElement endSS : endSchnittStellen) {
							...
							//wenn der aktuelle Pfad kostengünstiger oder kürzer ist oder noch gar kein Pfad gefunden wurde -> nimm den aktuellen
