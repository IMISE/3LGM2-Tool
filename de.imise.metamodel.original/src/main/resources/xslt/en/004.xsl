...
<!--type: html -->
<!--description: extracts all application components with their names, descriptions, 
	software products and database management sytsems -->
		...
		<html>
			...
			<body>
				...
				<xsl:variable name="anzahl"
					select="count(key('element','RechAnwendungsbaustein'))" />
				<xsl:choose>
					...
					<xsl:otherwise>
						<table cellspacing="0" cellpadding="3" border="1">
							<tr>
								...
								<th>
									<xsl:value-of select="'Software Product'" />
								</th>
								<!-- <th><xsl:value-of select="'Database System'" /></th> -->
								<th>
									<xsl:value-of select="'Database Management System'" />
								</th>
							</tr>
							<tbody align="left" valign="top">
								<xsl:for-each select="key('element','RechAnwendungsbaustein')">
									...
									<tr>
										...
										<!--Voraussetzungen: Softwareprodukthash und Datenbankhash immer 
											im End-Feld einer Relation definiert nur ein Softwareprodukt und nur eine 
											Datenbank -->

										<xsl:variable name="Datenbankhash"
											select="key('element','RawbDbsVerbindung')/field[@name='start'][text()=$hash]/../field[@name='end']/text()" />
										<xsl:variable name="DBVShash"
											select="key('element','DbsDbvsVerbindung')/field[@name='start'][text()=$Datenbankhash]/../field[@name='end']/text()" />
										<xsl:variable name="Anwendungsprodukthash"
											select="key('element','RawbAwpVerbindung')/field[@name='start'][text()=$hash]/../field[@name='end']/text()" />
										<xsl:variable name="Softwareprodukthash"
											select="key('element','AwpSwpVerbindung')/field[@name='start'][text()=$Anwendungsprodukthash]/../field[@name='end']/text()" />

										<td>
											<xsl:variable name="Softwareprodukt"
												select="key('element','Softwareprodukt')[@hash=$Softwareprodukthash]/field[@name='name']/text()" />
											<xsl:value-of select="$Softwareprodukt" />
											&#160;
										</td>

										<!-- <td> <xsl:variable name="Datenbank" select="key('element','Datenbanksystem')[@hash=$Datenbankhash]/field[@name='name']/text()" 
											/> <xsl:value-of select="$Datenbank" /> &#160; </td> -->

										<td>
											<xsl:variable name="DBVerwaltungssystem"
												select="key('element','DBVerwaltungssystem')[@hash=$DBVShash]/field[@name='name']/text()" />
				...
				<xsl:variable name="anzahl"
					select="count(key('element','KonAnwendungsbaustein'))" />
				<xsl:choose>
					<xsl:when test="$anzahl=0">
						<xsl:text>nonexistent</xsl:text>
					</xsl:when>
