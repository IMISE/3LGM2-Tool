...
/**
 * Eine TextPane, die xml-Elemente hervorhebt. Problematisch ist die langsame
 * Verarbeitung bei langen Texten.
 *
...
public class SimpleXMLTextPane extends ExtendedTextPane {
     ...
     */
    public SimpleXMLTextPane(final String xmlText) {
        super();
        initAttributeSets();
        initPatterns();
    ...
    /**
     * Initialisiert AttributeSets für die unterschiedlichen syntaktischen
     * Bestandteile der XML-Datei.
     */
    ...
    /**
     * Führt die Formatierung des gesamten Textes durch. Dies kann bei längeren
     * Texten lange dauern.
     */
    public void format() {
        String text = getText();
     ...
     */
    private void formatPart(final Matcher partMatcher) {
     ...
     */
    private void formatElement(final Matcher partMatcher) {
    ...
    /**
     * Formatiert den Text der aktuellen Group des Matchers mit dem
     * AttributeSet.
     */
    private void applyAttributeSet(final Matcher m, final AttributeSet attributeSet) {
        applyAttributeSet(m, attributeSet, 0, 0);
    }

    /**
     * Formatiert den Text der aktuellen Group (abzüglich bestimmter Ränder
     * links und rechts) des Matchers mit dem AttributeSet.
     */
    private void applyAttributeSet(final Matcher m, final AttributeSet attributeSet, final int additionalOffset, final int lengthReduction) {
        int offset = m.start();
        int length = m.group().length();
        getStyledDocument().setCharacterAttributes(offset + additionalOffset, length - lengthReduction, attributeSet, true);
    }
}
