...
public class ImageChooser extends ExtendedFileChooser {
     ...
     * @param pathKey
     */
    public ImageChooser(final Object pathKey) {
        this(pathKey, null);
    }
    ...
    public static File getImageFile(final Component parent, final File defaultPath) {
        ImageChooser fc = new ImageChooser(ImageChooser.class, defaultPath);
        int answer = fc.showDialog(parent);
        if (answer == ImageChooser.APPROVE_OPTION) {
            File iconFile = fc.getSelectedFile();
            return iconFile;
        }
        return null;
    }
    ...
    private class ImagePreviewer extends JLabel {
         ...
         * @param f
         */
        public void configure(final File f) {
