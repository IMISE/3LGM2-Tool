 ...
 * container's components on the specified X or Y axis. Components can be layed
 * out at their preferred size or at a relative size. When relative sizing is
 * used the component must be added to the container using a relative size
 * constraint, which is simply a Float value. The space available for relative
 * sized components is determined by subtracting the preferred size of the other
 * components from the space available in the container. Each component is then
 ...
 * Float(2)); There is a total of 3 relative units. If the container has 300
 * pixels of space available then component1 will get 100 and component2, 200.
 * It is possible that rounding errors will occur in which case you can specify
 * a rounding policy to use to allocate the extra pixels. By defaults components
 * are center aligned on the secondary axis however this can be changed at the
 * container or component level.<br>
 * Source: <a href=
 * "https://tips4java.wordpress.com/2008/11/02/relative-layout/">https://tips4java.wordpress.com/2008/11/02/relative-layout/</a><br>
 * Source: <a href=
 * "http://www.camick.com/java/source/RelativeLayout.java">http://www.camick.com/java/source/RelativeLayout.java</a>
 */
public class RelativeLayout implements LayoutManager2, java.io.Serializable {
     ...
     */
    public RelativeLayout(final int axis) {
        this(axis, 0);
    }
     ...
     * @param gap the gap
     */
    public RelativeLayout(final int axis, final int gap) {
        setAxis(axis);
        setGap(gap);
        setBorderGap(gap);
    }
    ...
    /*
     * Set the alignment of the component on the opposite axis. For X-AXIS this
     * would refer to the Y alignemt. For Y-AXIS this would refer to the X
     * alignment. Must be between 0.0 and 1.0, or -1. Values can be specified
     * using: RelativeLayout.LEADING RelativeLayout.CENTER
     * RelativeLayout.TRAILING RelativeLayout.COMPONENT - the getAlignemntX/Y
     * method for the opposite axis will be used
     */
     ...
     */
    public void setFillGap(final int fillGap) {
     ...
     * not been allocated to a component. DO_NOTHING FIRST - extra pixels added
     * to the first relative component LAST - extra pixels added to the last
     * relative component LARGEST (default) - extra pixels added to the larger
     * relative component EQUAL - a single pixel is added to each relative
     * component (until pixels are used up)
     */
     ...
     */
    public Float getConstraints(final Component component) {
     ...
     */
    private void layoutContainerHorizontally(final Container parent) {
     ...
     */
    private int getLocationY(final Component component, final int height) {
     ...
     */
    private void layoutContainerVertically(final Container parent) {
        int components = parent.getComponentCount();
        int visibleComponents = getVisibleComponents(parent);
     ...
     */
    private int getLocationX(final Component component, final int width) {
        //  Use the Container alignment policy
     ...
     */
    private int[] allocateRelativeSpace(final Container parent, final int spaceAvailable, final float relativeTotal) {
        int spaceUsed = 0;
        int components = parent.getComponentCount();
     ...
     */
    protected void adjustForRounding(final int[] relativeSpace, final int spaceRemaining) {
     ...
     */
    private void adjustFirst(final int[] relativeSpace, final int spaceRemaining) {
     ...
     */
    private void adjustLast(final int[] relativeSpace, final int spaceRemaining) {
     ...
     */
    private void adjustLargest(final int[] relativeSpace, final int spaceRemaining) {
        int largest = 0;
        int largestSpace = 0;
     ...
     */
    private void adjustEqual(final int[] relativeSpace, int spaceRemaining) {
        for (int i = 0; i < relativeSpace.length; i++) {
            if (relativeSpace[i] > 0) {
                if (spaceRemaining > 0) {
                    relativeSpace[i]++;
    ...
    private int getVisibleComponents(final Container container) {
    ...
    private Dimension getDimension(final Component component, final int type) {
        switch (type) {
        case PREFERRED:
