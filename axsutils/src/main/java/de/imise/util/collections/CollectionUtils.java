...
/**
 * Stellt Funktionen für Objektsammlungen bereit, die <code>Arrays</code> und <code>Collections</code>
 * nicht bieten.
 * @author AXS
 * @version 0.0.7
 */
public abstract class CollectionUtils {
	 ...
	 * Index ab 1 besteht. Der erste Name dieser Form, der nicht in der übergebene 
	 * <code>Collection</code> vorkommt, wird zurückgegeben.
	 * @param prefix
	 * @param alreadyExistingNames
	 * @return
	 */
	public static final String getNextIndicatedName(String prefix, Collection<?> alreadyExistingNames){
	 ...
	 * @return
	 */
	public static final String getNextIndicatedName(String prefix, String postfix, Collection<?> alreadyExistingNames){
		if (prefix == null)
			prefix = "";
		if (postfix == null)
			postfix = "";
	...
	public static <T> Class<T>[] joinClassArrays(Class<? extends T>[] array1, Class<? extends T>[] array2){
	...
	/**
	 * Sortiert das <code>Object</code> <code>elementToAdd</code> in die uebergebene breits sortierte 
	 * Liste nach den Vorgaben des Comparators ein.
	 */
	public static void insertElementInSortedList(List <Object> sortedList,  Object elementToAdd, Comparator <Object>comparator) {
		sortedList.add(getInsertPosition(sortedList, elementToAdd, comparator), elementToAdd);
	}

	/**
	 * Liefert die Position an der ein Vergleich über den Comparator das übergebene Object in die Liste
	 ...
	 * @return
	 */
	public static final int getInsertPosition(List<Object> sortedList, Object elementToAdd, Comparator<Object> comparator) {
		int l = 0;
		int r = sortedList.size() - 1;
		// Teile und Herrsche :))
		while (r >= l) {
			int pos = l + ((r - l) / 2);
			Object o = sortedList.get(pos);
			int compareValue = comparator.compare(elementToAdd, o);
			if (compareValue > 0) {
				l = pos + 1;
	 ...
	 */
	public static final <T> ArrayList<T> getAllElementsOf(Collection<T> source, Class<T> clazz){
	 ...
	 * @see #getAllElementsOf(Collection, Class, boolean)
	 */
	public static final <T> ArrayList<T> getAllInstancesOf(Collection<?> source, Class<T> clazz){
	 ...
	 * @return
	 */
	public static boolean containsInstancesOf (Collection<?> source, Class<?> clazz, boolean includeSubClasses) {
		for (Object o : source) {
	 ...
	 * @return
	 */
	public static boolean containsOnlyInstancesOf (Collection<?> source, Class<?> clazz, boolean includeSubClasses) {
		for (Object o : source) {
			if (includeSubClasses) {
				if (!clazz.isAssignableFrom(o.getClass()))
					return false;
	 ...
	 * @return
	 */
	public static boolean containsOnlyInstancesOf (Collection<?> source, boolean includeSubClasses, Class<?>... classes) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	private static final <T> ArrayList<T> getAllElementsOf(Collection<?> source, Class<T> clazz, boolean strict){
	 ...
	 * @param list
	 */
	public static final ArrayList<?> getNoMultiplesList(ArrayList<?> list){
	 ...
	 * @param list
	 */
	public static final void removeMultiples(ArrayList<?> list){
		for (int i=0; i<list.size(); i++){
			Object o = list.get(i);
	 ...
	 * 		col1
	 */
	public static final <T> Collection<T> addNonMultiples(Collection<T> col1, Collection<T> col2){
	...
	/**
	 * Wandelt <code>rows</code> in ein <code>Object[][]</code> um, falls es sich um eine gültige
	 * <code>Collection</code> handelt.<br>
	 * Sonst wird <code>null</code> zurückgegeben.
	 * <p>
	 * <code>rows</code> ist eine gültige <code>Collection</code>, falls ihre Elemente selbst
	 * wieder vom Typ {@link Collection} sind und alle die gleiche Elementanzahl besitzen.<br>
	 * Das heißt, dass <code>rows</code> eine <code>Collection</code> aller Zeilen sein muss.
	 * Außerdem darf <code>rows</code> nicht <code>null</code> sein.
	 * <p>
	 * Diese Methode bildet dabei die Zeilen und Spalten von <code>rows</code> identisch auf
	 * die Zeilen und Spalten des zurückgegebenen Arrays ab.
	 * 
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static <T> T[][] toMatrixArray(Collection<Collection<? extends T>> rows) throws ArrayIndexOutOfBoundsException, IllegalArgumentException{
		...
		for (Object nextRow : rows) {
			...
			if(nextRow instanceof Collection<?>) { // rows ist mehrdimensional
				...
				throw new IllegalArgumentException("Die übergebene Collection ist nicht 2-dimensional");
			}

			if (array == null) // Initiales Erstellen des Arrays
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static <T> T[] toArray(Collection<? extends T> row) throws ArrayIndexOutOfBoundsException, IllegalArgumentException{
		if(row == null)
			return null;
	 ...
	 * Dabei wird auf jedes {@link Object} aus <code>objectArray</code> {@link Object#toString()}
	 * angewendet und das Resultat an die entsprechende Stelle im <code>String[][]</code> geschrieben.
	 * <p>
	 ...
	 * @return
	 */
	public static String[][] toStringArray(Object[][] objectArray) throws ArrayIndexOutOfBoundsException, IllegalArgumentException {
	 ...
	 * @return
	 */
	public static final String toString(long[] longs) {
	 ...
	 * @return
	 */
	public static final String toString(double[] doubles) {
		StringBuilder sb = new StringBuilder("[");
	 ...
	 * @return
	 */
	public static final String toString(int[] ints) {
		StringBuilder sb = new StringBuilder("[");
	 ...
	 * @return
	 */
	public static final String toListString(Object[] array) {
	 ...
	 * @return
	 */
	public static final String toJoinedString(Object[] array) {
	 ...
	 * @return
	 */
	public static final String toString(Object[] array, String suffix, String postfix, String delimiter) {
		StringBuilder sb = new StringBuilder();
		if (suffix != null)
			sb.append(suffix);
		if (array != null) {
	 ...
	 */
	public static int maxSize(Collection<?>... allCollections) throws NullPointerException {
		int max = 0;
