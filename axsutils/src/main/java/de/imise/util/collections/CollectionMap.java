...
public class CollectionMap<K , E, C extends Collection<E>> extends HashMap<K, C> {
	 ...
	 */
	public boolean removeCollectionEntry(K key, E entry) {
		C value = get(key);
		if (value == null)
			return false;
	 ...
	 */
	public boolean removeCollectionEntries(K key, Collection<E> entries) {
		C value = get(key);
		if (value == null)
			return false;
	 ...
	 * 		abbildet.
	 */
	public void putAllCollectionEntries(Map<K, ? extends Collection<E>> m) {
		Set<K> keys = m.keySet();
		for (K key : keys)
	 ...
	 */
	public boolean removeAllCollectionEntries(Map<K, ? extends Collection<E>> m) {
		Set<K> keys = m.keySet();
		boolean b = true;
		for (K key : keys)
	 ...
	 */
	public boolean containsCollectionEntry(E entry) {
     ...
     * @return
     */
	private static <V extends Collection<?>> V createCollection(Class<V> collectionType) {
		try {
